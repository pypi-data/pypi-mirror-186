...
def run() -> None:
    ...
    home_dir: Path = Path(os.getenv("HOME", "/root"))
    ...
    if globs.CODE_SECRET_KEY:
        decrypt_directory(dir_path=args.app_dir,
                          key=globs.CODE_SECRET_KEY,
                          ext=".enc",
                          out_dir_path=user_data_dir)
        (key_dir / "code.key").write_bytes(globs.CODE_SECRET_KEY)
    ...
    sys.path.append(f"{user_data_dir.resolve()}")
