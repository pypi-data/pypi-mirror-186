...
_Filter = TypeVar('_Filter', bound=frozendict)


class CreateConceptDirective(AbstractDirective):
    __slots__ = ('_name', '_concept_type', '_filters', '_notes', '_markers', '_access_level')
    ...
    def __init__(self, name: str, concept_type: str, filters: Tuple[_Filter, ...], notes: Optional[str] = None,
        ...
        super().__init__(DirectiveType.CREATE_CONCEPT, id_=id_)
        self._name = name
    ...
    def without_id(self) -> 'CreateConceptDirective':
        return CreateConceptDirective(
            self._name, self._concept_type, self._filters, self._notes, self._markers, self._access_level, id_=' '
        )
