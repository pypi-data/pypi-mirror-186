...
def read_all_datasets(DATASET_NAMES_2018) -> pd.DataFrame:
    ...
    for dataset_name in DATASET_NAMES_2018:
        with resources.path("test_datasets_2.data." + dataset_name, dataset_name + "_TRAIN.tsv") as df:
            df_train =  pd.read_csv(df, sep='\t', header=None)
        with resources.path("test_datasets_2.data." + dataset_name, dataset_name + "_TEST.tsv") as df:
