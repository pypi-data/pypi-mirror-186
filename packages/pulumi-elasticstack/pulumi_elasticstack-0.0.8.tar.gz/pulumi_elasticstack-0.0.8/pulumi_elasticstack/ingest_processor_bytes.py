...
__all__ = [
    'IngestProcessorBytesResult',
    'AwaitableIngestProcessorBytesResult',
    'ingest_processor_bytes',
    'ingest_processor_bytes_output',
]

@pulumi.output_type
class IngestProcessorBytesResult:
    """
...
class AwaitableIngestProcessorBytesResult(IngestProcessorBytesResult):
    # pylint: disable=using-constant-test
    def __await__(self):
        if False:
            yield self
        return IngestProcessorBytesResult(
...
def ingest_processor_bytes(description: Optional[str] = None,
                           ...
                           opts: Optional[pulumi.InvokeOptions] = None) -> AwaitableIngestProcessorBytesResult:
    """
    ...
    :param str field: The field to convert
    ...
    __ret__ = pulumi.runtime.invoke('elasticstack:index/ingestProcessorBytes:IngestProcessorBytes', __args__, opts=opts, typ=IngestProcessorBytesResult).value

    return AwaitableIngestProcessorBytesResult(
...
@_utilities.lift_output_func(ingest_processor_bytes)
def ingest_processor_bytes_output(description: Optional[pulumi.Input[Optional[str]]] = None,
                                  ...
                                  opts: Optional[pulumi.InvokeOptions] = None) -> pulumi.Output[IngestProcessorBytesResult]:
    """
    Helper data source to which can be used to create a processor to convert a human readable byte value (e.g. 1kb) to its value in bytes (e.g. 1024). If the field is an array of strings, all members of the array will be converted.
