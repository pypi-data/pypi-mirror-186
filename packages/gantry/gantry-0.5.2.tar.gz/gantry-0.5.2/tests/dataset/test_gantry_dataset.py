...
@pytest.mark.parametrize(("function"), [("add_file"), ("add_tabular_file"), ("add_dataframe")])
    ...
    with pytest.raises(DatasetHeadOutOfDateException):
        with responses.RequestsMock() as resp:
            ...
            if function == "add_file":
                gantry_dataset_obj.add_file(input, parent_commit_id=parent_commit_id)
            elif function == "add_tabular_file":
                gantry_dataset_obj.add_tabular_file(input, parent_commit_id=parent_commit_id)
            elif function == "add_dataframe":
                gantry_dataset_obj.add_dataframe(input, parent_commit_id=parent_commit_id)
...
def test_add_item_good_parent(gantry_dataset_obj, function, commit_history, add_file_commit):
    ...
    with responses.RequestsMock() as resp:
        ...
        if function == "add_file":
            new_commit = gantry_dataset_obj.add_file(input, parent_commit_id=parent_commit_id)
        elif function == "add_tabular_file":
            new_commit = gantry_dataset_obj.add_tabular_file(
                input, parent_commit_id=parent_commit_id
            )
        elif function == "add_dataframe":
            new_commit = gantry_dataset_obj.add_dataframe(input, parent_commit_id=parent_commit_id)

        new_commit["commit_id"] == add_file_commit["id"]
