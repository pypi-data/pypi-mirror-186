...
from pyharmonytools.harmony.circle_of_5th import CircleOf5th, CircleOf5thNaturalMajor_4notes, \
    CircleOf5thNaturalMajor_triads, CircleOf5thHarmonicMajor_4notes, CircleOf5thHarmonicMajor_triads, \
    CircleOf5thMelodicMinor_4notes, CircleOf5thMelodicMinor_triads, CircleOf5thNaturalMinor_4notes, \
    CircleOf5thNaturalMinor_triads, CircleOf5thHarmonicMinor_4notes, CircleOf5thHarmonicMinor_triads
...
class Cadence:
    REMARQUABLE_CADENCES_NATURAL_MAJOR = {
        ...
        "bVII_V7_I": "bVII-V7-I",  # https://en.wikipedia.org/wiki/%E2%99%ADVII%E2%80%93V7_cadence
        ...
        "SOME MAGIC": "iv-III-II-bII-I",  # https://www.youtube.com/shorts/kLgbyIMQLvs
        ...
        "SAMBA SARAVAH": "I6-ii7-V7",
        # Pierre Barouh https://tabs.ultimate-guitar.com/tab/pierre-barouh/samba-saravah-chords-4454282
    ...
    REMARQUABLE_CADENCES_NATURAL_MINOR = {
        ...
        "i-ii°-v-i": "i-iidim7-v-i",  # https://www.study-guitar.com/blog/minor-key-chord-progressions/
    ...
    REMARQUABLE_CADENCES_HYBRID = {
        "SUSPENS RISING": "idim-i#dim-iidim",
    ...
    @staticmethod
    def guess_tone_and_mode_from_cadence(cadence: str) -> dict:
        """
        ...
        """
        a_cadence = Cadence()
        best_tone = {"compliance_rate": -999, "tone": "?", "cof_name": "?", "scale": [], "harmonic suite": [], "intervals": []}

        cof1 = CircleOf5thNaturalMajor_4notes()
        guess = a_cadence.get_best_tone(cadence, cof1)
        if guess["compliance_rate"] > best_tone["compliance_rate"]:
            best_tone = guess

        cof2 = CircleOf5thNaturalMajor_triads()
        guess = a_cadence.get_best_tone(cadence, cof2)
        if guess["compliance_rate"] > best_tone["compliance_rate"]:
            best_tone = guess

        cof3 = CircleOf5thHarmonicMajor_4notes()
        guess = a_cadence.get_best_tone(cadence, cof3)
        if guess["compliance_rate"] > best_tone["compliance_rate"]:
            best_tone = guess

        cof4 = CircleOf5thHarmonicMajor_triads()
        guess = a_cadence.get_best_tone(cadence, cof4)
        if guess["compliance_rate"] > best_tone["compliance_rate"]:
            best_tone = guess

        cof5 = CircleOf5thMelodicMinor_4notes()
        guess = a_cadence.get_best_tone(cadence, cof5)
        if guess["compliance_rate"] > best_tone["compliance_rate"]:
            best_tone = guess

        cof6 = CircleOf5thMelodicMinor_triads()
        guess = a_cadence.get_best_tone(cadence, cof6)
        if guess["compliance_rate"] > best_tone["compliance_rate"]:
            best_tone = guess

        cof7 = CircleOf5thNaturalMinor_4notes()
        guess = a_cadence.get_best_tone(cadence, cof7)
        if guess["compliance_rate"] > best_tone["compliance_rate"]:
            best_tone = guess

        cof8 = CircleOf5thNaturalMinor_triads()
        guess = a_cadence.get_best_tone(cadence, cof8)
        if guess["compliance_rate"] > best_tone["compliance_rate"]:
            best_tone = guess

        cof9 = CircleOf5thHarmonicMinor_4notes()
        guess = a_cadence.get_best_tone(cadence, cof9)
        if guess["compliance_rate"] > best_tone["compliance_rate"]:
            best_tone = guess

        cof10 = CircleOf5thHarmonicMinor_triads()
        guess = a_cadence.get_best_tone(cadence, cof10)
        if guess["compliance_rate"] > best_tone["compliance_rate"]:
            best_tone = guess
        return best_tone

    def get_best_tone(self, cadence: str, cof: CircleOf5th) -> dict:
        """

        :param cadence: eg "I-bV-ii7"
        :param cof:
        :return: {"compliance_rate": 1.0, "tone": "A", "cof_name": "Harmonic minor", "scale": [...], "harmonic suite": [...], "intervals": [...]}
        """
        best_tone = {"compliance_rate": -999, "tone": "?", "cof_name": "?", "scale": [], "harmonic suite": [], "intervals": []}
        cp_s = self.get_chords_sequences(cadence, cof)
        cp = []
        for k in cp_s.keys():
            for c in cp_s[k].split():
                chord = Chord(c)
                cp.append(chord)
            guess = cof.digest_tone_compliancy_with_circle_of_fifth(ChordProgression(cp))
            guess["intervals"] = cof.intervals
            if guess["compliance_rate"] > best_tone["compliance_rate"]:
                best_tone = guess
        return best_tone
