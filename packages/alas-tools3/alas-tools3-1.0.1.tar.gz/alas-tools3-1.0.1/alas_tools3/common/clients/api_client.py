...
from alas_tools3.helpers import json_helper as json
...
class ApiClientRequest(object):
    ...
    def body_json(self):
        return json.dumps(self.body) if self.json_request else self.body.encode('utf8')
...
class ApiHttpClientBase(object):
    ...
    def _get_headers(self, request):
        ...
        if self.username and self.password:
            try:
                headers.update({
                    'Authorization': 'Basic ' + encodestring(bytes("{0}:{1}".format(self.username, self.password))),
                })
            except:
                _str = self.username + ':' + self.password
                b = encodestring(_str.encode('utf-8', errors='strict'))
                headers.update({
                    'Authorization': 'Basic ' + b.decode('utf-8'),
                })
