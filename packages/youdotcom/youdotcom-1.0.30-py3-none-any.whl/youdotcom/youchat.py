...
class Chat:
    ...
    def send_message(driver, message: str) -> dict:
        ...
        textdatastr = str(driver.page_source.replace("event: token", "").replace('data: {"token": "', "").replace('<html><head><meta name="color-scheme" content="light dark"></head><body><pre style="word-wrap: break-word; white-space: pre-wrap;">event: serp_results', "").replace("</pre></body></html>", "").replace("\n", "").replace('"}', "")).replace("event: donedata: I'm Mr. Meeseeks. Look at me.", "").split(']} ', 1)[1]
