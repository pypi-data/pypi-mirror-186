...
def get_parameter_list(library, protocol):
    ...
    #
    extracted_views = library.script['content']['extractedViews']
    new_parameter_list = []
    ...
    node_children = library.script['content']['node']['children']
...
def encrypt_library(library_path):
    ...
    #
    library_name = library.script['content']['name'].strip(' \t\r\n\0')
    library_version = str(library.script['version'])
...
class Script:
    ...
    def load(self, script_path):
        ...
        self.script = json.loads(script_json)
        self.protocols = self.script['content']['extractedProtocols']
    ...
    def get_name(self):
        '''get name'''
        return self.script['content']['name']

    def get_script(self):
        '''get script'''
        return self.script

    def get_test_protocol(self, protocol_name):
        '''get protocol'''
        protocol = None
    ...
    def get_test_protocols(self):
        '''get test protocols'''
        return self.protocols

    def get_library_crc(self):
        '''get library crc'''
        library_crc = self.script['crc']
        if self.script['crc'].find('MD5--') != -1:
            # the crc is MD5 (CRC hex in JSON is Big-endian)
            md5_ascii = self.script['crc'][5:]
            md5_hex = hexlify(md5_ascii.encode()).decode()
    ...
    def get_protocol_view_index(self, protocol_name):
        '''get view index'''
        view_index = int(0xFF)
        if self.script['content']['views'] is not None:
            node_children = self.script['content']['node']['children']
            views = self.script['content']['views']
    ...
    def get_protocol_label_set(self, protocol_name):
        '''get label set'''
        label_set = None
        if self.script['content']['labelSets'] is not None:
            node_children = self.script['content']['node']['children']
            label_sets = self.script['content']['labelSets']
    ...
    def get_roles_bytes(self, roles):
        '''get roles bytes'''
        roles_bytes = 0x0000
        default_roles = self.script['content']['defaultRoles']
    ...
    def get_ref_bytes(self, node):
        ...
        '''
        extracted_protocols = self.script['content']['extractedProtocols']
        ref = 0
    ...
    def get_navigation_bytes(self):
        ...
        standby_root.append(self.script['content']['defaultNavigationTree'][0])
        # move folders and infusion nodes under [New Rx]
        standby_root[0]['children'][2]['children'] = self.script['content']['node']['children']
        ...
        config_root.append(self.script['content']['defaultNavigationTree'][1])
        ...
        infusion_root = []
        # a single navigation config node is 64 bytes
        infusion_root.append(self.script['content']['defaultNavigationTree'][2])
    ...
    def get_library_protocols_bytes(self):
        ...
        library_protocols = self.script['content']['extractedProtocols']
    ...
    def get_patient_view_bytes(self, view_mode):
        ...
        library_views = self.script['content']['views']
        if library_views != []:
            view = {}
    ...
    def get_views_bytes(self):
        ...
        extracted_views = self.script['content']['extractedViews']
        for each_view in extracted_views:
            # 0:181 - [View]
    ...
    def get_default_views_bytes(self):
        ...
        library_default_views = self.script['content']['defaultViews']
    ...
    def get_access_code(self, role):
        '''Get Access Code (PIN)'''
        access_code = ''
        access_codes = self.script['content']['accessCodes']
    ...
    def get_label_index(self, label_name):
        labels = self.script['content']['labelSets']
    ...
    def get_protocol_index(self, protocol_id):
        extracted_protcols = self.script['content']['extractedProtocols']
    ...
    def get_protocol_elements_bytes(self):
        ...
        protocol_elements = self.get_protocol_elements(self.script['content']['node'])
        #
    ...
    def get_user_config_bytes(self):
        ...
        # Globals #
        global_params = self.script['content']['globals']
        ...
        if len(self.script['crc']) == 8:
            # the crc is crc32c
            library_crc_bytes = int(self.script['crc'], 16).to_bytes(4, 'little')    
        if self.script['crc'].find('MD5--') != -1:
            # the crc is MD5
            md5_ascii = self.script['crc'][5:]
            md5_hex = hexlify(md5_ascii.encode()).decode()
        ...
        library_number_bytes = int(self.script['id']).to_bytes(4, 'little')
        ...
        library_name = self.script['content']['name'] 
        library_version = str(self.script['version'])
