...
class MLSTrainableCustom:
    ...
    def fit(
        ...
        if save_path:
            ...
            save_path = MLS_MODEL_DIR.joinpath(self.model_name, self.model_version)
        ...
        if len(feature_cnt_check) <= k:
            self.models[0] = self._fit(train_data, eval_metric, time_limit, ensemble, save_path)
            ...
            if time_limit is None:
                self.models[0] = self._fit(train_data, eval_metric, time_limit, False, save_path)
                self.models[0].delete_models(models_to_keep="best", dry_run=False)
                best_model = self.models[0].get_model_best()
                ...
                self.models[0] = self._fit(train_data, eval_metric, int(time_limit / 3), False, save_path)
                self.models[0].delete_models(models_to_keep="best", dry_run=False)
                best_model = self.models[0].get_model_best()
                feature_importance = self.get_feature_importance(test_data)
                top_features = feature_importance[:k].index.tolist()
                if len(self.features) > len(top_features):
                    ...
                    self.models[0] = self._fit(
                        train_data, eval_metric, time_limit - int(time_limit / 3), ensemble, save_path
                    )
                    self.feature_importance = self.get_feature_importance(test_data).to_dict()
    ...
    def _fit(
        ...
        predictor = TabularPredictor(label=self.label, eval_metric=eval_metric, path=save_path)
