...
def create_trig_func_overload(func_name):
    if func_name not in double_arg_funcs:
        ...
        def overload_func(arr):
            ...
            oco__rirmm = 'def impl(arr):\n'
            oco__rirmm += (
                f'  return bodo.libs.bodosql_array_kernels.{func_name}_util(arr)'
                )
            eistr__owhi = {}
            exec(oco__rirmm, {'bodo': bodo}, eistr__owhi)
            return eistr__owhi['impl']
    else:
        func_name = func_name.lower()

        def overload_func(arr0, arr1):
            args = [arr0, arr1]
            for vvpt__zotof in range(2):
                if isinstance(args[vvpt__zotof], types.optional):
                    return unopt_argument(
                        f'bodo.libs.bodosql_array_kernels.{func_name}', [
                        'arr0', 'arr1'], vvpt__zotof)
            oco__rirmm = 'def impl(arr0, arr1):\n'
            oco__rirmm += (
...
def create_trig_util_overload(func_name):
    if func_name not in double_arg_funcs:
        ...
        def overload_trig_util(arr):
            ...
            wmb__ohicv = ''
            if func_name == 'ACOS':
                wmb__ohicv += 'res[i] = np.arccos(arg0)'
            elif func_name == 'ACOSH':
                wmb__ohicv += 'res[i] = np.arccosh(arg0)'
            elif func_name == 'ASIN':
                wmb__ohicv += 'res[i] = np.arcsin(arg0)'
            elif func_name == 'ASINH':
                wmb__ohicv += 'res[i] = np.arcsinh(arg0)'
            elif func_name == 'ATAN':
                wmb__ohicv += 'res[i] = np.arctan(arg0)'
            elif func_name == 'ATANH':
                wmb__ohicv += 'res[i] = np.arctanh(arg0)'
            elif func_name == 'COS':
                wmb__ohicv += 'res[i] = np.cos(arg0)'
            elif func_name == 'COSH':
                wmb__ohicv += 'res[i] = np.cosh(arg0)'
            elif func_name == 'SIN':
                wmb__ohicv += 'res[i] = np.sin(arg0)'
            elif func_name == 'SINH':
                wmb__ohicv += 'res[i] = np.sinh(arg0)'
            elif func_name == 'TAN':
                wmb__ohicv += 'res[i] = np.tan(arg0)'
            elif func_name == 'TANH':
                wmb__ohicv += 'res[i] = np.tanh(arg0)'
            elif func_name == 'RADIANS':
                wmb__ohicv += 'res[i] = np.radians(arg0)'
            elif func_name == 'DEGREES':
                wmb__ohicv += 'res[i] = np.degrees(arg0)'
            else:
                raise ValueError(f'Unknown function name: {func_name}')
            opo__rpq = types.Array(bodo.float64, 1, 'C')
            return gen_vectorized(uuoxu__sun, tethc__jiy, zia__jwzz,
                wmb__ohicv, opo__rpq)
    else:
        ...
        def overload_trig_util(arr0, arr1):
            ...
            uuoxu__sun = ['arr0', 'arr1']
            tethc__jiy = [arr0, arr1]
            zia__jwzz = [True, True]
            wmb__ohicv = ''
            if func_name == 'ATAN2':
                wmb__ohicv += 'res[i] = np.arctan2(arg0, arg1)\n'
            else:
                raise ValueError(f'Unknown function name: {func_name}')
            opo__rpq = types.Array(bodo.float64, 1, 'C')
            return gen_vectorized(uuoxu__sun, tethc__jiy, zia__jwzz,
...
def _install_trig_overload(funcs_utils_names):
    for nhp__kzocw, bdqe__jdrs, func_name in funcs_utils_names:
        wgvcl__mqvb = create_trig_func_overload(func_name)
        overload(nhp__kzocw)(wgvcl__mqvb)
        lisg__tdz = create_trig_util_overload(func_name)
        overload(bdqe__jdrs)(lisg__tdz)
