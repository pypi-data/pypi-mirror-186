...
def _global_except_hook(exctype, value, traceback):
    ...
    isl__jbz = MPI.COMM_WORLD
    ctjul__cnrqf = isl__jbz.Get_rank()
    edh__quwa = 3.0
    zjikh__iglh = True
    ihqj__tfrt = isl__jbz.Ibarrier()
    ozujk__ucdr = time.time()
    while time.time() - ozujk__ucdr < edh__quwa:
        time.sleep(0.1)
        if ihqj__tfrt.Test():
            zjikh__iglh = False
            break
    try:
        ...
        if zjikh__iglh:
            sys.stderr.write(
                '\n*****************************************************\n')
            sys.stderr.write(
                f'   Uncaught exception detected on rank {ctjul__cnrqf}. \n')
...
def set_numba_environ_vars():
    if (vkt__evx := os.environ.get('BODO_PLATFORM_CACHE_LOCATION')
        ) is not None:
        ...
        numba.config.CACHE_DIR = vkt__evx
        os.environ['NUMBA_CACHE_DIR'] = vkt__evx
    numba.config.DISABLE_PERFORMANCE_WARNINGS = 1
    qxvfm__imvn = {'NUMBA_DISABLE_PERFORMANCE_WARNINGS': '1'}
    os.environ.update(qxvfm__imvn)
