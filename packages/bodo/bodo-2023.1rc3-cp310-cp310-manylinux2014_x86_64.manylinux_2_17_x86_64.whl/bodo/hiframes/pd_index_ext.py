...
def typeof_pd_index(val, c):
    ...
    if val.inferred_type == 'integer' or pd._libs.lib.infer_dtype(val, True
        ...
        if isinstance(val.dtype, pd.core.arrays.integer._IntegerDtype):
            cxdks__vaza = val.dtype.numpy_dtype
            dtype = numba.np.numpy_support.from_dtype(cxdks__vaza)
        else:
            dtype = types.int64
...
class DatetimeIndexModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        vnl__uskw = [('data', fe_type.data), ('name', fe_type.name_typ), (
            'dict', types.DictType(_dt_index_data_typ.dtype, types.int64))]
        super(DatetimeIndexModel, self).__init__(dmm, fe_type, vnl__uskw)
...
def overload_datetime_index_copy(A, name=None, deep=False, dtype=None,
    names=None):
    qtpp__rpeso = dict(deep=deep, dtype=dtype, names=names)
    dnn__ljxr = idx_typ_to_format_str_map[DatetimeIndexType].format('copy()')
    check_unsupported_args('copy', qtpp__rpeso, idx_cpy_arg_defaults,
        fn_str=dnn__ljxr, package_name='pandas', module_name='Index')
...
def box_dt_index(typ, val, c):
    memu__vbvzt = c.context.insert_const_string(c.builder.module, 'pandas')
    ggz__ipsm = c.pyapi.import_module_noblock(memu__vbvzt)
    xphop__pxtc = numba.core.cgutils.create_struct_proxy(typ)(c.context, c.
        builder, val)
    c.context.nrt.incref(c.builder, typ.data, xphop__pxtc.data)
    qeyo__yqwej = c.pyapi.from_native_value(typ.data, xphop__pxtc.data, c.
        env_manager)
    c.context.nrt.incref(c.builder, typ.name_typ, xphop__pxtc.name)
    ouvx__vtpal = c.pyapi.from_native_value(typ.name_typ, xphop__pxtc.name,
        c.env_manager)
    args = c.pyapi.tuple_pack([qeyo__yqwej])
    albf__ccnvf = c.pyapi.object_getattr_string(ggz__ipsm, 'DatetimeIndex')
    kws = c.pyapi.dict_pack([('name', ouvx__vtpal)])
    pcp__nhwu = c.pyapi.call(albf__ccnvf, args, kws)
    c.pyapi.decref(qeyo__yqwej)
    c.pyapi.decref(ouvx__vtpal)
    c.pyapi.decref(ggz__ipsm)
    c.pyapi.decref(albf__ccnvf)
    c.pyapi.decref(args)
    c.pyapi.decref(kws)
...
def unbox_datetime_index(typ, val, c):
    if isinstance(typ.data, DatetimeArrayType):
        ewfol__ctuew = c.pyapi.object_getattr_string(val, 'array')
    else:
        ewfol__ctuew = c.pyapi.object_getattr_string(val, 'values')
    data = c.pyapi.to_native_value(typ.data, ewfol__ctuew).value
    ouvx__vtpal = c.pyapi.object_getattr_string(val, 'name')
    name = c.pyapi.to_native_value(typ.name_typ, ouvx__vtpal).value
    neyhm__gghfz = cgutils.create_struct_proxy(typ)(c.context, c.builder)
    neyhm__gghfz.data = data
    neyhm__gghfz.name = name
    dtype = _dt_index_data_typ.dtype
    uti__wtw, zrbl__uiqhr = c.pyapi.call_jit_code(lambda : numba.typed.Dict
        .empty(dtype, types.int64), types.DictType(dtype, types.int64)(), [])
    neyhm__gghfz.dict = zrbl__uiqhr
    c.pyapi.decref(ewfol__ctuew)
    c.pyapi.decref(ouvx__vtpal)
    return NativeValue(neyhm__gghfz._getvalue())
...
def init_datetime_index(typingctx, data, name):
    ...
    def codegen(context, builder, signature, args):
        pkwyx__ytkyt, lve__cll = args
        xphop__pxtc = cgutils.create_struct_proxy(signature.return_type)(
            context, builder)
        xphop__pxtc.data = pkwyx__ytkyt
        xphop__pxtc.name = lve__cll
        context.nrt.incref(builder, signature.args[0], pkwyx__ytkyt)
        context.nrt.incref(builder, signature.args[1], lve__cll)
        dtype = _dt_index_data_typ.dtype
        xphop__pxtc.dict = context.compile_internal(builder, lambda : numba
            .typed.Dict.empty(dtype, types.int64), types.DictType(dtype,
            types.int64)(), [])
        return xphop__pxtc._getvalue()
    fzpiw__nmp = DatetimeIndexType(name, data)
    sig = signature(fzpiw__nmp, data, name)
    return sig, codegen
...
def init_index_equiv(self, scope, equiv_set, loc, args, kws):
    ...
    fsjos__osui = args[0]
    if equiv_set.has_shape(fsjos__osui):
        return ArrayAnalysis.AnalyzeResult(shape=fsjos__osui, pre=[])
    return None
...
def gen_dti_field_impl(field):
    xth__gka = 'def impl(dti):\n'
    xth__gka += '    numba.parfors.parfor.init_prange()\n'
    xth__gka += '    A = bodo.hiframes.pd_index_ext.get_index_data(dti)\n'
    xth__gka += '    name = bodo.hiframes.pd_index_ext.get_index_name(dti)\n'
    xth__gka += '    n = len(A)\n'
    xth__gka += '    S = np.empty(n, np.int64)\n'
    xth__gka += '    for i in numba.parfors.parfor.internal_prange(n):\n'
    xth__gka += '        val = A[i]\n'
    xth__gka += '        ts = bodo.utils.conversion.box_if_dt64(val)\n'
    if field in ['weekday']:
        xth__gka += '        S[i] = ts.' + field + '()\n'
    else:
        xth__gka += '        S[i] = ts.' + field + '\n'
    xth__gka += (
        '    return bodo.hiframes.pd_index_ext.init_numeric_index(S, name)\n')
    rov__fhqn = {}
    exec(xth__gka, {'numba': numba, 'np': np, 'bodo': bodo}, rov__fhqn)
    impl = rov__fhqn['impl']
    return impl
...
def overload_datetime_index_is_leap_year(dti):
    ...
    def impl(dti):
        ...
        bsz__jtfd = len(A)
        S = np.empty(bsz__jtfd, np.bool_)
        for i in numba.parfors.parfor.internal_prange(bsz__jtfd):
            val = A[i]
            ucikc__vbw = bodo.utils.conversion.box_if_dt64(val)
            S[i] = bodo.hiframes.pd_timestamp_ext.is_leap_year(ucikc__vbw.year)
        return S
    return impl
...
def overload_datetime_index_date(dti):
    ...
    def impl(dti):
        ...
        A = bodo.hiframes.pd_index_ext.get_index_data(dti)
        bsz__jtfd = len(A)
        S = bodo.hiframes.datetime_date_ext.alloc_datetime_date_array(bsz__jtfd
            )
        for i in numba.parfors.parfor.internal_prange(bsz__jtfd):
            val = A[i]
            ucikc__vbw = bodo.utils.conversion.box_if_dt64(val)
            S[i] = datetime.date(ucikc__vbw.year, ucikc__vbw.month,
                ucikc__vbw.day)
        return S
    return impl
...
def overload_datetime_index_min(dti, axis=None, skipna=True):
    qkzc__ksaig = dict(axis=axis, skipna=skipna)
    udqq__wuzu = dict(axis=None, skipna=True)
    check_unsupported_args('DatetimeIndex.min', qkzc__ksaig, udqq__wuzu,
    ...
    def impl(dti, axis=None, skipna=True):
        numba.parfors.parfor.init_prange()
        oaeq__tnrjp = bodo.hiframes.pd_index_ext.get_index_data(dti)
        s = numba.cpython.builtins.get_type_max_value(numba.core.types.int64)
        count = 0
        for i in numba.parfors.parfor.internal_prange(len(oaeq__tnrjp)):
            if not bodo.libs.array_kernels.isna(oaeq__tnrjp, i):
...
def overload_datetime_index_max(dti, axis=None, skipna=True):
    qkzc__ksaig = dict(axis=axis, skipna=skipna)
    udqq__wuzu = dict(axis=None, skipna=True)
    check_unsupported_args('DatetimeIndex.max', qkzc__ksaig, udqq__wuzu,
        package_name='pandas', module_name='Index')
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(dti,
        'Index.max()')
...
def pd_datetimeindex_overload(data=None, freq=None, tz=None, normalize=
    ...
    qkzc__ksaig = dict(freq=freq, tz=tz, normalize=normalize, closed=closed,
        ambiguous=ambiguous, dayfirst=dayfirst, yearfirst=yearfirst, dtype=
        dtype, copy=copy)
    udqq__wuzu = dict(freq=None, tz=None, normalize=False, closed=None,
        ambiguous='raise', dayfirst=False, yearfirst=False, dtype=None,
        copy=False)
    check_unsupported_args('pandas.DatetimeIndex', qkzc__ksaig, udqq__wuzu,
        package_name='pandas', module_name='Index')

    def f(data=None, freq=None, tz=None, normalize=False, closed=None,
        ambiguous='raise', dayfirst=False, yearfirst=False, dtype=None,
        copy=False, name=None):
        typk__abbvr = bodo.utils.conversion.coerce_to_array(data)
        S = bodo.utils.conversion.convert_to_dt64ns(typk__abbvr)
...
def overload_sub_operator_datetime_index(lhs, rhs):
    if isinstance(lhs, DatetimeIndexType
        ...
        kcwj__lkcvy = np.dtype('timedelta64[ns]')

        def impl(lhs, rhs):
            numba.parfors.parfor.init_prange()
            oaeq__tnrjp = bodo.hiframes.pd_index_ext.get_index_data(lhs)
            name = bodo.hiframes.pd_index_ext.get_index_name(lhs)
            bsz__jtfd = len(oaeq__tnrjp)
            S = np.empty(bsz__jtfd, kcwj__lkcvy)
            wqaq__fgykm = rhs.value
            for i in numba.parfors.parfor.internal_prange(bsz__jtfd):
                S[i] = bodo.hiframes.pd_timestamp_ext.integer_to_timedelta64(
                    bodo.hiframes.pd_timestamp_ext.dt64_to_integer(
                    oaeq__tnrjp[i]) - wqaq__fgykm)
    ...
    if isinstance(rhs, DatetimeIndexType
        ...
        def impl(lhs, rhs):
            numba.parfors.parfor.init_prange()
            oaeq__tnrjp = bodo.hiframes.pd_index_ext.get_index_data(rhs)
            name = bodo.hiframes.pd_index_ext.get_index_name(rhs)
            bsz__jtfd = len(oaeq__tnrjp)
            S = np.empty(bsz__jtfd, kcwj__lkcvy)
            wqaq__fgykm = lhs.value
            for i in numba.parfors.parfor.internal_prange(bsz__jtfd):
                S[i] = bodo.hiframes.pd_timestamp_ext.integer_to_timedelta64(
                    wqaq__fgykm - bodo.hiframes.pd_timestamp_ext.
                    dt64_to_integer(oaeq__tnrjp[i]))
            return bodo.hiframes.pd_index_ext.init_timedelta_index(S, name)
        return impl


def gen_dti_str_binop_impl(op, is_lhs_dti):
    zgwii__pmptj = numba.core.utils.OPERATORS_TO_BUILTINS[op]
    xth__gka = 'def impl(lhs, rhs):\n'
    if is_lhs_dti:
        xth__gka += '  dt_index, _str = lhs, rhs\n'
        jfsd__ohgld = 'arr[i] {} other'.format(zgwii__pmptj)
    else:
        xth__gka += '  dt_index, _str = rhs, lhs\n'
        jfsd__ohgld = 'other {} arr[i]'.format(zgwii__pmptj)
    xth__gka += '  arr = bodo.hiframes.pd_index_ext.get_index_data(dt_index)\n'
    xth__gka += '  l = len(arr)\n'
    xth__gka += (
        '  other = bodo.hiframes.pd_timestamp_ext.parse_datetime_str(_str)\n')
    xth__gka += '  S = bodo.libs.bool_arr_ext.alloc_bool_array(l)\n'
    xth__gka += '  for i in numba.parfors.parfor.internal_prange(l):\n'
    xth__gka += '    S[i] = {}\n'.format(jfsd__ohgld)
    xth__gka += '  return S\n'
    rov__fhqn = {}
    exec(xth__gka, {'bodo': bodo, 'numba': numba, 'np': np}, rov__fhqn)
    impl = rov__fhqn['impl']
    return impl
...
def pd_index_overload(data=None, dtype=None, copy=False, name=None,
    ...
    if not is_overload_none(dtype):
        nuwgx__afaqp = parse_dtype(dtype, 'pandas.Index')
        btt__zvk = False
    else:
        nuwgx__afaqp = getattr(data, 'dtype', None)
        btt__zvk = True
    if isinstance(nuwgx__afaqp, types.misc.PyObject):
        raise BodoError(
    ...
    if isinstance(data, RangeIndexType):
        ...
        if isinstance(nuwgx__afaqp, (types.Integer, types.Float, types.Boolean)
            ):
            if btt__zvk:
                ...
                def impl(data=None, dtype=None, copy=False, name=None,
                    tupleize_cols=True):
                    typk__abbvr = bodo.utils.conversion.coerce_to_array(data)
                    ssxe__rzsw = bodo.utils.conversion.fix_arr_dtype(
                        typk__abbvr, nuwgx__afaqp)
                    return bodo.hiframes.pd_index_ext.init_numeric_index(
                        ssxe__rzsw, name)
        elif nuwgx__afaqp in [types.string, bytes_type]:
...
def overload_datetime_index_getitem(dti, ind):
    if isinstance(dti, DatetimeIndexType):
        if isinstance(ind, types.Integer):
            ...
            def impl(dti, ind):
                ktjr__rgojs = bodo.hiframes.pd_index_ext.get_index_data(dti)
                val = ktjr__rgojs[ind]
...
def overload_timedelta_index_getitem(I, ind):
    ...
    if isinstance(ind, types.Integer):
        ...
        def impl(I, ind):
            rpp__pcz = bodo.hiframes.pd_index_ext.get_index_data(I)
            return pd.Timedelta(rpp__pcz[ind])
        return impl

    def impl(I, ind):
        rpp__pcz = bodo.hiframes.pd_index_ext.get_index_data(I)
        name = bodo.hiframes.pd_index_ext.get_index_name(I)
        lzxwo__hiyv = rpp__pcz[ind]
        return bodo.hiframes.pd_index_ext.init_timedelta_index(lzxwo__hiyv,
            name)
    return impl
...
def overload_categorical_index_getitem(I, ind):
    ...
    if isinstance(ind, types.Integer):
        ...
        def impl(I, ind):
            hik__cutq = bodo.hiframes.pd_index_ext.get_index_data(I)
            val = hik__cutq[ind]
            return val
        return impl
...
def pd_date_range_overload(start=None, end=None, periods=None, freq=None,
    ...
    qkzc__ksaig = dict(tz=tz, normalize=normalize, closed=closed)
    udqq__wuzu = dict(tz=None, normalize=False, closed=None)
    check_unsupported_args('pandas.date_range', qkzc__ksaig, udqq__wuzu,
    ...
    xth__gka = """def f(start=None, end=None, periods=None, freq=None, tz=None, normalize=False, name=None, closed=None):
"""
    xth__gka += wulf__xcy
    if is_overload_none(start):
        xth__gka += "  start_t = pd.Timestamp('1800-01-03')\n"
    else:
        xth__gka += '  start_t = pd.Timestamp(start)\n'
    if is_overload_none(end):
        xth__gka += "  end_t = pd.Timestamp('1800-01-03')\n"
    else:
        xth__gka += '  end_t = pd.Timestamp(end)\n'
    if not is_overload_none(freq):
        xth__gka += (
            '  stride = bodo.hiframes.pd_index_ext.to_offset_value(freq)\n')
        if is_overload_none(periods):
            xth__gka += '  b = start_t.value\n'
            xth__gka += (
                '  e = b + (end_t.value - b) // stride * stride + stride // 2 + 1\n'
                )
        elif not is_overload_none(start):
            xth__gka += '  b = start_t.value\n'
            xth__gka += '  addend = np.int64(periods) * np.int64(stride)\n'
            xth__gka += '  e = np.int64(b) + addend\n'
        elif not is_overload_none(end):
            xth__gka += '  e = end_t.value + stride\n'
            xth__gka += '  addend = np.int64(periods) * np.int64(-stride)\n'
            xth__gka += '  b = np.int64(e) + addend\n'
        else:
            raise_bodo_error(
                "at least 'start' or 'end' should be specified if a 'period' is given."
                )
        xth__gka += '  arr = np.arange(b, e, stride, np.int64)\n'
    else:
        xth__gka += '  delta = end_t.value - start_t.value\n'
        xth__gka += '  step = delta / (periods - 1)\n'
        xth__gka += '  arr1 = np.arange(0, periods, 1, np.float64)\n'
        xth__gka += '  arr1 *= step\n'
        xth__gka += '  arr1 += start_t.value\n'
        xth__gka += '  arr = arr1.astype(np.int64)\n'
        xth__gka += '  arr[-1] = end_t.value\n'
    xth__gka += '  A = bodo.utils.conversion.convert_to_dt64ns(arr)\n'
    xth__gka += (
        '  return bodo.hiframes.pd_index_ext.init_datetime_index(A, name)\n')
    rov__fhqn = {}
    exec(xth__gka, {'bodo': bodo, 'np': np, 'pd': pd}, rov__fhqn)
    f = rov__fhqn['f']
    return f
...
def pd_timedelta_range_overload(start=None, end=None, periods=None, freq=
    ...
    def f(start=None, end=None, periods=None, freq=None, name=None, closed=None
        ...
        hif__bgwa = pd.Timedelta('1 day')
        if start is not None:
            hif__bgwa = pd.Timedelta(start)
        iucw__gbe = pd.Timedelta('1 day')
        if end is not None:
            iucw__gbe = pd.Timedelta(end)
        if start is None and end is None and closed is not None:
            raise ValueError(
                'Closed has to be None if not both of start and end are defined'
                )
        tju__yewu, svyut__jtiww = (bodo.hiframes.pd_index_ext.
            validate_endpoints(closed))
        if freq is not None:
            nzv__bnqbf = _dummy_convert_none_to_int(freq)
            if periods is None:
                b = hif__bgwa.value
                umn__tex = b + (iucw__gbe.value - b
                    ) // nzv__bnqbf * nzv__bnqbf + nzv__bnqbf // 2 + 1
            elif start is not None:
                periods = _dummy_convert_none_to_int(periods)
                b = hif__bgwa.value
                mmsu__qiros = np.int64(periods) * np.int64(nzv__bnqbf)
                umn__tex = np.int64(b) + mmsu__qiros
            elif end is not None:
                periods = _dummy_convert_none_to_int(periods)
                umn__tex = iucw__gbe.value + nzv__bnqbf
                mmsu__qiros = np.int64(periods) * np.int64(-nzv__bnqbf)
                b = np.int64(umn__tex) + mmsu__qiros
            else:
                raise ValueError(
                    "at least 'start' or 'end' should be specified if a 'period' is given."
                    )
            zod__zxgv = np.arange(b, umn__tex, nzv__bnqbf, np.int64)
        else:
            periods = _dummy_convert_none_to_int(periods)
            pli__zprpi = iucw__gbe.value - hif__bgwa.value
            step = pli__zprpi / (periods - 1)
            cgdau__ajbbg = np.arange(0, periods, 1, np.float64)
            cgdau__ajbbg *= step
            cgdau__ajbbg += hif__bgwa.value
            zod__zxgv = cgdau__ajbbg.astype(np.int64)
            zod__zxgv[-1] = iucw__gbe.value
        if not tju__yewu and len(zod__zxgv) and zod__zxgv[0
            ] == hif__bgwa.value:
            zod__zxgv = zod__zxgv[1:]
        if not svyut__jtiww and len(zod__zxgv) and zod__zxgv[-1
            ] == iucw__gbe.value:
            zod__zxgv = zod__zxgv[:-1]
        S = bodo.utils.conversion.convert_to_dt64ns(zod__zxgv)
        return bodo.hiframes.pd_index_ext.init_timedelta_index(S, name)
    return f
...
def overload_pd_timestamp_isocalendar(idx):
    gmg__tdy = ColNamesMetaType(('year', 'week', 'day'))

    def impl(idx):
        A = bodo.hiframes.pd_index_ext.get_index_data(idx)
        numba.parfors.parfor.init_prange()
        bsz__jtfd = len(A)
        wad__rnmen = bodo.libs.int_arr_ext.alloc_int_array(bsz__jtfd, np.uint32
            )
        spa__runw = bodo.libs.int_arr_ext.alloc_int_array(bsz__jtfd, np.uint32)
        euyjz__kgf = bodo.libs.int_arr_ext.alloc_int_array(bsz__jtfd, np.uint32
            )
        for i in numba.parfors.parfor.internal_prange(bsz__jtfd):
            if bodo.libs.array_kernels.isna(A, i):
                bodo.libs.array_kernels.setna(wad__rnmen, i)
                bodo.libs.array_kernels.setna(spa__runw, i)
                bodo.libs.array_kernels.setna(euyjz__kgf, i)
                continue
            wad__rnmen[i], spa__runw[i], euyjz__kgf[i
                ] = bodo.utils.conversion.box_if_dt64(A[i]).isocalendar()
        return bodo.hiframes.pd_dataframe_ext.init_dataframe((wad__rnmen,
            spa__runw, euyjz__kgf), idx, gmg__tdy)
    return impl
...
class TimedeltaIndexTypeModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        vnl__uskw = [('data', _timedelta_index_data_typ), ('name', fe_type.
            name_typ), ('dict', types.DictType(_timedelta_index_data_typ.
            dtype, types.int64))]
        super(TimedeltaIndexTypeModel, self).__init__(dmm, fe_type, vnl__uskw)
...
def box_timedelta_index(typ, val, c):
    ...
    qeyo__yqwej = c.pyapi.from_native_value(_timedelta_index_data_typ,
        timedelta_index.data, c.env_manager)
    c.context.nrt.incref(c.builder, typ.name_typ, timedelta_index.name)
    ouvx__vtpal = c.pyapi.from_native_value(typ.name_typ, timedelta_index.
        name, c.env_manager)
    args = c.pyapi.tuple_pack([qeyo__yqwej])
    kws = c.pyapi.dict_pack([('name', ouvx__vtpal)])
    albf__ccnvf = c.pyapi.object_getattr_string(ggz__ipsm, 'TimedeltaIndex')
    pcp__nhwu = c.pyapi.call(albf__ccnvf, args, kws)
    c.pyapi.decref(qeyo__yqwej)
    c.pyapi.decref(ouvx__vtpal)
    c.pyapi.decref(ggz__ipsm)
...
def unbox_timedelta_index(typ, val, c):
    gyp__qgm = c.pyapi.object_getattr_string(val, 'values')
    data = c.pyapi.to_native_value(_timedelta_index_data_typ, gyp__qgm).value
    ouvx__vtpal = c.pyapi.object_getattr_string(val, 'name')
    name = c.pyapi.to_native_value(typ.name_typ, ouvx__vtpal).value
    c.pyapi.decref(gyp__qgm)
    c.pyapi.decref(ouvx__vtpal)
    neyhm__gghfz = cgutils.create_struct_proxy(typ)(c.context, c.builder)
    neyhm__gghfz.data = data
    neyhm__gghfz.name = name
...
def init_timedelta_index(typingctx, data, name=None):
    ...
    def codegen(context, builder, signature, args):
        ...
        timedelta_index = cgutils.create_struct_proxy(signature.return_type)(
            context, builder)
        timedelta_index.data = pkwyx__ytkyt
        timedelta_index.name = lve__cll
        context.nrt.incref(builder, signature.args[0], pkwyx__ytkyt)
        context.nrt.incref(builder, signature.args[1], lve__cll)
        dtype = _timedelta_index_data_typ.dtype
    ...
    fzpiw__nmp = TimedeltaIndexType(name)
    sig = signature(fzpiw__nmp, data, name)
    return sig, codegen
...
def overload_timedelta_index_copy(A, name=None, deep=False, dtype=None,
    names=None):
    qtpp__rpeso = dict(deep=deep, dtype=dtype, names=names)
    dnn__ljxr = idx_typ_to_format_str_map[TimedeltaIndexType].format('copy()')
    check_unsupported_args('TimedeltaIndex.copy', qtpp__rpeso,
        idx_cpy_arg_defaults, fn_str=dnn__ljxr, package_name='pandas',
        module_name='Index')
    if not is_overload_none(name):
...
def overload_timedelta_index_min(tdi, axis=None, skipna=True):
    qkzc__ksaig = dict(axis=axis, skipna=skipna)
    udqq__wuzu = dict(axis=None, skipna=True)
    check_unsupported_args('TimedeltaIndex.min', qkzc__ksaig, udqq__wuzu,
        package_name='pandas', module_name='Index')
    ...
    def impl(tdi, axis=None, skipna=True):
        ...
        bsz__jtfd = len(data)
        itwz__zjuyz = numba.cpython.builtins.get_type_max_value(numba.core.
            types.int64)
        count = 0
        for i in numba.parfors.parfor.internal_prange(bsz__jtfd):
            ...
            count += 1
            itwz__zjuyz = min(itwz__zjuyz, val)
        cpuil__iqqp = bodo.hiframes.pd_timestamp_ext.integer_to_timedelta64(
            itwz__zjuyz)
        return bodo.hiframes.pd_index_ext._tdi_val_finalize(cpuil__iqqp, count)
    return impl
...
def overload_timedelta_index_max(tdi, axis=None, skipna=True):
    qkzc__ksaig = dict(axis=axis, skipna=skipna)
    udqq__wuzu = dict(axis=None, skipna=True)
    check_unsupported_args('TimedeltaIndex.max', qkzc__ksaig, udqq__wuzu,
        package_name='pandas', module_name='Index')
    ...
    def impl(tdi, axis=None, skipna=True):
        ...
        data = bodo.hiframes.pd_index_ext.get_index_data(tdi)
        bsz__jtfd = len(data)
        mklx__mtsr = numba.cpython.builtins.get_type_min_value(numba.core.
            types.int64)
        count = 0
        for i in numba.parfors.parfor.internal_prange(bsz__jtfd):
            if bodo.libs.array_kernels.isna(data, i):
...
def gen_tdi_field_impl(field):
    xth__gka = 'def impl(tdi):\n'
    xth__gka += '    numba.parfors.parfor.init_prange()\n'
    xth__gka += '    A = bodo.hiframes.pd_index_ext.get_index_data(tdi)\n'
    xth__gka += '    name = bodo.hiframes.pd_index_ext.get_index_name(tdi)\n'
    xth__gka += '    n = len(A)\n'
    xth__gka += '    S = np.empty(n, np.int64)\n'
    xth__gka += '    for i in numba.parfors.parfor.internal_prange(n):\n'
    xth__gka += (
    ...
    if field == 'nanoseconds':
        xth__gka += '        S[i] = td64 % 1000\n'
    elif field == 'microseconds':
        xth__gka += '        S[i] = td64 // 1000 % 100000\n'
    elif field == 'seconds':
        xth__gka += (
            '        S[i] = td64 // (1000 * 1000000) % (60 * 60 * 24)\n')
    elif field == 'days':
        xth__gka += '        S[i] = td64 // (1000 * 1000000 * 60 * 60 * 24)\n'
    else:
...
def pd_timedelta_index_overload(data=None, unit=None, freq=None, dtype=None,
    ...
    qkzc__ksaig = dict(unit=unit, freq=freq, dtype=dtype, copy=copy)
    udqq__wuzu = dict(unit=None, freq=None, dtype=None, copy=False)
    check_unsupported_args('pandas.TimedeltaIndex', qkzc__ksaig, udqq__wuzu,
        package_name='pandas', module_name='Index')

    def impl(data=None, unit=None, freq=None, dtype=None, copy=False, name=None
        ):
        typk__abbvr = bodo.utils.conversion.coerce_to_array(data)
        S = bodo.utils.conversion.convert_to_td64ns(typk__abbvr)
        return bodo.hiframes.pd_index_ext.init_timedelta_index(S, name)
    return impl
...
class RangeIndexModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        vnl__uskw = [('start', types.int64), ('stop', types.int64), ('step',
            types.int64), ('name', fe_type.name_typ)]
        super(RangeIndexModel, self).__init__(dmm, fe_type, vnl__uskw)
...
def overload_range_index_copy(A, name=None, deep=False, dtype=None, names=None
    ...
    qtpp__rpeso = dict(deep=deep, dtype=dtype, names=names)
    dnn__ljxr = idx_typ_to_format_str_map[RangeIndexType].format('copy()')
    check_unsupported_args('RangeIndex.copy', qtpp__rpeso,
        idx_cpy_arg_defaults, fn_str=dnn__ljxr, package_name='pandas',
        module_name='Index')
    if not is_overload_none(name):
...
def box_range_index(typ, val, c):
    memu__vbvzt = c.context.insert_const_string(c.builder.module, 'pandas')
    tbw__penkr = c.pyapi.import_module_noblock(memu__vbvzt)
    fuu__dytv = cgutils.create_struct_proxy(typ)(c.context, c.builder, val)
    okcoz__nza = c.pyapi.from_native_value(types.int64, fuu__dytv.start, c.
        env_manager)
    moz__yca = c.pyapi.from_native_value(types.int64, fuu__dytv.stop, c.
        env_manager)
    satmd__hcs = c.pyapi.from_native_value(types.int64, fuu__dytv.step, c.
        env_manager)
    c.context.nrt.incref(c.builder, typ.name_typ, fuu__dytv.name)
    ouvx__vtpal = c.pyapi.from_native_value(typ.name_typ, fuu__dytv.name, c
        .env_manager)
    args = c.pyapi.tuple_pack([okcoz__nza, moz__yca, satmd__hcs])
    kws = c.pyapi.dict_pack([('name', ouvx__vtpal)])
    albf__ccnvf = c.pyapi.object_getattr_string(tbw__penkr, 'RangeIndex')
    ivgy__eis = c.pyapi.call(albf__ccnvf, args, kws)
    c.pyapi.decref(okcoz__nza)
    c.pyapi.decref(moz__yca)
    c.pyapi.decref(satmd__hcs)
    c.pyapi.decref(ouvx__vtpal)
    c.pyapi.decref(tbw__penkr)
    c.pyapi.decref(albf__ccnvf)
    c.pyapi.decref(args)
    c.pyapi.decref(kws)
    c.context.nrt.decref(c.builder, typ, val)
...
def init_range_index(typingctx, start, stop, step, name=None):
    name = types.none if name is None else name
    syyc__fsj = is_overload_constant_int(step) and get_overload_const_int(step
        ) == 0
    ...
    def codegen(context, builder, signature, args):
        ...
        syyyp__jys = cgutils.is_scalar_zero(builder, args[2])
        uedo__xmlc = context.get_python_api(builder)
        with builder.if_then(syyyp__jys):
            uedo__xmlc.err_format('PyExc_ValueError', 'Step must not be zero')
            val = context.get_constant(types.int32, -1)
            builder.ret(val)
        fuu__dytv = cgutils.create_struct_proxy(signature.return_type)(context,
            builder)
        fuu__dytv.start = args[0]
        fuu__dytv.stop = args[1]
        fuu__dytv.step = args[2]
        fuu__dytv.name = args[3]
        context.nrt.incref(builder, signature.return_type.name_typ, args[3])
        return fuu__dytv._getvalue()
    return RangeIndexType(name)(start, stop, step, name), codegen


def init_range_index_equiv(self, scope, equiv_set, loc, args, kws):
    assert len(args) == 4 and not kws
    start, stop, step, chsz__wuro = args
...
def unbox_range_index(typ, val, c):
    okcoz__nza = c.pyapi.object_getattr_string(val, 'start')
    start = c.pyapi.to_native_value(types.int64, okcoz__nza).value
    moz__yca = c.pyapi.object_getattr_string(val, 'stop')
    stop = c.pyapi.to_native_value(types.int64, moz__yca).value
    satmd__hcs = c.pyapi.object_getattr_string(val, 'step')
    step = c.pyapi.to_native_value(types.int64, satmd__hcs).value
    ouvx__vtpal = c.pyapi.object_getattr_string(val, 'name')
    name = c.pyapi.to_native_value(typ.name_typ, ouvx__vtpal).value
    c.pyapi.decref(okcoz__nza)
    c.pyapi.decref(moz__yca)
    c.pyapi.decref(satmd__hcs)
    c.pyapi.decref(ouvx__vtpal)
    fuu__dytv = cgutils.create_struct_proxy(typ)(c.context, c.builder)
    fuu__dytv.start = start
    fuu__dytv.stop = stop
    fuu__dytv.step = step
    fuu__dytv.name = name
    return NativeValue(fuu__dytv._getvalue())
...
def range_index_overload(start=None, stop=None, step=None, dtype=None, copy
    ...
    def _ensure_int_or_none(value, field):
        ...
        if not is_overload_none(value) and not isinstance(value, types.
            ...
            raise BodoError(znzf__ewmdq.format(value=value, field=field))
    ...
    if is_overload_none(start) and is_overload_none(stop) and is_overload_none(
        step):
        znzf__ewmdq = 'RangeIndex(...) must be called with integers'
        raise BodoError(znzf__ewmdq)
    qyexn__ziyfr = 'start'
    lrc__zvr = 'stop'
    mhva__xhw = 'step'
    if is_overload_none(start):
    ...
    if is_overload_none(step):
        mhva__xhw = '1'
    xth__gka = """def _pd_range_index_imp(start=None, stop=None, step=None, dtype=None, copy=False, name=None):
"""
    xth__gka += '  return init_range_index({}, {}, {}, name)\n'.format(
        qyexn__ziyfr, lrc__zvr, mhva__xhw)
    rov__fhqn = {}
    exec(xth__gka, {'init_range_index': init_range_index}, rov__fhqn)
    bqqfs__xrem = rov__fhqn['_pd_range_index_imp']
    return bqqfs__xrem
...
def overload_range_index_getitem(I, idx):
    if isinstance(I, RangeIndexType):
        ...
        if isinstance(idx, types.SliceType):
            ...
            def impl(I, idx):
                jbmr__ple = numba.cpython.unicode._normalize_slice(idx, len(I))
                name = bodo.hiframes.pd_index_ext.get_index_name(I)
                start = I._start + I._step * jbmr__ple.start
                stop = I._start + I._step * jbmr__ple.stop
                step = I._step * jbmr__ple.step
...
class PeriodIndexModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        vnl__uskw = [('data', bodo.IntegerArrayType(types.int64)), ('name',
            fe_type.name_typ), ('dict', types.DictType(types.int64, types.
            int64))]
        super(PeriodIndexModel, self).__init__(dmm, fe_type, vnl__uskw)
...
def overload_period_index_copy(A, name=None, deep=False, dtype=None, names=None
    ...
    freq = A.freq
    qtpp__rpeso = dict(deep=deep, dtype=dtype, names=names)
    dnn__ljxr = idx_typ_to_format_str_map[PeriodIndexType].format('copy()')
    check_unsupported_args('PeriodIndex.copy', qtpp__rpeso,
        idx_cpy_arg_defaults, fn_str=dnn__ljxr, package_name='pandas',
        module_name='Index')
    if not is_overload_none(name):
...
def init_period_index(typingctx, data, name, freq):
    ...
    def codegen(context, builder, signature, args):
        pkwyx__ytkyt, lve__cll, chsz__wuro = args
        kcs__wwnp = signature.return_type
        bwhkh__tauku = cgutils.create_struct_proxy(kcs__wwnp)(context, builder)
        bwhkh__tauku.data = pkwyx__ytkyt
        bwhkh__tauku.name = lve__cll
        context.nrt.incref(builder, signature.args[0], args[0])
        context.nrt.incref(builder, signature.args[1], args[1])
        bwhkh__tauku.dict = context.compile_internal(builder, lambda :
            numba.typed.Dict.empty(types.int64, types.int64), types.
            DictType(types.int64, types.int64)(), [])
        return bwhkh__tauku._getvalue()
    phk__tzx = get_overload_const_str(freq)
    fzpiw__nmp = PeriodIndexType(phk__tzx, name)
    sig = signature(fzpiw__nmp, data, name, freq)
    return sig, codegen
...
def box_period_index(typ, val, c):
    memu__vbvzt = c.context.insert_const_string(c.builder.module, 'pandas')
    tbw__penkr = c.pyapi.import_module_noblock(memu__vbvzt)
    neyhm__gghfz = cgutils.create_struct_proxy(typ)(c.context, c.builder, val)
    c.context.nrt.incref(c.builder, bodo.IntegerArrayType(types.int64),
        neyhm__gghfz.data)
    ewfol__ctuew = c.pyapi.from_native_value(bodo.IntegerArrayType(types.
        int64), neyhm__gghfz.data, c.env_manager)
    c.context.nrt.incref(c.builder, typ.name_typ, neyhm__gghfz.name)
    ouvx__vtpal = c.pyapi.from_native_value(typ.name_typ, neyhm__gghfz.name,
        c.env_manager)
    xnrs__bksp = c.pyapi.string_from_constant_string(typ.freq)
    args = c.pyapi.tuple_pack([])
    kws = c.pyapi.dict_pack([('ordinal', ewfol__ctuew), ('name',
        ouvx__vtpal), ('freq', xnrs__bksp)])
    albf__ccnvf = c.pyapi.object_getattr_string(tbw__penkr, 'PeriodIndex')
    ivgy__eis = c.pyapi.call(albf__ccnvf, args, kws)
    c.pyapi.decref(ewfol__ctuew)
    c.pyapi.decref(ouvx__vtpal)
    c.pyapi.decref(xnrs__bksp)
    c.pyapi.decref(tbw__penkr)
    c.pyapi.decref(albf__ccnvf)
    c.pyapi.decref(args)
    c.pyapi.decref(kws)
...
def unbox_period_index(typ, val, c):
    ...
    qsxmi__yweo = c.pyapi.object_getattr_string(val, 'asi8')
    otv__lpl = c.pyapi.call_method(val, 'isna', ())
    ouvx__vtpal = c.pyapi.object_getattr_string(val, 'name')
    name = c.pyapi.to_native_value(typ.name_typ, ouvx__vtpal).value
    memu__vbvzt = c.context.insert_const_string(c.builder.module, 'pandas')
    ggz__ipsm = c.pyapi.import_module_noblock(memu__vbvzt)
    hnp__jhlac = c.pyapi.object_getattr_string(ggz__ipsm, 'arrays')
    ewfol__ctuew = c.pyapi.call_method(hnp__jhlac, 'IntegerArray', (
        qsxmi__yweo, otv__lpl))
    data = c.pyapi.to_native_value(arr_typ, ewfol__ctuew).value
    c.pyapi.decref(qsxmi__yweo)
    c.pyapi.decref(otv__lpl)
    c.pyapi.decref(ouvx__vtpal)
    c.pyapi.decref(ggz__ipsm)
    c.pyapi.decref(hnp__jhlac)
    c.pyapi.decref(ewfol__ctuew)
    neyhm__gghfz = cgutils.create_struct_proxy(typ)(c.context, c.builder)
    neyhm__gghfz.data = data
    neyhm__gghfz.name = name
...
class CategoricalIndexTypeModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        from bodo.hiframes.pd_categorical_ext import get_categories_int_type
        ukcbn__iincw = get_categories_int_type(fe_type.data.dtype)
        vnl__uskw = [('data', fe_type.data), ('name', fe_type.name_typ), (
            'dict', types.DictType(ukcbn__iincw, types.int64))]
        super(CategoricalIndexTypeModel, self).__init__(dmm, fe_type, vnl__uskw
            )
...
def box_categorical_index(typ, val, c):
    memu__vbvzt = c.context.insert_const_string(c.builder.module, 'pandas')
    ggz__ipsm = c.pyapi.import_module_noblock(memu__vbvzt)
    ryy__jqn = numba.core.cgutils.create_struct_proxy(typ)(c.context, c.
        builder, val)
    c.context.nrt.incref(c.builder, typ.data, ryy__jqn.data)
    qeyo__yqwej = c.pyapi.from_native_value(typ.data, ryy__jqn.data, c.
        env_manager)
    c.context.nrt.incref(c.builder, typ.name_typ, ryy__jqn.name)
    ouvx__vtpal = c.pyapi.from_native_value(typ.name_typ, ryy__jqn.name, c.
        env_manager)
    args = c.pyapi.tuple_pack([qeyo__yqwej])
    kws = c.pyapi.dict_pack([('name', ouvx__vtpal)])
    albf__ccnvf = c.pyapi.object_getattr_string(ggz__ipsm, 'CategoricalIndex')
    pcp__nhwu = c.pyapi.call(albf__ccnvf, args, kws)
    c.pyapi.decref(qeyo__yqwej)
    c.pyapi.decref(ouvx__vtpal)
    c.pyapi.decref(ggz__ipsm)
...
def unbox_categorical_index(typ, val, c):
    from bodo.hiframes.pd_categorical_ext import get_categories_int_type
    gyp__qgm = c.pyapi.object_getattr_string(val, 'values')
    data = c.pyapi.to_native_value(typ.data, gyp__qgm).value
    ouvx__vtpal = c.pyapi.object_getattr_string(val, 'name')
    name = c.pyapi.to_native_value(typ.name_typ, ouvx__vtpal).value
    c.pyapi.decref(gyp__qgm)
    c.pyapi.decref(ouvx__vtpal)
...
def init_categorical_index(typingctx, data, name=None):
    ...
    def codegen(context, builder, signature, args):
        from bodo.hiframes.pd_categorical_ext import get_categories_int_type
        pkwyx__ytkyt, lve__cll = args
        ryy__jqn = cgutils.create_struct_proxy(signature.return_type)(context,
            builder)
        ryy__jqn.data = pkwyx__ytkyt
        ryy__jqn.name = lve__cll
        context.nrt.incref(builder, signature.args[0], pkwyx__ytkyt)
        context.nrt.incref(builder, signature.args[1], lve__cll)
        dtype = get_categories_int_type(signature.return_type.data.dtype)
        ryy__jqn.dict = context.compile_internal(builder, lambda : numba.
            typed.Dict.empty(dtype, types.int64), types.DictType(dtype,
            types.int64)(), [])
        return ryy__jqn._getvalue()
    fzpiw__nmp = CategoricalIndexType(data, name)
    sig = signature(fzpiw__nmp, data, name)
    return sig, codegen
...
def overload_categorical_index_copy(A, name=None, deep=False, dtype=None,
    names=None):
    dnn__ljxr = idx_typ_to_format_str_map[CategoricalIndexType].format('copy()'
        )
    qtpp__rpeso = dict(deep=deep, dtype=dtype, names=names)
    check_unsupported_args('CategoricalIndex.copy', qtpp__rpeso,
        idx_cpy_arg_defaults, fn_str=dnn__ljxr, package_name='pandas',
        module_name='Index')
    if not is_overload_none(name):
...
class IntervalIndexTypeModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        vnl__uskw = [('data', fe_type.data), ('name', fe_type.name_typ), (
            ...
            dtype, 2), types.int64))]
        super(IntervalIndexTypeModel, self).__init__(dmm, fe_type, vnl__uskw)
...
def box_interval_index(typ, val, c):
    memu__vbvzt = c.context.insert_const_string(c.builder.module, 'pandas')
    ggz__ipsm = c.pyapi.import_module_noblock(memu__vbvzt)
    xmf__ppto = numba.core.cgutils.create_struct_proxy(typ)(c.context, c.
        builder, val)
    c.context.nrt.incref(c.builder, typ.data, xmf__ppto.data)
    qeyo__yqwej = c.pyapi.from_native_value(typ.data, xmf__ppto.data, c.
        env_manager)
    c.context.nrt.incref(c.builder, typ.name_typ, xmf__ppto.name)
    ouvx__vtpal = c.pyapi.from_native_value(typ.name_typ, xmf__ppto.name, c
        .env_manager)
    args = c.pyapi.tuple_pack([qeyo__yqwej])
    kws = c.pyapi.dict_pack([('name', ouvx__vtpal)])
    albf__ccnvf = c.pyapi.object_getattr_string(ggz__ipsm, 'IntervalIndex')
    pcp__nhwu = c.pyapi.call(albf__ccnvf, args, kws)
    c.pyapi.decref(qeyo__yqwej)
    c.pyapi.decref(ouvx__vtpal)
    c.pyapi.decref(ggz__ipsm)
...
def init_interval_index(typingctx, data, name=None):
    ...
    def codegen(context, builder, signature, args):
        pkwyx__ytkyt, lve__cll = args
        xmf__ppto = cgutils.create_struct_proxy(signature.return_type)(context,
            builder)
        xmf__ppto.data = pkwyx__ytkyt
        xmf__ppto.name = lve__cll
        context.nrt.incref(builder, signature.args[0], pkwyx__ytkyt)
        context.nrt.incref(builder, signature.args[1], lve__cll)
        dtype = types.UniTuple(data.arr_type.dtype, 2)
        xmf__ppto.dict = context.compile_internal(builder, lambda : numba.
            typed.Dict.empty(dtype, types.int64), types.DictType(dtype,
            types.int64)(), [])
        return xmf__ppto._getvalue()
    fzpiw__nmp = IntervalIndexType(data, name)
    sig = signature(fzpiw__nmp, data, name)
    return sig, codegen
...
class NumericIndexModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        ...
        super(NumericIndexModel, self).__init__(dmm, fe_type, vnl__uskw)
...
def overload_numeric_index_copy(A, name=None, deep=False, dtype=None, names
    =None):
    dnn__ljxr = idx_typ_to_format_str_map[NumericIndexType].format('copy()')
    qtpp__rpeso = dict(deep=deep, dtype=dtype, names=names)
    check_unsupported_args('Index.copy', qtpp__rpeso, idx_cpy_arg_defaults,
        fn_str=dnn__ljxr, package_name='pandas', module_name='Index')
    if not is_overload_none(name):
...
def box_numeric_index(typ, val, c):
    ...
    tbw__penkr = c.pyapi.import_module_noblock(memu__vbvzt)
    neyhm__gghfz = cgutils.create_struct_proxy(typ)(c.context, c.builder, val)
    c.context.nrt.incref(c.builder, typ.data, neyhm__gghfz.data)
    ewfol__ctuew = c.pyapi.from_native_value(typ.data, neyhm__gghfz.data, c
        .env_manager)
    c.context.nrt.incref(c.builder, typ.name_typ, neyhm__gghfz.name)
    ouvx__vtpal = c.pyapi.from_native_value(typ.name_typ, neyhm__gghfz.name,
        c.env_manager)
    ppwr__enbg = c.pyapi.make_none()
    inx__btevw = c.pyapi.bool_from_bool(c.context.get_constant(types.bool_,
        False))
    ivgy__eis = c.pyapi.call_method(tbw__penkr, 'Index', (ewfol__ctuew,
        ppwr__enbg, inx__btevw, ouvx__vtpal))
    c.pyapi.decref(ewfol__ctuew)
    c.pyapi.decref(ppwr__enbg)
    c.pyapi.decref(inx__btevw)
    c.pyapi.decref(ouvx__vtpal)
    c.pyapi.decref(tbw__penkr)
    c.context.nrt.decref(c.builder, typ, val)
    return ivgy__eis
...
def init_numeric_index(typingctx, data, name=None):
    ...
    def codegen(context, builder, signature, args):
        assert len(args) == 2
        kcs__wwnp = signature.return_type
        neyhm__gghfz = cgutils.create_struct_proxy(kcs__wwnp)(context, builder)
        neyhm__gghfz.data = args[0]
        neyhm__gghfz.name = args[1]
        context.nrt.incref(builder, kcs__wwnp.data, args[0])
        context.nrt.incref(builder, kcs__wwnp.name_typ, args[1])
        dtype = kcs__wwnp.dtype
        neyhm__gghfz.dict = context.compile_internal(builder, lambda :
            numba.typed.Dict.empty(dtype, types.int64), types.DictType(
            dtype, types.int64)(), [])
        return neyhm__gghfz._getvalue()
...
def create_numeric_constructor(func, func_str, default_dtype):
    ...
    def overload_impl(data=None, dtype=None, copy=False, name=None):
        zble__afewk = dict(dtype=dtype)
        qxam__ubwcd = dict(dtype=None)
        check_unsupported_args(func_str, zble__afewk, qxam__ubwcd,
            package_name='pandas', module_name='Index')
        if is_overload_false(copy):

            def impl(data=None, dtype=None, copy=False, name=None):
                typk__abbvr = bodo.utils.conversion.coerce_to_ndarray(data)
                rsafp__qsnp = bodo.utils.conversion.fix_arr_dtype(typk__abbvr,
...
class StringIndexModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        ...
        super(StringIndexModel, self).__init__(dmm, fe_type, vnl__uskw)
...
class BinaryIndexModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        vnl__uskw = [('data', binary_array_type), ('name', fe_type.name_typ
            ), ('dict', types.DictType(bytes_type, types.int64))]
        super(BinaryIndexModel, self).__init__(dmm, fe_type, vnl__uskw)
...
def unbox_binary_str_index(typ, val, c):
    exth__xzcvm = typ.data
    scalar_type = typ.data.dtype
    gyp__qgm = c.pyapi.object_getattr_string(val, 'values')
    data = c.pyapi.to_native_value(exth__xzcvm, gyp__qgm).value
    ouvx__vtpal = c.pyapi.object_getattr_string(val, 'name')
    name = c.pyapi.to_native_value(typ.name_typ, ouvx__vtpal).value
    c.pyapi.decref(gyp__qgm)
    c.pyapi.decref(ouvx__vtpal)
...
def box_binary_str_index(typ, val, c):
    ...
    tbw__penkr = c.pyapi.import_module_noblock(memu__vbvzt)
    neyhm__gghfz = cgutils.create_struct_proxy(typ)(c.context, c.builder, val)
    c.context.nrt.incref(c.builder, exth__xzcvm, neyhm__gghfz.data)
    ewfol__ctuew = c.pyapi.from_native_value(exth__xzcvm, neyhm__gghfz.data,
        c.env_manager)
    c.context.nrt.incref(c.builder, typ.name_typ, neyhm__gghfz.name)
    ouvx__vtpal = c.pyapi.from_native_value(typ.name_typ, neyhm__gghfz.name,
        c.env_manager)
...
def init_binary_str_index(typingctx, data, name=None):
    ...
    sig = type(bodo.utils.typing.get_index_type_from_dtype(data.dtype))(name,
        data)(data, name)
    foum__mbn = get_binary_str_codegen(is_binary=data.dtype == bytes_type)
    return sig, foum__mbn
...
def get_binary_str_codegen(is_binary=False):
    if is_binary:
        nxh__owyeb = 'bytes_type'
    else:
        nxh__owyeb = 'string_type'
    xth__gka = 'def impl(context, builder, signature, args):\n'
    xth__gka += '    assert len(args) == 2\n'
    xth__gka += '    index_typ = signature.return_type\n'
    xth__gka += (
        '    index_val = cgutils.create_struct_proxy(index_typ)(context, builder)\n'
        )
    xth__gka += '    index_val.data = args[0]\n'
    xth__gka += '    index_val.name = args[1]\n'
    xth__gka += '    # increase refcount of stored values\n'
    xth__gka += '    context.nrt.incref(builder, signature.args[0], args[0])\n'
    xth__gka += (
        '    context.nrt.incref(builder, index_typ.name_typ, args[1])\n')
    xth__gka += '    # create empty dict for get_loc hashmap\n'
    xth__gka += '    index_val.dict = context.compile_internal(\n'
    xth__gka += '       builder,\n'
    xth__gka += (
        f'       lambda: numba.typed.Dict.empty({nxh__owyeb}, types.int64),\n')
    xth__gka += f'        types.DictType({nxh__owyeb}, types.int64)(), [],)\n'
    xth__gka += '    return index_val._getvalue()\n'
    rov__fhqn = {}
    exec(xth__gka, {'bodo': bodo, 'signature': signature, 'cgutils':
        cgutils, 'numba': numba, 'types': types, 'bytes_type': bytes_type,
        'string_type': string_type}, rov__fhqn)
    impl = rov__fhqn['impl']
    return impl
...
def overload_binary_string_index_copy(A, name=None, deep=False, dtype=None,
    names=None):
    typ = type(A)
    dnn__ljxr = idx_typ_to_format_str_map[typ].format('copy()')
    qtpp__rpeso = dict(deep=deep, dtype=dtype, names=names)
    check_unsupported_args('Index.copy', qtpp__rpeso, idx_cpy_arg_defaults,
        fn_str=dnn__ljxr, package_name='pandas', module_name='Index')
    if not is_overload_none(name):
...
def _verify_setop_compatible(func_name, I, other):
    ...
    djwqv__fzdwe = I.dtype if not isinstance(I, RangeIndexType
        ) else types.int64
    dzrcr__hhd = other.dtype if not isinstance(other, RangeIndexType
        ) else types.int64
    if djwqv__fzdwe != dzrcr__hhd:
        raise BodoError(
            f'Index.{func_name}(): incompatible types {djwqv__fzdwe} and {dzrcr__hhd}'
            )
...
def overload_index_union(I, other, sort=None):
    qkzc__ksaig = dict(sort=sort)
    alno__usrw = dict(sort=None)
    check_unsupported_args('Index.union', qkzc__ksaig, alno__usrw,
        package_name='pandas', module_name='Index')
    _verify_setop_compatible('union', I, other)
    lweh__ewqat = get_index_constructor(I) if not isinstance(I, RangeIndexType
        ) else init_numeric_index

    def impl(I, other, sort=None):
        ofmk__rnbyn = bodo.utils.conversion.coerce_to_array(I)
        liiip__ffr = bodo.utils.conversion.coerce_to_array(other)
        otp__uhtl = bodo.libs.array_kernels.concat([ofmk__rnbyn, liiip__ffr])
        raot__tlno = bodo.libs.array_kernels.unique(otp__uhtl)
        return lweh__ewqat(raot__tlno, None)
    return impl
...
def overload_index_intersection(I, other, sort=None):
    qkzc__ksaig = dict(sort=sort)
    alno__usrw = dict(sort=None)
    check_unsupported_args('Index.intersection', qkzc__ksaig, alno__usrw,
        package_name='pandas', module_name='Index')
    ...
    def impl(I, other, sort=None):
        ofmk__rnbyn = bodo.utils.conversion.coerce_to_array(I)
        liiip__ffr = bodo.utils.conversion.coerce_to_array(other)
        dsoo__sqj = bodo.libs.array_kernels.unique(ofmk__rnbyn)
        jfxaa__gvvzw = bodo.libs.array_kernels.unique(liiip__ffr)
        otp__uhtl = bodo.libs.array_kernels.concat([dsoo__sqj, jfxaa__gvvzw])
        nbz__jhv = pd.Series(otp__uhtl).sort_values().values
        qje__gfleg = bodo.libs.array_kernels.intersection_mask(nbz__jhv)
        return lweh__ewqat(nbz__jhv[qje__gfleg], None)
    return impl
...
def overload_index_difference(I, other, sort=None):
    qkzc__ksaig = dict(sort=sort)
    alno__usrw = dict(sort=None)
    check_unsupported_args('Index.difference', qkzc__ksaig, alno__usrw,
        package_name='pandas', module_name='Index')
    ...
    def impl(I, other, sort=None):
        ...
        dsoo__sqj = bodo.libs.array_kernels.unique(ofmk__rnbyn)
        jfxaa__gvvzw = bodo.libs.array_kernels.unique(liiip__ffr)
        qje__gfleg = np.empty(len(dsoo__sqj), np.bool_)
        bodo.libs.array.array_isin(qje__gfleg, dsoo__sqj, jfxaa__gvvzw, False)
        return lweh__ewqat(dsoo__sqj[~qje__gfleg], None)
    return impl
...
def overload_index_symmetric_difference(I, other, result_name=None, sort=None):
    qkzc__ksaig = dict(result_name=result_name, sort=sort)
    alno__usrw = dict(result_name=None, sort=None)
    check_unsupported_args('Index.symmetric_difference', qkzc__ksaig,
        alno__usrw, package_name='pandas', module_name='Index')
    ...
    def impl(I, other, result_name=None, sort=None):
        ...
        dsoo__sqj = bodo.libs.array_kernels.unique(ofmk__rnbyn)
        jfxaa__gvvzw = bodo.libs.array_kernels.unique(liiip__ffr)
        ycmzx__eraat = np.empty(len(dsoo__sqj), np.bool_)
        yip__kcev = np.empty(len(jfxaa__gvvzw), np.bool_)
        bodo.libs.array.array_isin(ycmzx__eraat, dsoo__sqj, jfxaa__gvvzw, False
            )
        bodo.libs.array.array_isin(yip__kcev, jfxaa__gvvzw, dsoo__sqj, False)
        vcpo__ehpd = bodo.libs.array_kernels.concat([dsoo__sqj[~
            ycmzx__eraat], jfxaa__gvvzw[~yip__kcev]])
        return lweh__ewqat(vcpo__ehpd, None)
    return impl
...
def overload_index_take(I, indices, axis=0, allow_fill=True, fill_value=None):
    qkzc__ksaig = dict(axis=axis, allow_fill=allow_fill, fill_value=fill_value)
    alno__usrw = dict(axis=0, allow_fill=True, fill_value=None)
    check_unsupported_args('Index.take', qkzc__ksaig, alno__usrw,
        package_name='pandas', module_name='Index')
    ...
    if isinstance(I, CategoricalIndexType):
        ...
        def impl(I, ban_unique=True):
            if len(I) > 0 and not I._dict:
                zod__zxgv = bodo.utils.conversion.coerce_to_array(I)
                for i in range(len(zod__zxgv)):
                    if not bodo.libs.array_kernels.isna(zod__zxgv, i):
                        val = (bodo.hiframes.pd_categorical_ext.
                            get_code_for_value(zod__zxgv.dtype, zod__zxgv[i]))
    ...
    if isinstance(I, CategoricalIndexType):
        ...
        def impl(I, val):
            ...
            if not is_null_value(I._dict):
                ...
                zod__zxgv = bodo.utils.conversion.coerce_to_array(I)
                dfead__zzq = (bodo.hiframes.pd_categorical_ext.
                    get_code_for_value(zod__zxgv.dtype, key))
                return dfead__zzq in I._dict
            else:
                znzf__ewmdq = (
                    'Global Index objects can be slow (pass as argument to JIT function for better performance).'
                    )
                warnings.warn(znzf__ewmdq)
                zod__zxgv = bodo.utils.conversion.coerce_to_array(I)
                ind = -1
                for i in range(len(zod__zxgv)):
                    if not bodo.libs.array_kernels.isna(zod__zxgv, i):
...
def overload_index_get_loc(I, key, method=None, tolerance=None):
    qkzc__ksaig = dict(method=method, tolerance=tolerance)
    udqq__wuzu = dict(method=None, tolerance=None)
    check_unsupported_args('Index.get_loc', qkzc__ksaig, udqq__wuzu,
        package_name='pandas', module_name='Index')
...
def create_isna_specific_method(overload_name):
    ...
    def overload_index_isna_specific_method(I):
        zwx__qjwcl = overload_name in {'isna', 'isnull'}
        if isinstance(I, RangeIndexType):

            def impl(I):
                numba.parfors.parfor.init_prange()
                bsz__jtfd = len(I)
                wyyzp__ccci = np.empty(bsz__jtfd, np.bool_)
                for i in numba.parfors.parfor.internal_prange(bsz__jtfd):
                    wyyzp__ccci[i] = not zwx__qjwcl
                return wyyzp__ccci
            return impl
        xth__gka = f"""def impl(I):
    numba.parfors.parfor.init_prange()
    arr = bodo.hiframes.pd_index_ext.get_index_data(I)
    n = len(arr)
    out_arr = np.empty(n, np.bool_)
    for i in numba.parfors.parfor.internal_prange(n):
       out_arr[i] = {'' if zwx__qjwcl else 'not '}bodo.libs.array_kernels.isna(arr, i)
    return out_arr
"""
        rov__fhqn = {}
        exec(xth__gka, {'bodo': bodo, 'np': np, 'numba': numba}, rov__fhqn)
        impl = rov__fhqn['impl']
        return impl
...
def overload_index_is_montonic(I):
    ...
    if isinstance(I, (NumericIndexType, DatetimeIndexType, TimedeltaIndexType)
        ...
        def impl(I):
            zod__zxgv = bodo.hiframes.pd_index_ext.get_index_data(I)
            return bodo.libs.array_kernels.series_monotonicity(zod__zxgv, 1)
        return impl
...
def overload_index_is_montonic_decreasing(I):
    ...
    if isinstance(I, (NumericIndexType, DatetimeIndexType, TimedeltaIndexType)
        ...
        def impl(I):
            zod__zxgv = bodo.hiframes.pd_index_ext.get_index_data(I)
            return bodo.libs.array_kernels.series_monotonicity(zod__zxgv, 2)
        return impl
    elif isinstance(I, RangeIndexType):
...
def overload_index_duplicated(I, keep='first'):
    ...
    def impl(I, keep='first'):
        zod__zxgv = bodo.hiframes.pd_index_ext.get_index_data(I)
        wyyzp__ccci = bodo.libs.array_kernels.duplicated((zod__zxgv,))
        return wyyzp__ccci
    return impl
...
def overload_index_drop_duplicates(I, keep='first'):
    qkzc__ksaig = dict(keep=keep)
    udqq__wuzu = dict(keep='first')
    check_unsupported_args('Index.drop_duplicates', qkzc__ksaig, udqq__wuzu,
        package_name='pandas', module_name='Index')
    if isinstance(I, RangeIndexType):
        return lambda I, keep='first': I.copy()
    xth__gka = """def impl(I, keep='first'):
...
"""
    if isinstance(I, PeriodIndexType):
        xth__gka += f"""    return bodo.hiframes.pd_index_ext.init_period_index(arr, name, '{I.freq}')
"""
    else:
        xth__gka += (
            '    return bodo.utils.conversion.index_from_array(arr, name)')
    rov__fhqn = {}
    exec(xth__gka, {'bodo': bodo}, rov__fhqn)
    impl = rov__fhqn['impl']
    return impl
...
def get_index_data_equiv(self, scope, equiv_set, loc, args, kws):
    assert len(args) == 1 and not kws
    fsjos__osui = args[0]
    if isinstance(self.typemap[fsjos__osui.name], (HeterogeneousIndexType,
        MultiIndexType)):
        return None
    if equiv_set.has_shape(fsjos__osui):
        return ArrayAnalysis.AnalyzeResult(shape=fsjos__osui, pre=[])
...
def overload_index_map(I, mapper, na_action=None):
    ...
    qkzc__ksaig = dict(na_action=na_action)
    acfq__eihcp = dict(na_action=None)
    check_unsupported_args('Index.map', qkzc__ksaig, acfq__eihcp,
        package_name='pandas', module_name='Index')
    dtype = I.dtype
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(I,
        'DatetimeIndex.map')
    ...
    ldy__azbz = numba.core.registry.cpu_target.typing_context
    csm__qpu = numba.core.registry.cpu_target.target_context
    try:
        bektp__fqgdq = get_const_func_output_type(mapper, (dtype,), {},
            ldy__azbz, csm__qpu)
    except Exception as umn__tex:
        raise_bodo_error(get_udf_error_msg('Index.map()', umn__tex))
    nphm__qny = get_udf_out_arr_type(bektp__fqgdq)
    func = get_overload_const_func(mapper, None)
    xth__gka = 'def f(I, mapper, na_action=None):\n'
    xth__gka += '  name = bodo.hiframes.pd_index_ext.get_index_name(I)\n'
    xth__gka += '  A = bodo.utils.conversion.coerce_to_array(I)\n'
    xth__gka += '  numba.parfors.parfor.init_prange()\n'
    xth__gka += '  n = len(A)\n'
    xth__gka += '  S = bodo.utils.utils.alloc_type(n, _arr_typ, (-1,))\n'
    xth__gka += '  for i in numba.parfors.parfor.internal_prange(n):\n'
    xth__gka += '    t2 = bodo.utils.conversion.box_if_dt64(A[i])\n'
    xth__gka += '    v = map_func(t2)\n'
    xth__gka += (
        '    S[i] = bodo.utils.conversion.unbox_if_tz_naive_timestamp(v)\n')
    xth__gka += '  return bodo.utils.conversion.index_from_array(S, name)\n'
    lne__dfn = bodo.compiler.udf_jit(func)
    rov__fhqn = {}
    exec(xth__gka, {'numba': numba, 'np': np, 'pd': pd, 'bodo': bodo,
        'map_func': lne__dfn, '_arr_typ': nphm__qny, 'init_nested_counts':
        bodo.utils.indexing.init_nested_counts, 'add_nested_counts': bodo.
        utils.indexing.add_nested_counts, 'data_arr_type': nphm__qny.dtype},
        rov__fhqn)
    f = rov__fhqn['f']
    return f
...
def index_is(context, builder, sig, args):
    fxmk__dis, rwlwo__lyr = sig.args
    if fxmk__dis != rwlwo__lyr:
    ...
    def overload_index_binary_op(lhs, rhs):
        if is_index_type(lhs):
            ...
            if rhs in [bodo.hiframes.pd_timestamp_ext.
                ...
                pd_timedelta_type]:
                xth__gka += """  dt = bodo.utils.conversion.unbox_if_tz_naive_timestamp(rhs)
  return op(arr, dt)
"""
            else:
                xth__gka += """  rhs_arr = bodo.utils.conversion.get_array_if_series_or_index(rhs)
  return op(arr, rhs_arr)
"""
            rov__fhqn = {}
            exec(xth__gka, {'bodo': bodo, 'op': op}, rov__fhqn)
            impl = rov__fhqn['impl']
            return impl
        if is_index_type(rhs):
            ...
            if lhs in [bodo.hiframes.pd_timestamp_ext.
                pd_timestamp_tz_naive_type, bodo.hiframes.pd_timestamp_ext.
                pd_timedelta_type]:
                xth__gka += """  dt = bodo.utils.conversion.unbox_if_tz_naive_timestamp(lhs)
  return op(dt, arr)
"""
            else:
                xth__gka += """  lhs_arr = bodo.utils.conversion.get_array_if_series_or_index(lhs)
        ...
        if isinstance(lhs, HeterogeneousIndexType):
            if not is_heterogeneous_tuple_type(lhs.data):
                ...
                def impl3(lhs, rhs):
                    ...
                    zod__zxgv = bodo.utils.conversion.coerce_to_array(data)
                    yqgz__vqx = (bodo.utils.conversion.
                        get_array_if_series_or_index(rhs))
                    wyyzp__ccci = op(zod__zxgv, yqgz__vqx)
                    return wyyzp__ccci
                return impl3
            count = len(lhs.data.types)
            xth__gka = 'def f(lhs, rhs):\n'
            xth__gka += '  return [{}]\n'.format(','.join(
                'op(lhs[{}], rhs{})'.format(i, f'[{i}]' if is_iterable_type
                (rhs) else '') for i in range(count)))
            rov__fhqn = {}
            exec(xth__gka, {'op': op, 'np': np}, rov__fhqn)
            impl = rov__fhqn['f']
            return impl
        if isinstance(rhs, HeterogeneousIndexType):
            if not is_heterogeneous_tuple_type(rhs.data):
                ...
                def impl4(lhs, rhs):
                    ...
                    wyyzp__ccci = op(yqgz__vqx, zod__zxgv)
                    return wyyzp__ccci
                return impl4
...
class HeterogeneousIndexModel(models.StructModel):
    ...
    def __init__(self, dmm, fe_type):
        vnl__uskw = [('data', fe_type.data), ('name', fe_type.name_typ)]
        super(HeterogeneousIndexModel, self).__init__(dmm, fe_type, vnl__uskw)
...
def overload_heter_index_copy(A, name=None, deep=False, dtype=None, names=None
    ...
    dnn__ljxr = idx_typ_to_format_str_map[HeterogeneousIndexType].format(
        'copy()')
    qtpp__rpeso = dict(deep=deep, dtype=dtype, names=names)
    check_unsupported_args('Index.copy', qtpp__rpeso, idx_cpy_arg_defaults,
        fn_str=dnn__ljxr, package_name='pandas', module_name='Index')
...
def overload_nbytes(I):
    if isinstance(I, RangeIndexType):
        ...
        xth__gka = 'def _impl_nbytes(I):\n'
        xth__gka += '    total = 0\n'
        xth__gka += '    data = I._data\n'
        for i in range(I.nlevels):
            xth__gka += f'    total += data[{i}].nbytes\n'
        xth__gka += '    return total\n'
        qxnvl__scg = {}
        exec(xth__gka, {}, qxnvl__scg)
        return qxnvl__scg['_impl_nbytes']
    else:
...
def overload_index_to_series(I, index=None, name=None):
    ...
    if is_overload_none(name):
        ukt__nqr = 'bodo.hiframes.pd_index_ext.get_index_name(I)'
    else:
        ukt__nqr = 'name'
    xth__gka = 'def impl(I, index=None, name=None):\n'
    xth__gka += '    data = bodo.utils.conversion.index_to_array(I)\n'
    if is_overload_none(index):
        xth__gka += '    new_index = I\n'
    elif is_pd_index_type(index):
        xth__gka += '    new_index = index\n'
    elif isinstance(index, SeriesType):
        xth__gka += '    arr = bodo.utils.conversion.coerce_to_array(index)\n'
        xth__gka += (
...
def overload_index_to_frame(I, index=True, name=None):
    ...
    xth__gka = 'def impl(I, index=True, name=None):\n'
    xth__gka += '    data = bodo.utils.conversion.index_to_array(I)\n'
    xth__gka += f'    new_index = {nhyv__opy}\n'
    if is_overload_none(name) and I.name_typ == types.none:
        rjvbj__pybks = ColNamesMetaType((0,))
    elif is_overload_none(name):
        rjvbj__pybks = ColNamesMetaType((I.name_typ,))
    elif is_overload_constant_str(name):
        rjvbj__pybks = ColNamesMetaType((get_overload_const_str(name),))
    elif is_overload_constant_int(name):
        rjvbj__pybks = ColNamesMetaType((get_overload_const_int(name),))
    else:
        raise_bodo_error(
            f'Index.to_frame(): only constant string/int are supported for argument name'
            )
    xth__gka += """    return bodo.hiframes.pd_dataframe_ext.init_dataframe((data,), new_index, __col_name_meta_value)
"""
    rov__fhqn = {}
    exec(xth__gka, {'bodo': bodo, 'np': np, '__col_name_meta_value':
        rjvbj__pybks}, rov__fhqn)
    impl = rov__fhqn['impl']
    return impl
...
def overload_multi_index_to_frame(I, index=True, name=None):
    ...
    xth__gka = 'def impl(I, index=True, name=None):\n'
    xth__gka += '    data = bodo.hiframes.pd_index_ext.get_index_data(I)\n'
    xth__gka += f'    new_index = {nhyv__opy}\n'
    msy__bzzl = len(I.array_types)
    if is_overload_none(name) and I.names_typ == (types.none,) * msy__bzzl:
        rjvbj__pybks = ColNamesMetaType(tuple(range(msy__bzzl)))
    elif is_overload_none(name):
        rjvbj__pybks = ColNamesMetaType(I.names_typ)
        ...
        if msy__bzzl != len(names):
            raise_bodo_error(
                f'MultiIndex.to_frame(): expected {msy__bzzl} names, not {len(names)}'
                )
        if all(is_overload_constant_str(tfjqx__twxsl) or
            is_overload_constant_int(tfjqx__twxsl) for tfjqx__twxsl in names):
            rjvbj__pybks = ColNamesMetaType(names)
        else:
            raise_bodo_error(
...
def overload_index_to_numpy(I, dtype=None, copy=False, na_value=None):
    qkzc__ksaig = dict(dtype=dtype, na_value=na_value)
    udqq__wuzu = dict(dtype=None, na_value=None)
    check_unsupported_args('Index.to_numpy', qkzc__ksaig, udqq__wuzu,
        package_name='pandas', module_name='Index')
...
def overload_inferred_type(I):
    ...
    if isinstance(I, StringIndexType):
        ...
        def impl(I):
            ...
            return 'string'
        return impl
    dcn__vmqet = {DatetimeIndexType: 'datetime64', TimedeltaIndexType:
    ...
    inferred_type = dcn__vmqet[type(I)]
...
def overload_inferred_type(I):
    ...
    if isinstance(I, CategoricalIndexType):
        ...
        return lambda I: dtype
    wgvl__afoss = {DatetimeIndexType: np.dtype('datetime64[ns]'),
...
def init_index_from_index(I, name):
    vtj__mgxe = {NumericIndexType: bodo.hiframes.pd_index_ext.
    ...
    if type(I) in vtj__mgxe:
        init_func = vtj__mgxe[type(I)]
...
def get_index_constructor(I):
    lxwj__glmiy = {NumericIndexType: bodo.hiframes.pd_index_ext.
        init_numeric_index, DatetimeIndexType: bodo.hiframes.pd_index_ext.
        init_datetime_index, TimedeltaIndexType: bodo.hiframes.pd_index_ext
        .init_timedelta_index, StringIndexType: bodo.hiframes.pd_index_ext.
        init_binary_str_index, BinaryIndexType: bodo.hiframes.pd_index_ext.
    ...
    if type(I) in lxwj__glmiy:
        return lxwj__glmiy[type(I)]
    raise BodoError(
...
def overload_index_min(I, axis=None, skipna=True):
    qkzc__ksaig = dict(axis=axis, skipna=skipna)
    udqq__wuzu = dict(axis=None, skipna=True)
    check_unsupported_args('Index.min', qkzc__ksaig, udqq__wuzu,
        package_name='pandas', module_name='Index')
    if isinstance(I, RangeIndexType):
        ...
        def impl(I, axis=None, skipna=True):
            ...
            if ffwm__ruzov == 0:
                return np.nan
            if I._step < 0:
                return I._start + I._step * (ffwm__ruzov - 1)
            else:
                return I._start
        return impl
    if isinstance(I, CategoricalIndexType):
    ...
    def impl(I, axis=None, skipna=True):
        zod__zxgv = bodo.hiframes.pd_index_ext.get_index_data(I)
        return bodo.libs.array_ops.array_op_min(zod__zxgv)
    return impl
...
def overload_index_max(I, axis=None, skipna=True):
    qkzc__ksaig = dict(axis=axis, skipna=skipna)
    udqq__wuzu = dict(axis=None, skipna=True)
    check_unsupported_args('Index.max', qkzc__ksaig, udqq__wuzu,
        package_name='pandas', module_name='Index')
    if isinstance(I, RangeIndexType):
    ...
    def impl(I, axis=None, skipna=True):
        zod__zxgv = bodo.hiframes.pd_index_ext.get_index_data(I)
        return bodo.libs.array_ops.array_op_max(zod__zxgv)
    return impl
...
def overload_index_argmin(I, axis=0, skipna=True):
    qkzc__ksaig = dict(axis=axis, skipna=skipna)
    udqq__wuzu = dict(axis=0, skipna=True)
    check_unsupported_args('Index.argmin', qkzc__ksaig, udqq__wuzu,
        package_name='pandas', module_name='Index')
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(I,
        'Index.argmin()')
    if isinstance(I, RangeIndexType):
    ...
    def impl(I, axis=0, skipna=True):
        zod__zxgv = bodo.hiframes.pd_index_ext.get_index_data(I)
        index = init_numeric_index(np.arange(len(zod__zxgv)))
        return bodo.libs.array_ops.array_op_idxmin(zod__zxgv, index)
    return impl
...
def overload_index_argmax(I, axis=0, skipna=True):
    qkzc__ksaig = dict(axis=axis, skipna=skipna)
    udqq__wuzu = dict(axis=0, skipna=True)
    check_unsupported_args('Index.argmax', qkzc__ksaig, udqq__wuzu,
        package_name='pandas', module_name='Index')
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(I,
        'Index.argmax()')
    if isinstance(I, RangeIndexType):
    ...
    def impl(I, axis=0, skipna=True):
        zod__zxgv = bodo.hiframes.pd_index_ext.get_index_data(I)
        index = np.arange(len(zod__zxgv))
        return bodo.libs.array_ops.array_op_idxmax(zod__zxgv, index)
    return impl
...
def overload_index_unique(I):
    lweh__ewqat = get_index_constructor(I)

    def impl(I):
        zod__zxgv = bodo.hiframes.pd_index_ext.get_index_data(I)
        name = bodo.hiframes.pd_index_ext.get_index_name(I)
        ssn__qrj = bodo.libs.array_kernels.unique(zod__zxgv)
        return lweh__ewqat(ssn__qrj, name)
    return impl
...
def overload_index_nunique(I, dropna=True):
    ...
    def impl(I, dropna=True):
        zod__zxgv = bodo.hiframes.pd_index_ext.get_index_data(I)
        bsz__jtfd = bodo.libs.array_kernels.nunique(zod__zxgv, dropna)
        return bsz__jtfd
    return impl
...
def overload_index_isin(I, values):
    if bodo.utils.utils.is_array_typ(values):
        ...
        def impl_arr(I, values):
            vthdm__cghjm = bodo.utils.conversion.coerce_to_array(values)
            A = bodo.hiframes.pd_index_ext.get_index_data(I)
            bsz__jtfd = len(A)
            wyyzp__ccci = np.empty(bsz__jtfd, np.bool_)
            bodo.libs.array.array_isin(wyyzp__ccci, A, vthdm__cghjm, False)
            return wyyzp__ccci
        return impl_arr
    ...
    def impl(I, values):
        A = bodo.hiframes.pd_index_ext.get_index_data(I)
        wyyzp__ccci = bodo.libs.array_ops.array_op_isin(A, values)
        return wyyzp__ccci
    return impl
...
def order_range(I, ascending):
    ...
    if ascending == (step > 0):
        ...
        ffwm__ruzov = len(I)
        fxrl__oqfld = start + step * (ffwm__ruzov - 1)
        ydtkf__opfz = fxrl__oqfld - step * ffwm__ruzov
        return init_range_index(fxrl__oqfld, ydtkf__opfz, -step, name)
...
def overload_index_sort_values(I, return_indexer=False, ascending=True,
    ...
    qkzc__ksaig = dict(return_indexer=return_indexer, key=key)
    udqq__wuzu = dict(return_indexer=False, key=None)
    check_unsupported_args('Index.sort_values', qkzc__ksaig, udqq__wuzu,
        package_name='pandas', module_name='Index')
    ...
    if isinstance(I, RangeIndexType):
        ...
        return impl
    lweh__ewqat = get_index_constructor(I)
    idwgn__kamo = ColNamesMetaType(('$_bodo_col_',))

    def impl(I, return_indexer=False, ascending=True, na_position='last',
        key=None):
        zod__zxgv = bodo.hiframes.pd_index_ext.get_index_data(I)
        name = get_index_name(I)
        index = init_range_index(0, len(zod__zxgv), 1, None)
        scrch__aqx = bodo.hiframes.pd_dataframe_ext.init_dataframe((
            zod__zxgv,), index, idwgn__kamo)
        zfolw__jqhno = scrch__aqx.sort_values(['$_bodo_col_'], ascending=
            ascending, inplace=False, na_position=na_position)
        wyyzp__ccci = bodo.hiframes.pd_dataframe_ext.get_dataframe_data(
            zfolw__jqhno, 0)
        return lweh__ewqat(wyyzp__ccci, name)
    return impl
...
def overload_index_argsort(I, axis=0, kind='quicksort', order=None):
    qkzc__ksaig = dict(axis=axis, kind=kind, order=order)
    udqq__wuzu = dict(axis=0, kind='quicksort', order=None)
    check_unsupported_args('Index.argsort', qkzc__ksaig, udqq__wuzu,
        package_name='pandas', module_name='Index')
    if isinstance(I, RangeIndexType):
    ...
    def impl(I, axis=0, kind='quicksort', order=None):
        zod__zxgv = bodo.hiframes.pd_index_ext.get_index_data(I)
        wyyzp__ccci = bodo.hiframes.series_impl.argsort(zod__zxgv)
        return wyyzp__ccci
    return impl
...
def overload_index_where(I, cond, other=np.nan):
    ...
    if is_overload_constant_nan(other):
        ...
        bhtzp__jqxm = 'other'
    xth__gka = 'def impl(I, cond, other=np.nan):\n'
    if isinstance(I, RangeIndexType):
        xth__gka += '  arr = np.arange(I._start, I._stop, I._step)\n'
        lweh__ewqat = 'init_numeric_index'
    else:
        xth__gka += '  arr = bodo.hiframes.pd_index_ext.get_index_data(I)\n'
    xth__gka += '  name = bodo.hiframes.pd_index_ext.get_index_name(I)\n'
    xth__gka += (
        f'  out_arr = bodo.hiframes.series_impl.where_impl(cond, arr, {bhtzp__jqxm})\n'
        )
    xth__gka += f'  return constructor(out_arr, name)\n'
    rov__fhqn = {}
    lweh__ewqat = init_numeric_index if isinstance(I, RangeIndexType
        ) else get_index_constructor(I)
    exec(xth__gka, {'bodo': bodo, 'np': np, 'constructor': lweh__ewqat},
        rov__fhqn)
    impl = rov__fhqn['impl']
    return impl
...
def overload_index_putmask(I, cond, other):
    ...
    if is_overload_constant_nan(other):
        ...
        bhtzp__jqxm = 'other'
    xth__gka = 'def impl(I, cond, other):\n'
    xth__gka += '  cond = ~cond\n'
    if isinstance(I, RangeIndexType):
        xth__gka += '  arr = np.arange(I._start, I._stop, I._step)\n'
    else:
...
def overload_index_repeat(I, repeats, axis=None):
    qkzc__ksaig = dict(axis=axis)
    udqq__wuzu = dict(axis=None)
    check_unsupported_args('Index.repeat', qkzc__ksaig, udqq__wuzu,
        package_name='pandas', module_name='Index')
    bodo.hiframes.pd_timestamp_ext.check_tz_aware_unsupported(I,
        'Index.repeat()')
    ...
    xth__gka = 'def impl(I, repeats, axis=None):\n'
    ...
    xth__gka += '    return constructor(out_arr, name)'
    rov__fhqn = {}
    lweh__ewqat = init_numeric_index if isinstance(I, RangeIndexType
        ) else get_index_constructor(I)
    exec(xth__gka, {'bodo': bodo, 'np': np, 'constructor': lweh__ewqat},
...
def lower_constant_time_index(context, builder, ty, pyval):
    ...
    name = context.get_constant_generic(builder, ty.name_typ, pyval.name)
    dtype = ty.dtype
    acmit__gtet = context.get_constant_null(types.DictType(dtype, types.int64))
    return lir.Constant.literal_struct([data, name, acmit__gtet])
...
def lower_constant_period_index(context, builder, ty, pyval):
    ...
    name = context.get_constant_generic(builder, ty.name_typ, pyval.name)
    acmit__gtet = context.get_constant_null(types.DictType(types.int64,
        types.int64))
    return lir.Constant.literal_struct([data, name, acmit__gtet])
...
def lower_constant_binary_string_index(context, builder, ty, pyval):
    ...
    data = context.get_constant_generic(builder, exth__xzcvm, pyval.values)
    name = context.get_constant_generic(builder, ty.name_typ, pyval.name)
    acmit__gtet = context.get_constant_null(types.DictType(scalar_type,
        types.int64))
    return lir.Constant.literal_struct([data, name, acmit__gtet])
...
def getiter_range_index(context, builder, sig, args):
    [npo__uole] = sig.args
    [index] = args
    heoan__lmpsk = context.make_helper(builder, npo__uole, value=index)
    lsqaz__rjrvt = context.make_helper(builder, sig.return_type)
    sou__kim = cgutils.alloca_once_value(builder, heoan__lmpsk.start)
    yxb__dlq = context.get_constant(types.intp, 0)
    prtf__duxu = cgutils.alloca_once_value(builder, yxb__dlq)
    lsqaz__rjrvt.iter = sou__kim
    lsqaz__rjrvt.stop = heoan__lmpsk.stop
    lsqaz__rjrvt.step = heoan__lmpsk.step
    lsqaz__rjrvt.count = prtf__duxu
    tscv__les = builder.sub(heoan__lmpsk.stop, heoan__lmpsk.start)
    wzren__gtr = context.get_constant(types.intp, 1)
    vyqg__btxjl = builder.icmp_signed('>', tscv__les, yxb__dlq)
    huj__mbqy = builder.icmp_signed('>', heoan__lmpsk.step, yxb__dlq)
    qdhmq__jzir = builder.not_(builder.xor(vyqg__btxjl, huj__mbqy))
    with builder.if_then(qdhmq__jzir):
        owfl__wfad = builder.srem(tscv__les, heoan__lmpsk.step)
        owfl__wfad = builder.select(vyqg__btxjl, owfl__wfad, builder.neg(
            owfl__wfad))
        jyjw__tioo = builder.icmp_signed('>', owfl__wfad, yxb__dlq)
        ftk__mxo = builder.add(builder.sdiv(tscv__les, heoan__lmpsk.step),
            builder.select(jyjw__tioo, wzren__gtr, yxb__dlq))
        builder.store(ftk__mxo, prtf__duxu)
    pcp__nhwu = lsqaz__rjrvt._getvalue()
    zra__pko = impl_ret_new_ref(context, builder, sig.return_type, pcp__nhwu)
    return zra__pko
...
def _install_index_unsupported():
    for xzgp__qbu in index_unsupported_methods:
        for cyezy__fmoin, typ in index_types:
            overload_method(typ, xzgp__qbu, no_unliteral=True)(
                create_unsupported_overload(cyezy__fmoin.format(xzgp__qbu +
                '()')))
    for yhj__mzv in index_unsupported_atrs:
        for cyezy__fmoin, typ in index_types:
            overload_attribute(typ, yhj__mzv, no_unliteral=True)(
                create_unsupported_overload(cyezy__fmoin.format(yhj__mzv)))
    kqn__rpf = [(StringIndexType, string_index_unsupported_atrs), (
    ...
    dbcpu__bfdl = [(CategoricalIndexType, cat_idx_unsupported_methods), (
    ...
    for typ, bayo__ubbl in dbcpu__bfdl:
        cyezy__fmoin = idx_typ_to_format_str_map[typ]
        for djz__yzvzr in bayo__ubbl:
            overload_method(typ, djz__yzvzr, no_unliteral=True)(
                create_unsupported_overload(cyezy__fmoin.format(djz__yzvzr +
                '()')))
    for typ, lytp__uyth in kqn__rpf:
        cyezy__fmoin = idx_typ_to_format_str_map[typ]
        for yhj__mzv in lytp__uyth:
            overload_attribute(typ, yhj__mzv, no_unliteral=True)(
                create_unsupported_overload(cyezy__fmoin.format(yhj__mzv)))
