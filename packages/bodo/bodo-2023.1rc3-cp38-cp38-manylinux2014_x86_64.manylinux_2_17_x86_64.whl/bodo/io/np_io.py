...
def get_dtype_size(typingctx, dtype=None):
    ...
    def codegen(context, builder, sig, args):
        usa__crd = context.get_abi_sizeof(context.get_data_type(dtype.dtype))
        return context.get_constant(types.intp, usa__crd)
...
def tofile_overload(arr, fname):
    if fname == string_type or isinstance(fname, types.StringLiteral):
        ...
        def tofile_impl(arr, fname):
            ...
            vurpr__cag = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(vurpr__cag.dtype)
            file_write(unicode_to_utf8(fname), vurpr__cag.ctypes, 
                dtype_size * vurpr__cag.size)
...
def file_write_parallel_overload(fname, arr, start, count):
    if fname == string_type:
        ...
        def _impl(fname, arr, start, count):
            vurpr__cag = np.ascontiguousarray(arr)
            dtype_size = get_dtype_size(vurpr__cag.dtype)
            hpka__ysmg = dtype_size * bodo.libs.distributed_api.get_tuple_prod(
                vurpr__cag.shape[1:])
            _file_write_parallel(unicode_to_utf8(fname), vurpr__cag.ctypes,
                start, count, hpka__ysmg)
            bodo.utils.utils.check_and_propagate_cpp_exception()
        return _impl
...
def get_file_size_overload(fname, count, offset, dtype_size):
    if fname == string_type:
        ...
        def impl(fname, count, offset, dtype_size):
            if offset < 0:
                return -1
            phsko__kcdrx = _get_file_size(unicode_to_utf8(fname)) - offset
            bodo.utils.utils.check_and_propagate_cpp_exception()
            if count != -1:
                phsko__kcdrx = min(phsko__kcdrx, count * dtype_size)
            if phsko__kcdrx < 0:
                return -1
            return phsko__kcdrx
        return impl
