...
def box_regex_flag(typ, val, c):
    vpo__jcw = c.box(types.int64, val)
    yigei__ahp = c.pyapi.unserialize(c.pyapi.serialize_object(re.RegexFlag))
    return c.pyapi.call_function_objargs(yigei__ahp, (vpo__jcw,))


@unbox(RegexFlagsType)
def unbox_regex_flag(typ, obj, c):
    vpo__jcw = c.pyapi.object_getattr_string(obj, 'value')
    return c.unbox(types.int64, vpo__jcw)
...
def cast_match_obj_bool(context, builder, fromty, toty, val):
    out = cgutils.alloca_once_value(builder, context.get_constant(types.
        bool_, True))
    kzcig__rlnvp = context.get_python_api(builder)
    rzcg__klx = builder.icmp_signed('==', val, kzcig__rlnvp.borrow_none())
    with builder.if_then(rzcg__klx):
...
def init_const_pattern(typingctx, pat, pat_const=None):
    aiiw__orf = get_overload_const_str(pat_const)

    def codegen(context, builder, sig, args):
        return impl_ret_borrowed(context, builder, sig.return_type, args[0])
    return RePatternType(aiiw__orf)(pat, pat_const), codegen
...
def overload_pat_findall(p, string, pos=0, endpos=9223372036854775807):
    if p.pat_const:
        ncf__hvnu = re.compile(p.pat_const).groups
        typ = types.List(string_type)
        if ncf__hvnu > 1:
            typ = types.List(types.Tuple([string_type] * ncf__hvnu))
...
def overload_match_group(m, *args):
    ...
    xvaxg__kuqp = types.optional(string_type)
    if len(args) == 1:

        def _match_group_impl_one(m, *args):
            ikltl__qmrn = args[0]
            with numba.objmode(out=xvaxg__kuqp):
                out = m.group(ikltl__qmrn)
            return out
        return _match_group_impl_one
    avzzt__lsw = 'tuple_str_{}'.format(len(args))
    setattr(types, avzzt__lsw, types.Tuple([xvaxg__kuqp] * len(args)))
    ijn__mihcs = ', '.join('group{}'.format(vwxxw__bxp + 1) for vwxxw__bxp in
        range(len(args)))
    suw__qlg = 'def _match_group_impl(m, *args):\n'
    suw__qlg += '  ({}) = args\n'.format(ijn__mihcs)
    suw__qlg += "  with numba.objmode(out='{}'):\n".format(avzzt__lsw)
    suw__qlg += '    out = m.group({})\n'.format(ijn__mihcs)
    suw__qlg += '  return out\n'
    nkwrs__wrc = {}
    exec(suw__qlg, globals(), nkwrs__wrc)
    ocn__vta = nkwrs__wrc['_match_group_impl']
    return ocn__vta
...
def overload_match_groups(m, default=None):
    pzd__ywn = types.List(types.optional(string_type))

    def _match_groups_impl(m, default=None):
        with numba.objmode(out=pzd__ywn):
...
def overload_match_groupdict(m, default=None):
    ...
    def _check_dict_none(out):
        if any(fzxzz__kbf is None for fzxzz__kbf in out.values()):
            raise BodoError(
...
def overload_match_lastgroup(p):
    ...
    def _match_lastgroup_impl(p):
        with numba.objmode(lastgroup=xvaxg__kuqp):
