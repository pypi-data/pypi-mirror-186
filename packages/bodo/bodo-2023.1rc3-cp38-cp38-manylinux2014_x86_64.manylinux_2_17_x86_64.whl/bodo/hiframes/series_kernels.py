...
def _column_filter_impl(B, ind):
    gst__rnqsy = bodo.hiframes.rolling.alloc_shift(len(B), B, (-1,), None)
    for cmkt__ndjre in numba.parfors.parfor.internal_prange(len(gst__rnqsy)):
        if ind[cmkt__ndjre]:
            gst__rnqsy[cmkt__ndjre] = B[cmkt__ndjre]
        else:
            bodo.libs.array_kernels.setna(gst__rnqsy, cmkt__ndjre)
    return gst__rnqsy
...
def _series_dropna_str_alloc_impl_inner(B):
    ...
    qgh__uedrw = len(B)
    fualg__owf = 0
    for cmkt__ndjre in range(len(B)):
        if bodo.libs.str_arr_ext.str_arr_is_na(B, cmkt__ndjre):
            fualg__owf += 1
    skcgf__kten = qgh__uedrw - fualg__owf
    javos__sjdbl = bodo.libs.str_arr_ext.num_total_chars(B)
    gst__rnqsy = bodo.libs.str_arr_ext.pre_alloc_string_array(skcgf__kten,
        javos__sjdbl)
    bodo.libs.str_arr_ext.copy_non_null_offsets(gst__rnqsy, B)
    bodo.libs.str_arr_ext.copy_data(gst__rnqsy, B)
    bodo.libs.str_arr_ext.set_null_bits_to_value(gst__rnqsy, -1)
    return gst__rnqsy
...
def compute_skew(first_moment, second_moment, third_moment, count):
    if count < 3:
        return np.nan
    zdghb__foi = first_moment / count
    dled__pwo = (third_moment - 3 * second_moment * zdghb__foi + 2 * count *
        zdghb__foi ** 3)
    yncwn__tciy = second_moment - zdghb__foi * first_moment
    s = count * (count - 1) ** 1.5 / (count - 2
        ) * dled__pwo / yncwn__tciy ** 1.5
    s = s / (count - 1)
    return s
...
def compute_kurt(first_moment, second_moment, third_moment, fourth_moment,
    ...
    if count < 4:
        return np.nan
    zdghb__foi = first_moment / count
    ofym__xoam = (fourth_moment - 4 * third_moment * zdghb__foi + 6 *
        second_moment * zdghb__foi ** 2 - 3 * count * zdghb__foi ** 4)
    vvyzi__qhs = second_moment - zdghb__foi * first_moment
    iolj__tppal = 3 * (count - 1) ** 2 / ((count - 2) * (count - 3))
    jsgf__mcdob = count * (count + 1) * (count - 1) * ofym__xoam
    kyqr__sphzc = (count - 2) * (count - 3) * vvyzi__qhs ** 2
    s = (count - 1) * (jsgf__mcdob / kyqr__sphzc - iolj__tppal)
    s = s / (count - 1)
    return s
