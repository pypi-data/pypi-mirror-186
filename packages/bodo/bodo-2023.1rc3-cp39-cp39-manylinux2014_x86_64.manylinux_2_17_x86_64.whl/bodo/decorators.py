...
def distributed_diagnostics(self, signature=None, level=1):
    ...
    def dump(sig):
        fflv__kdk = self.overloads[sig]
        afcz__afztb = fflv__kdk.metadata.get('distributed_diagnostics', None)
        if afcz__afztb is None:
            vbjb__eqg = 'No distributed diagnostic available'
            raise bodo.utils.typing.BodoError(vbjb__eqg)
        afcz__afztb.dump(level, self.get_metadata(sig))
...
def master_mode_wrapper(numba_jit_wrapper):
    ...
    def _wrapper(pyfunc):
        ylbow__nhvf = numba_jit_wrapper(pyfunc)
        return master_mode.MasterModeDispatcher(ylbow__nhvf)
    return _wrapper
...
def jit(signature_or_function=None, pipeline_class=None, **options):
    ...
    if 'distributed' in options and isinstance(options['distributed'], bool):
        qpet__ksfg = options.pop('distributed')
        pipeline_class = (pipeline_class if qpet__ksfg else bodo.compiler.
            BodoCompilerSeq)
    if 'replicated' in options and isinstance(options['replicated'], bool):
        oss__vgtpt = options.pop('replicated')
        pipeline_class = (bodo.compiler.BodoCompilerSeq if oss__vgtpt else
            pipeline_class)
    icyza__sfx = numba.jit(signature_or_function, pipeline_class=
        pipeline_class, **options)
    if master_mode.master_mode_on and bodo.get_rank(
        ) == master_mode.MASTER_RANK:
        if isinstance(icyza__sfx, numba.dispatcher._DispatcherBase):
            return master_mode.MasterModeDispatcher(icyza__sfx)
        else:
            return master_mode_wrapper(icyza__sfx)
    else:
        return icyza__sfx
