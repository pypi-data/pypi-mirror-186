...
class UserMethods:
    ...
    get_chat_last_author        =   lambda self=Client, chat_id: self.get_chat_info(chat_id).get('data').get('chat').get('last_message').get('author_object_guid')
...
class EventBuilder(Client):
    ...
    def pin(self, chat_id: str,
        ...
        return StartClient(self.auth).set_pin_message(chat_id=chat_id, message_id=message_id, action='Pin')
    
    def unpin(self, chat_id: str,
              message_id: str) -> (dict):
        return StartClient(self.auth).set_pin_message(chat_id=chat_id, message_id=message_id, action='Unpin')

    def seen(self, chat_id: str,
             message: str) -> (dict):
        return StartClient(self.auth).seen_chats({chat_id: message})
    
    def reply(self, chat_id: str, text: str,
              message: str) -> (dict):
        return StartClient(self.auth).send_message(text, chat_id, reply_to_message_id=message)
    
    def edit(self, chat_id: str,
             text: str, message: str) -> (dict):
        return StartClient(self.auth).edit_message(message, text, chat_id)
    
    def forwards(self, from_: str,
                 to: str, messages: list) -> (dict):
        return StartClient(self.auth).forward_messages(from_, messages, to)
    
    def download(self, chat_id: str,
                 message: str, name: str) -> (dict):
        return StartClient(self.auth).get_file('message', True, saveAS='name', chat_id=chat_id, message_id=message)

    def delete(self, chat_id: str,
               messages: list) -> (dict):
        StartClient(self.auth).delete_messages(messages, chat_id)
...
class Handler(Client):
    ...
    def __appender(self, msg_id: str = None, action: str = 'edit'):
        ...
        if action == 'edit':
            if not msg_id in open(self.auth+'-ids.sty', 'r').splitlines():
    ...
    def handle(self, method: str = 'ChatsUpdates', get_chats: bool = True,
        ...
        if ('handshake' in method.lower()):
            ...
            #from . import StartClient # TODO: use __init__
            
            self.__appender(action='create')
            
            for msg in StartClient(self.auth).get_chats_updates().get('data').get('chats'):
                
                if not msg.get('last_message').get('message_id') in open(self.auth+'-ids.sty', 'r').splitlines():
                    ...
                    if chat_ids:
                        ...
                        if msg.get('last_message').get('object_guid') in chat_ids:
                            if pattern:
                                if search(r'%s' % pattern[0], msg.get('last_message').get('text')):
    ...
    def handler(self, builder: object) -> (typing.Union[object,
        ...
        def decorator():
            ...
            #from . import StartClient # __init__
            ...
            if self.handling.get('events'):
                ...
                self.handling.update({'events': dict(get_message=True, get_chats=True)})
            if self.handling.get('func') and isinstance(self.handling.get('func'), str) and self.handling.get('func').lower() in ['handshake', 'chatsupdates', 'messagesupdates']:
                pass
            else:
                self.handling.update({'func': 'chatsupdates'})
            
            for (event) in (update.handle(self.handling.get('func'), **self.handling.get('events') or {})):
                message = EventBuilder(event)
                builder(methods, message, event)
        
        #while 1:
        __import__('threading').Thread(target=decorator).start()
    ...
    def add_event_handling(self, **handlers) -> (None):
        self.handling.update({'func': handlers.get('func') or handlers.get('method'), 'events': funcs.get('events')})
