...
from mldesigner._utils import _is_primitive_type
...
class NodeRunSettingWriter:
    ...
    def _delete_prefix_in_compute(self):
        if 'compute' in self.node_attribute_dict.keys():
            ...
            self.node_attribute_dict['compute'] = \
                literal_eval(_process_data_of_binding_and_primitive(self.node_attribute_dict['compute']))
    ...
    def conf_setter(self, attribute_value):
        ...
        for attr, attr_value in attribute_value.items():
            try:
                ...
                self.run_setting[renamed_attr] = \
                    f"'{attr_value}'" if isinstance(attr_value, str) else attr_value
            except KeyError:
            ...
            try:
                ...
                if limit_attribute_value:
                    limit_str_list.append(
                        f"{limit_attribute}="
                        f"{_process_data_of_binding_and_primitive(limit_attribute_value)}"
                    )
...
class CommandRunSettingWriter(NodeRunSettingWriter):
    def __init__(self, node, logger):
        ...
        self.command_attribute_need = ['limits', 'environment', 'compute', 'environment_variables', 'resources',
                                       'distribution', 'identity', 'comment']
...
class SparkRunSettingWriter(NodeRunSettingWriter):
    def __init__(self, node, logger):
        ...
        self.spark_attribute_no_need = [
            'properties', 'component', 'args', 'code', 'jars', 'files', 'py_files', 'archives', 'entry']
...
class ParallelRunSettingWriter(NodeRunSettingWriter):
    def __init__(self, node, logger):
        ...
        self.parallel_attribute_need = ['logging_level', 'mini_batch_size', 'partition_keys',
                                   'resources', 'retry_settings', 'max_concurrency_per_instance',
                                   'error_threshold', 'mini_batch_error_threshold', 'environment_variables',
                                   'comment', 'compute']
        self._update_run_setting_dict()
...
class SweepRunSettingWriter(NodeRunSettingWriter):
    def __init__(self, node, logger):
        ...
        self.sweep_attribute_need = ['comment', 'compute', 'objective', 'early_termination', 'limits',
                                     'sampling_algorithm', 'search_space']
        self._update_run_setting_dict()
