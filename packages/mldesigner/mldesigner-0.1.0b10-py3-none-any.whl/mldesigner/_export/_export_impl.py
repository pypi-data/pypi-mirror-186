...
from azure.ai.ml.entities._builders import Command, Pipeline, Parallel, Spark, Sweep

from mldesigner._exceptions import UserErrorException
from mldesigner._export._parse_url import _parse_designer_url
from mldesigner._generate._generators.pipeline_generator import (
...
async def _load_component_from_pipeline(client, job_entity):
    ...
    for node in job_entity.jobs.values():  # pylint: disable=no-member
        ...
        if isinstance(node, (Command, Parallel, Spark)):
            ...
            subgraph_name2component_dict, sub_subgraph_components, sub_subgraphs = \
                await _load_component_from_pipeline(client, component)
...
def _export(source: str, include_components: List[str] = None):  # pylint: disable=unused-argument
    ...
    pattern_to_components, subgraph_components, subgraphs = \
        asyncio.run(_load_component_from_pipeline(client, job_entity))
