...
from mldesigner._utils import _write_properties_to_run_history
...
class Output(_IOBase):
    ...
    def ready(self) -> None:
        ...
        if self._ready is True:
            execute_logger.warning("Output '%s' has already been marked as ready, ignore current operation.", self.name)
            return
        if self._value is None:
            error_message = f"Early available output {self.name!r} is not ready yet, please assign value for it."
        ...
        if len(self.name) > 75:
            error_message = f"Early available output {self.name!r} name is too long, the limit is 75 characters."
            raise UserErrorException(error_message)
        if isinstance(self._value, str) and len(self._value) > 1000:
            error_message = (
                f"Early available output {self.name!r} content is too long, the limit is 1000 characters. "
        ...
        if _parsed_args is not None:
            ...
            Path(_parsed_args[self.name]).write_text(str(self._value))

        # write content to RH
        early_available_control_output_key = f"azureml.pipeline.control.{self.name}"
