...
from .program import AnnotatedDisjunction, NeuralFact, NeuralAD
...
class StableTransformer(lark.Transformer):
  ...
  def data(self, D):
    ...
    feats, furl = D[2][1], D[2][0] == "URL"
    labels, lurl = D[3][1], D[3][0] == "URL"
    return self.pack("data", f"{name}({arg}).", Data(name, arg, feats, furl, labels, lurl))

  # Neural fact.
  def nfact(self, A):
    name = A[0][1]
    inp = A[1][1]
    where_from, net, _, _ = A[2]
    body = A[3:]
    scope = self.join_scope(A)

    if len(scope) != 1:  raise ValueError(f"Neural fact {name} is not grounded!")
    if inp not in scope: raise ValueError(f"Neural fact {name} is unsafe!")

    return self.pack("nfact", "", (name, inp, net, where_from == "URL", body))
  ...
  def nad(self, A):
    ...
    return self.pack("nad", "", (name, inp, vals, net, where_from == "URL", body))
