...
/*bool exact_choice(program_t *P, double (*R)[2], bool lstable_sat, psemantics_t psem, bool quiet) {*/
  /*size_t total_choice_n = P->CF_n > 0 ? TOTAL_CHOICE_NCREDAL(P) : TOTAL_CHOICE_N(P);*/
  /*total_choice_t theta;*/
  /*bool ok = false;*/
  /*clingo_control_t *C = NULL;*/

  /*double *a, *b, *c, *d = c = b = a = NULL;*/
  /*size_t *count_e, *count_q_e, *count_partial_q_e = count_q_e = count_e = NULL;*/
  /*bool *cond_1, *cond_2, *cond_3, *cond_4 = cond_3 = cond_2 = cond_1 = NULL;*/

  /*if (psem == MAXENT_SEMANTICS) {*/
    /*if (!setup_counts(&count_q_e, &count_e, NULL, P->Q_n*sizeof(size_t))) goto cleanup;*/
    /*if (!setup_abcd(&a, &b, NULL, NULL, P->Q_n, sizeof(double))) goto cleanup;*/
  /*} else [> credal semantics <] {*/
    /*if (!setup_conds(&cond_1, &cond_2, &cond_3, &cond_4, P->Q_n*sizeof(bool))) goto cleanup;*/
    /*if (!setup_counts(&count_q_e, &count_e, &count_partial_q_e, P->Q_n*sizeof(size_t))) goto cleanup;*/
    /*if (!setup_abcd(&a, &b, &c, &d, P->Q_n, sizeof(double))) goto cleanup;*/
  /*}*/

  /*if (!init_total_choice(&theta, total_choice_n, P->AD, P->AD_n)) goto cleanup;*/
  /*if (!translate_choices(&C, P, NULL)) goto cleanup;*/
  ...
  /*if (psem == MAXENT_SEMANTICS) {*/
      ...
      /*if (M) {*/
        /*for (size_t i = 0; i < Q_n; ++i) {*/
          ...
          /*for (size_t j = 0; j < q->E_n; ++j) {*/
            /*if (!model_contains(M, q, j, &c, MODEL_CONTAINS_EVI, P->sem)) goto model_cleanup;*/
            /*if (!c) { all_e = false; break; }*/
          /*}*/
          /*if (!all_e) continue;*/
          /*for (size_t j = 0; j < q->Q_n; ++j) {*/
            /*if (!model_contains(M, q, j, &c, MODEL_CONTAINS_QUERY, P->sem)) goto model_cleanup;*/
            /*if (!c) { all_q = false; break; }*/
          /*}*/
          /*double p = prob_from_model(M, P);*/
          /*if (p < 0) goto model_cleanup;*/
          /*b[i] += p;*/
          /*++count_e[i];*/
          /*if (all_q) ++count_q_e[i];*/
        /*}*/
    ...
    /*model_ok = true;*/
/*model_cleanup:*/
    /*if (!(clingo_solve_handle_close(handle) && model_ok)) goto cleanup;*/
