...
def get_woe_iv(data: DataFrame,
    ...
    if language == 'cn':
        result.rename(
            columns={'Name': '名称', 'No.': '序号', 'Bin': '分组', '#Total': '#合计', '#Bad': '#坏', '#Good': '#好',
                     '%Total': '%合计', '%Bad': '%坏', '%Good': '%好', '%BadRate': '%坏件率', 'Total IV': 'IV合计'},
            inplace=True)
        result = result[
            ['名称', '序号', '分组', '#合计', '#坏', '#好', '%合计', '%坏', '%好', '%坏件率', 'WoE', 'IV', 'IV合计']]
    else:
        result = result[
...
def view_woe_iv(dataframe, col, target='y', criterion='tree', bins=[], qcut=0, missing=[], precision=2, language='en'):
    """
    ...
    Returns:
        结果数据集保存各分组名称和分组对应的WoE和IV值
    """
    result = get_woe_iv(dataframe, col, target, criterion, bins, qcut, missing, precision, language)
    result['WoE.'] = result['WoE'].replace('', np.nan)
    result['WoE.'] = result['WoE.'].apply(lambda x: float(x))
    result = result.style.bar(subset=['WoE.'], color='#007bff')
    return result
