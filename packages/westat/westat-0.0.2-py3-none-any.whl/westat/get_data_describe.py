...
def get_data_describe(data:DataFrame,
data_dict:DataFrame=pd.DataFrame(),
                      missing: list = [np.nan, None, ''],                      
                      key:list=['#Sum','#Min','#Mean','#Q1','#Median','#Q3','#Max','#Std','Top1','Top2','Top3'],
                      precision:int = 2,
    ...
    for col in df.columns:
        ...
        if len(df[df[col].notnull()]) > 0:
            ...
            if len(col_top_list) >= 3:
                ...
                col_top2 = np.nan
                col_top1 = np.nan
        result.append([col,'',col_dtype,col_role,col_total,col_n,col_n/col_total,col_missing,col_missing/col_total,col_unique,col_unique/col_total,col_sum,
                       col_min,col_mean,col_q1,col_median,col_q3,col_max,col_range,col_mode,col_var,col_std,col_cv,col_sem,col_skew,col_kurt,col_sum_of_squares,col_top1,col_top2,col_top3])
    result=pd.DataFrame(result,columns=['Name','Describe','Type','Role','#Count','#N','%N','#Missing','%Missing','#Unique','%Unique',
                                        '#Sum','#Min','#Mean','#Q1','#Median','#Q3','#Max','#Range','#Mode','#Var','#Std','#Cv','#StdMean','#Skew','#Kurt','#Sum_of_squares','Top1','Top2','Top3'])
    ...
    if language == 'cn':
        col_list = ['Name', 'Describe', 'Type', 'Role', '#Count', '#N', '%N', '#Missing', '%Missing', '#Unique','%Unique'] + key
        result = result[col_list]
        result.rename(columns={'Name':'名称','Describe':'描述','Type':'类型','Role':'角色','#Count':'#数量','#Missing':'#缺失值',
                               '%Missing':'%缺失值','#Unique':'#唯一值','%Unique':'%唯一值','#Sum':'#合计','#Min':'#最小值',
                               '#Mean':'#均值','#Median':'#中位数','#Max':'#最大值','#Mode':'#众数','#Var':'#方差','#Std':'#标准差',
                               '#Skew':'#偏度','#Kurt':'#峰度','#Q1':'下四分位数','Q3':'上四分位数','#StdMean':'#标准误差','#Range':'#极差','#Cv':'#变异系数',
                               '#Sum_of_squares':'#平方和'},
                      inplace=True)
    else:
        col_list = ['Name','Describe','Type','Role','#Count','#N','%N','#Missing','%Missing','#Unique','%Unique'] + key
