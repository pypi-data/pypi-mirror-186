...
class DsIdParser:
    ...
    def parse_as_groups(cls, ds_item_ids: List[str]) -> Dict[Union[int, ObjectId], List[str]]:
        ...
        for ds_item_id in cls.parse_as_list(ds_item_ids):
            ds_repo_id = ds_item_id.ds_repo_id
...
class DataEngineBase(object):
    ...
    def __init__(self,  accessor: Any, proj_id: int, point_names: List[str] = None, date_range: Optional[DateRange] = None):
...
class DataEngineFactory(object):
    ...
    @classmethod
    def get_engine_by_proj_id(cls, accessor: Any, proj_id: int, point_names: List[str] = None, date_range: Optional[DateRange] = None) -> Optional[DataEngineBase]:
        return DataEngineBase(accessor, proj_id, point_names, date_range)

    @classmethod
    def get_data_engines(cls, accessor: Any, point_names: List[str], date_range: Optional[DateRange] = None, proj_id: Optional[int] = None) -> Dict:
        results = {}
        if len(point_names) > 0:
            for proj_id, his_point_names in DsIdParser.parse_as_groups(point_names).items():
                if isinstance(proj_id, int) and proj_id not in results.keys():
...
class DataStorage(object):
    ...
    @classmethod
    def load_history_data(cls, accessor: Any, date_range: DateRange, point_names: List[str], project_id: Optional[int] = None, need_dict: bool = True):
        ...
        for proj_id, engine in DataEngineFactory.get_data_engines(accessor, list(ds_item_dict.values()), date_range, project_id).items():
        ...
        data_df.rename(columns=ds_item_dict, inplace=True)
    ...
    @classmethod
    def save_data(cls, accessors: List, data: List[List], proj_id: int = 0):
    ...
    @classmethod
    def save_log(cls, accessors: List, logs: list, proj_id: int = 0):
        for accessor in accessors:
            DataEngineFactory.get_engine_by_proj_id(accessor=accessor, proj_id=proj_id).write_log(logs)

    @classmethod
    def read_log(cls, accessor: Any, start: Optional[datetime], end: Optional[datetime], page_index: Optional[int] = None, page_size: Optional[int] = None, sort_order: Optional[str] = None, proj_id: int = 0, need_dict: bool = True, **kwargs):
        engine = DataEngineFactory.get_engine_by_proj_id(accessor=accessor, proj_id=proj_id)
        records = engine.read_record(engine.get_log_tb(), start, end, page_index, page_size, sort_order)
