...
def draw_transparent_image(texture: ImTextureID, rect: imgui.internal.ImRect, alpha: float):
    alpha_color = imgui.get_color_u32(ImVec4(1., 1., 1., alpha))
    imgui.get_foreground_draw_list().add_image_quad(
        texture,
        ImVec2(rect.min.x, rect.min.y), ImVec2(rect.max.x, rect.min.y),
        ImVec2(rect.max.x, rect.max.y), ImVec2(rect.min.x, rect.max.y),
...
def animate_logo(logo_file: str, ratio_width_height: float, em_top_right_margin: ImVec2, final_alpha: float, url:str):
    ...
    rect = ImRect(imgui.internal.im_lerp(rect0.min, rect1.min, k_animation), imgui.internal.im_lerp(rect0.max, rect1.max, k_animation))
    alpha = lerp(alpha0, alpha1, k_animation)

    if k_animation < 1:
        hello_imgui.get_runner_params().fps_idling.enable_idling = False
    if k_animation >= 1. and not static.was_idling_restored:
