    ...
    def __iter__(self):
        ...
        for _ in range(self.num_batches):
            ...
            if len(selected_examples) > 0:
                # Yield the indices of the selected examples (convert to LongTensor)
                print(f"Rank: {self.rank}, batch_size: {len(selected_examples)}, selected_examples: {selected_examples}")
                print(indices[selected_examples])
                yield indices[selected_examples].long()
