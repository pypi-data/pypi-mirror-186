...
from arize.utils.types import Metrics, ModelTypes, Environments
...
class InvalidModelTypeAndMetricsCombination(ValidationError):
    ...
    def __init__(self, model_type: ModelTypes, metrics: List[Metrics]) -> None:
        self.model_type = model_type
        self.metrics = metrics

    def error_message(self) -> str:
        # hannah todo: this docs link is out of date. Update once we are closer to release
        return (
            f"Invalid combination of model type {self.model_type.name} and metrics: "
            f"{', '.join(m.name for m in self.metrics)}."
            "See supported metrics for each model type: "
            "https://docs.arize.com/arize/concepts-and-terminology/model-types"
        )
