...
from arize.utils.types import ModelTypes, Metrics, Environments, Schema
...
class Validator:
    ...
    def _check_model_type_and_metrics(
        ...
        valid_combination, missing_columns, suggested_model_metric_combinations = \
            Validator._check_model_mapping_combinations(model_type, metric_families, schema, required_columns_map)
        if not valid_combination:
            # Model type + metrics combination is not valid.
            return [err.InvalidModelTypeAndMetricsCombination(model_type, metric_families,
                                                              suggested_model_metric_combinations)]
        if missing_columns:
            # For this model type, the schema is missing columns required for the requested metrics.
            return [err.MissingRequiredColumnsMetricsValidation(model_type, metric_families, missing_columns)]
        return []
    ...
    def _check_model_mapping_combinations(
            ...
            if model_type.name.lower() == item.get('external_model_type'):
                ...
                for mapping in item.get('mappings'):
                    ...
                    for metrics_list in mapping.get('metrics'):
                        ...
                        if set(metrics_list) == set(metric_family.name.lower() for metric_family in metric_families):
                            ...
                            if (schema.prediction_label_column_name is not None) or \
                                ...
                                required_columns = mapping.get('required_columns').get('arrow').get('required')
                                for column_combination in required_columns:
                                    missing_columns = []
                                    if None in {getattr(schema, column, None) for column in column_combination}:
        ...
        return True, missing_columns, None
    ...
    def _check_existence_pred_act_shap_score_or_label(
        ...
        if (
                (schema.prediction_label_column_name is not None or schema.prediction_score_column_name is not None)
                or (schema.actual_label_column_name is not None or schema.actual_score_column_name is not None)
                or schema.shap_values_column_names is not None
        ):
            return []
    ...
    def _check_existence_preprod_pred_act_score_or_label(
        ...
        if environment in (Environments.VALIDATION, Environments.TRAINING) and (
                (schema.prediction_label_column_name is None and schema.prediction_score_column_name is None)
