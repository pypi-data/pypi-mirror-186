...
@workspace.command(name='data_branch', short_help="Copy last partition for each Data Source in the main workspace passed as argument into the current workspace Data Sources", hidden=True)
@click.pass_context
@coro
async def data_branch(ctx):
    client = ctx.obj['client']
    config = ctx.ensure_object(dict)['config']

    try:
        await client.branch_workspace_data(config['id'])
