...
SYMAI_VERSION = "0.1.26"


def _start_symai():
    global _ai_config_
    # check if symai is already initialized
    _ai_config_path_ = os.path.join(os.getcwd(), 'ai.config.json')
    if not os.path.exists(_ai_config_path_):
        _parent_dir_ = os.path.dirname(__file__)
        _ai_config_path_ = os.path.join(_parent_dir_, 'ai.config.json')
    
    # read the ai.config.json file
    ...
    # set or update the ai.config.json file
