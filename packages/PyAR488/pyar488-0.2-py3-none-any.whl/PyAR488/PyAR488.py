...
class AR488:
    ...
    _mode = ['device', 'controller']
    _eor = ['\r\n', '\r', '\n', None, '\n\r', '\x03', '\r\n\x03', 'EOI']
    _eos = ['\r\n', '\r', '\n', None]

    def __init__(self, port="COM0", baud=115200, timeout=1, debug=False):
        try:
    ...
    def update_config(self):
        self.address = int(self.prologix_address())  #
        self.mode = self.prologix_mode()  #
        self.eoi = self.prologix_eoi()  #
        self.eos = self.prologix_eos()  #
        self.eot = self.prologix_eot()  #
        self.eot_char = self.prologix_eot_char()  #
        self.eor = self.prologix_eor()  #
        self.idn = self.prologix_idn()  #
    ...
    def query(self, message, payload=False, decode=True):
        """Write message to GPIB bus and read results, if a payload is expected send '++read' too,
        decode by default un UTF-8"""
        self.write(message)
        if payload:
            return self.prologix_read(decode=decode)
        return self.read(decode=decode)

    # Prologix commands
    def prologix_address(self, new_address: int = None, force: bool = False):
        ...
        if new_address is None:
            ...
            if self.address != new_address or force:
                if 1 <= new_address <= 30:
                    self.write(f'++addr {new_address}')
                else:
    ...
    def prologix_eoi(self, new_eoi: bool = None):
        ...
        if new_eoi is None:
            ...
            self.write(f'++eoi {"1" if new_eoi else "0"}')
    ...
    def prologix_eos(self, new_eos: str = None):
        ...
        if new_eos is None:
            return self._eos[int(self.query('++eos'))]
        else:
            if new_eos in self._eos:
                self.write(f'++eos {self._eos.index(new_eos)}')
            else:
    ...
    def prologix_eot(self, new_eot: bool = None):
        ...
        if new_eot is None:
            ...
            self.write(f'++eot_enable {"1" if new_eot else "0"}')
    ...
    def prologix_eot_char(self, new_eot_char: str = None):
        ...
        if new_eot_char is None:
            ...
            if 0 <= new_eot_char <= 256:
                self.write(f'++eot_char {new_eot_char}')  # send as char
            else:
                raise Exception('invalid eot_char')

            # check
            self.eot_char = self.prologix_eot_char()
            if self.eot_char == new_eot_char:
                raise Exception('unable to set new eos char sequence')

    # todo : ++help, ++ifc, ++llo, ++loc, ++lon,

    def prologix_mode(self, new_mode=None):
        ...
        if new_mode is None:
            return self._mode[int(self.query('++mode'))]
        else:
            if type(new_mode) == str and new_mode not in self._mode:
                raise Exception('invalid interface operation mode')
            else:
                new_mode = self._mode.index(new_mode)  # get mode index

            if type(new_mode) == int and new_mode in range(len(self._mode)):
                self.write(f'++mode {self._mode[new_mode]}')
            else:
                raise Exception('invalid mode type')

            # check
            self.mode = self.prologix_mode()  # string
            if self.mode != new_mode:
                raise Exception('unable to set correct operation mode')

    def prologix_read(self, terminator: str = None, decode=None, until_eoi=False):
    ...
    def prologix_spoll(self, targets=None):
    ...
    def prologix_send_trigger(self, target):
        ...
        if type(target) == int and 1 <= target <= 30:
            ...
            self.write('trg' + target_string)
        else:
            raise Exception('invalid type of trigger target')

    def prologix_ver(self):
    ...
    def prologix_eor(self, new_eor: str = None):
        ...
        if new_eor is None:
            return self._eor[int(self.query('++eor'))]
        else:
            if new_eor in self._eor:
                self.write(f'++eor {self._eor.index(new_eor)}')
            else:
                raise Exception('invalid eor char sequence')

            # check
            current_eor = self.prologix_eor()
            if current_eor != new_eor:
                raise Exception('unable to set new eor char sequence')

    # todo : ++id, ++id fwver, ++id serial, ++id verstr

    def prologix_idn(self, new_idn=None):
