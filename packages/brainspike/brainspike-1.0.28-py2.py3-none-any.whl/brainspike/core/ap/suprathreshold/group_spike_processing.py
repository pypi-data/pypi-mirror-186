...
class SpikeGroupFeatures(SpikeFeatures): 
    ...
    def __init__(self, data = None, dv_cutoff=5., max_interval=0.02, min_height = 2., min_peak = 0.,\
                thresh_frac = 0.3, reject_at_stim_start_interval = 0, baseline_interval = 0.05, rms_cutoff = 2,\
    ...
    def group_export_all_spike_features(self, fdir = 'processed/spikes/df_all_spikes_features', file_extension = '.xlsx'): 
        ...
        if self.group_data is not None: 
            ...
            for data in self.group_data: 
                ...
                if fdir is not None: 
                    fname = data.metadata['id'] # default to abf id 
            ...
            for data in self.group_data: 
                ...
                if math.isnan(max_firing_sweep) == False: 
                    if figdir is not None: 
                        figname = data.metadata['id'] + '_sweep_' + str(max_firing_sweep) # default to abf id + sweep \
