...
unsigned takeNodeFromPool(float *center, float * radius, struct OctreePool* nodePool) {
	...
	nodePool->ce_pool[idx] = malloc(sizeof(unsigned)*OT_NODE_CAPACITY);
...
void octreeDefragment(struct OctreePool* nodePool, unsigned *vertexIdxPool) {
	unsigned * childrenRefTMP = malloc(sizeof(unsigned)*nodePool->pool_idx);
	unsigned i;

	for (i=0; i<nodePool->pool_idx; i++)
		childrenRefTMP[i] = i;
	
	struct OctreeNode *swapNode = malloc(sizeof(struct OctreeNode));
...
unsigned ROIsSegmentationPopulateAndDefragmentPool(void * pyOCPool,
	...
	nodePool.ce_pool = malloc(sizeof(unsigned*)*octreePoolSize);
...
void querySphere(struct ROI * sphere, int nodeIdx, struct OctreeNode * nodePool, unsigned * vertex2Fiber, unsigned * vertexIdxPool, float * points, bool * result) {
	...
	if (SphereContainsBoundary(sphere, node)) {
		...
		for(unsigned k=0; k<node->numberOfElements; k++)
			result[vertex2Fiber[vertexIdxPool[node->contentElements+k]]] = true;
		return;
	}

	if (node->numberOfElements <= OT_NODE_CAPACITY)
		for (unsigned i=0, vertex; i<node->numberOfElements; i++) {
...
void queryTree(int nROI,
	...
	// unsigned * result = (unsigned *) malloc(sizeof(unsigned)*nodePool[0].numberOfElements);
	// unsigned resultLen;
...
void ROISegmentationExportbundlesdata(char * filePath,
	...
	for (int i=0; i<bundleN; i++) {
		for (int j=bundleStart[i]; j<bundleStart[i+1]; j++) {
			if (fiberValidator[j]) {
				bundleCount[i]++;
