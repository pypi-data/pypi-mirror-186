...
bool discard_center(float *subject_data, float *atlas_data, unsigned short int ndata_fiber,
...
bool discard_extremes(float *subject_data, float *atlas_data, unsigned short int ndata_fiber,
...
//
//
// MODIFICAR EVALUACION DE IS_INVERTED PARA QUE MODIFIQUE EL VECTOR POINTS AL PRINCIPIO EN VEZ DE ESTAR EVALUANDO CADA FOR
//
//
//
//
bool discard_four_points(float *subject_data, float *atlas_data, unsigned short int ndata_fiber, unsigned char threshold, 
						bool is_inverted, unsigned int fiber_index, unsigned int fatlas_index){

	unsigned short int *points = {3,7,13,17};
...
//
//
// Se podria calcular fiber_pos y atlas_pos antes del for, para evitar 3 multiplicaciones por ciclo
//
//
//
...
/*
float *atlas_data				vector with all the 3d points for the atlas
unsigned int atlas_data_size	size of the vector with all the points for the atlas
float *subject_data 			vector with all the 3d points for the subject
unsigned int subject_data_size 	size of the vector with all the points for the subject
unsigned short int ndata_fiber	number of points per fiber (*3) so 21 points = 63
unsigned char *thresholds		vector with the thresholds for each fascicle on the atlas
unsigned int *bundle_of_fiber	vector of atlas_points_size with id for the fascicle that correspondence
unsigned char *assignment 		size nfibers_subject. And all data set to 254 - result vector
*/
void parallel_segmentation(float *atlas_data, unsigned int atlas_data_size, float *subject_data, unsigned int subject_data_size, 
						unsigned short int ndata_fiber,	unsigned char *thresholds, unsigned int *bundle_of_fiber, unsigned char *assignment){

	unsigned int nfibers_subject = subject_data_size/ndata_fiber;
	unsigned int nfibers_atlas = atlas_data_size/ndata_fiber;
