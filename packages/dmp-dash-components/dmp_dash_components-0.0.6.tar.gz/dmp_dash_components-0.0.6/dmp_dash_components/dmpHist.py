...
class dmpHist(Figure):
    def __init__(
        ...
        if groupby is None:
            ...
            groups = sort_values(data[groupby].unique().tolist(), reverse=True)
            group_labels = [sort_trace_name(x) for x in groups]
            hist_data = [data[data[groupby] == x][value].tolist()
                         for x in groups]
            box = histogram(data, x=value, marginal='box', color=groupby,
                            color_discrete_sequence=colorway)
        distFig = create_distplot(hist_data, group_labels, colors=colorway, show_rug=False,
        ...
        self.update_layout(legend_traceorder='reversed', yaxis_ticksuffix=' ')
