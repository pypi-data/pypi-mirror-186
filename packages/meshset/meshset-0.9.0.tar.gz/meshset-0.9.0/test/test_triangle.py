...
def test_plot2d():
    bary, cells = meshset.triangle(4)
    meshset.show2d(_get_points(bary), cells)


def test_edges():
    _, cells = meshset.triangle(2)
    edges_nodes, edges_cells = meshset.create_edges(cells)
    assert np.all(
        edges_nodes
...
if __name__ == "__main__":
    points, cells = meshset.triangle(5000)
