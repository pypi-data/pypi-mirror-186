...
class Client(AClient):
    def __init__(
        self, service_host: str, secret: str = os.environ.get("ALASTRIA_SERVICE_SECRET", "")
    ):
    ...
    def signature(self, body: SignatureValidator) -> GenericStrResponse:
        headers: Dict[str, Any] = {}
        r: Any = requests.post(
            f"{self.host}/signature", json=json.loads(body.json()), headers=headers, timeout=5
        )
        return GenericStrResponse(**r.json())
    ...
    def delegate_call(self, body: DelegateCallValidator) -> GenericDictResponse:
        headers: Dict[str, Any] = {}
        r: Any = requests.post(
            f"{self.host}/delegate-call", json=json.loads(body.json()), headers=headers, timeout=5
        )
        return GenericDictResponse(**r.json())
    ...
    def add_key(self, body: AddKeyValidator) -> GenericStrResponse:
        headers: Dict[str, Any] = {}
        r: Any = requests.post(
            f"{self.host}/add-key", json=json.loads(body.json()), headers=headers, timeout=5
        )
        return GenericStrResponse(**r.json())
    ...
    def add_identity_issuer(self, body: AddIdentityIssuerValidator) -> GenericStrResponse:
        headers: Dict[str, Any] = {}
        r: Any = requests.post(
            f"{self.host}/add-identity-issuer",
            json=json.loads(body.json()),
            headers=headers,
            timeout=5,
        )
