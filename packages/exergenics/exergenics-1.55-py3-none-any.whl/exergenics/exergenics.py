...
class ExergenicsApi:
    ...
    def doRequest(self, endpoint, activity, body):
        ...
        request = requests.post(self.getActivityUrl(
            endpoint, activity), data=body, headers=self.getHeaders())
        self.resetCounter()
    ...
    ********************************
    ...
    def setJobStageRejected(self, jobId, rejectedMessage=None):
        ...
        activityUrl = "{}/{}?rejectedMessage={}".format(
            self.ex__JobRejected, jobId, errorejectedMessagerMessage)
        self.lastStatus, self.lastResponse = self.doRequest(
            self.getExchangeEndpoint(), activityUrl, {})
        if self.noResponse():
            return False
    ...
    def setJobData(self, jobId, fieldName, fieldValue):
        ...
        self.lastStatus, self.lastResponse = self.doRequest(
            self.getExchangeEndpoint(), activityUrl, json.dumps({'fieldName': fieldName, 'fieldValue': fieldValue}))
        if self.noResponse():
            return False
        return True
