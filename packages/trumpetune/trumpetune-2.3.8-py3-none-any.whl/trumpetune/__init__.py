...
if not RECIPE or not MODE:
    ...
    QUEUE_DIR = os.path.join(os.path.abspath(args.recipe), "trumpetune_dbs")
...
if not QUEUE_DIR:
    QUEUE_DIR = os.path.join(RECIPE, "trumpetune_dbs")
...
def build(mode="build_rest"):
    ...
    if mode == "build_rest":
        dockerfile_lines.append(
            f'CMD ["ulimit -n 1000000 && python3 -m trumpetune --recipe {recipe_base_name} --mode loop & python3 -m trumpetune --recipe {recipe_base_name} --mode rest"] \n'
        )
    ...
    docker_image_name = f"trumpetune_{os.path.basename(os.path.abspath(RECIPE))}".strip(
        "/"
    ).lower()
