...
def configure_logger(log_sink: str, log_level: str) -> None:
    ...
    logger.add(
        log_sink,
        format=(
            "{time:YYYY-MM-DD HH:mm:ss.SSS!UTC}Z {thread:12} {level:8} " "{module:15} {line:3} {function:25} {message}"
        ),
        level=log_level,
        enqueue=True,
    )
...
def handle_api_response(response: httpx.Response) -> httpx.Response:
    ...
    if s == HTTPStatus.BAD_REQUEST:
        ...
        error_msg = (
            "server error occurred, please ensure your request is correct " "and/or notify Saxo if this error persists"
        )
    ...
    logger.success(
        "successfully performed API request with X-Request-Id: " f"{response.request.headers['x-request-id']}"
    )
...
def decode_streaming_message(message: bytes) -> StreamingMessage:
    ...
    payload_size = int.from_bytes(message[12 + ref_id_len : 16 + ref_id_len], byteorder="little")  # noqa
