...
class SaxoOpenAPIClient:
    ...
    def __init__(
        ...
        logger.debug(f"initializing OpenAPI Client with session id: {self.client_session_id} " f"{version_info()}")
    ...
    def login(
        ...
        logger.debug(
            ...
            f"{catch_redirect=} {start_async_refresh=} {redirect_port=}"
        )
    ...
    def refresh(self) -> None:
        ...
        if self.streaming_connection:
            logger.debug("found streaming connection with context_id: " f"{self.streaming_context_id} - re-authorizing")
            self.put(
    ...
    def get_tokens(self, auth_code: Union[AuthorizationCode, None] = None) -> None:
        ...
        auth = b64encode((f"{self._app_config.client_id}:{self._app_config.client_secret}").encode("utf-8")).decode(
            "utf-8"
        )
        ...
        if response.status_code != 201:
            raise RuntimeError(
                "unexpected error occurred while retrieving token - response status: " f"{response.status_code}"
            )
    ...
    def setup_streaming_connection(self) -> None:
        ...
        url = f"{self._app_config.streaming_url}/connect?contextId=" f"{self.streaming_context_id}"
        headers = {"Authorization": "Bearer " f"{self._token_data.access_token}"}  # type: ignore[union-attr]
    ...
    def openapi_request(
        ...
        headers = {
            "x-request-id": (f"saxo-apy/{self.client_session_id}/{token_urlsafe(ID_NUM_BYTES)}"),
            "authorization": (
