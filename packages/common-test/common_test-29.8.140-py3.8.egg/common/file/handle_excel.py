...
def excel_to_list(data_file, sheet,_index:int=1,_suitName:str="", _replace: bool=True):
    """
    ...
    :return:datalist
    """
    if DataProcess.isNotNull(_suitName):
        set_system_key(Constant.TEST_SUIT_NAME_LIST, _suitName)
    ...
    for row in range(_index, sh.nrows):
        ...
        # 列表嵌套字典格式，每个元素是一个字典
        if check_excel_data(d, _mysql):
            if _replace:
                data_list.append(DataBus.get_data(d))
            else:
                data_list.append(d)
    _mysql.close()
    print_info(f'参数化数据: {data_list}')
    return data_list

def check_excel_data(testdata:dict, _mysql):
    casename = format_caseName(testdata[Constant.CASE_TITLE])
    if DataProcess.isNotNull(casename) == False:
        return False
    if DataProcess.isNotNull(get_system_key(Constant.ISSUE_KEY)) and DataProcess.isNotNull(
        ...
        if len(_info) > 0:
            logger.info(f'执行参数化用例:{casename} 执行周期：{cycleId} 测试计划:{jirakey}')
            return True
        else:
            logger.warning(f'参数化用例:{casename} 执行周期:{cycleId} 测试计划:{jirakey} 未发现')
            return False
