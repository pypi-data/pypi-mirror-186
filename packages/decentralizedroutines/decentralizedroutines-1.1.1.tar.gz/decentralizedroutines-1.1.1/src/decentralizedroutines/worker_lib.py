...
def run_routine(command,routine):
    ...
    if install_repo(command):
        ...
        if 'args' in command:
            cmd = [str(repo_path/'venv\\Scripts\\python.exe'),\
                str(repo_path/command['routine']),command['args']]
        else:
...
def run_logger(command):
    ...
    env['GIT_TERMINAL_PROMPT'] = "0"
    cmd = [str(repo_path/'venv\\Scripts\\python.exe'),\
        '-m','SharedData.LoggerConsumerProcess','worker']                            
    routine['process'] = subprocess.Popen(cmd,env=env,shell=False,stdout=DEVNULL)
    return routine
...
def run_scheduler(command):
    ...
    env['GIT_TERMINAL_PROMPT'] = "0"
    cmd = [str(repo_path/'venv\\Scripts\\python.exe'),\
        '-m','decentralizedroutines.runroutines',command['args']]
    routine['process'] = subprocess.Popen(cmd,env=env)
    return routine
