...
import timxfriend.plot as plot
import timxfriend.util as util
import timxfriend.svd as svd
...
class CA(base.BaseEstimator, base.TransformerMixin):
    ...
    def kfinspect(self,X):

            #计算期望值
            Sum_c = X.sum(axis = 0)
            Sum_r = X.sum(axis = 1)
            ka_data = Sum_c.values.reshape(-1,1).dot(Sum_r.values.reshape(1,-1))
            ka_data = pd.DataFrame((ka_data.T)/(data.sum().sum()),columns = data.columns,index = data.index)
            #计算卡方值
            ka = ((data - ka_data) * (data - ka_data))/ka_data

            #卡方阈值
            p=len(data.columns)-1
            q=len(data)-1
            #计算临界值(critical value),大于它的部分是拒绝域
            return '卡方值：{},卡方阈值:{}'.format(ka.sum().sum(),chi2.ppf(0.05, (p-1)*(q-1)))
    ...
    def plot_coordinates(self, X, ax=None, figsize=(6, 6), x_component=0, y_component=1,
                                   show_row_labels=True, show_col_labels=True, s = 10,dpi = 400,**kwargs):
