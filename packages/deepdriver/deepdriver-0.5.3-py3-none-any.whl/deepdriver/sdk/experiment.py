...
def init(exp_name: str = "", team_name: str = "", run_name: str = "", config: Dict = None) -> Run:
    ...
    if exp_name:
        ...
        if pattern.findall(exp_name):
            logger.info("init() failed : exp_name은 숫자(number), 영문자(alphabet), 언더바(_), 온점(.)만 가능합니다.")
            return None

        if len(exp_name) >= 50:
            logger.info("init() failed :  exp_name의 최대 길이는 50자 미만입니다. [max length 50]")
            return None
...
def prepare_dataset(dataset: dict = None) -> Tuple[bool, str]:
    ...
    arti = deepdriver.get_artifact(**dataset)
    if arti.entry_list == []:
...
def prepare_code(code: dict = None) -> Tuple[bool, str, Callable]:
    ...
    arti = deepdriver.get_artifact(name=code['name'], type=code['type'])
    if arti.entry_list == []:
        ...
        looking_for_body = next((body for body in node.body if body.name == code['fun']), None)
