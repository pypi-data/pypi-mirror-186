...
def import_file(location: Union[str, Sequence[str]], **params: Any) -> Pipeline:
    ...
    for key in params:        
        if not hasattr(importer_object, key):
            raise RuntimeError(f"Invalid keyword parameter. File reader object {importer_object!r} doesn't have a parameter named '{key}'.")
        importer_object.__setattr__(key, params[key])
