...
class IdentifyFCCPlanarFaultsModifier(ModifierInterface):
    ...
    color_nonhcp = ColorTrait(label='Non-hcp structure (0)', default=(1.0,1.0,1.0))

    # Define modifier parameters controlling the colors of different hcp-like atoms:
    color_other = ColorTrait(label='Indeterminate hcp-like (1)', default=(1.0,0.4,0.4))
    """
    ...
    color_isf = ColorTrait(label='Intrinsic stacking fault (2)', default=(0.9,0.7,0.2))
    """
    ...
    color_twin = ColorTrait(label='Coherent twin boundary (3)', default=(0.4,0.4,1.0))
    """
    ...
    color_multi = ColorTrait(label='Multilayer stacking fault (4)', default=(0.0,1.0,0.9))
    """
    ...
    def modify(self, data: DataCollection, **kwargs):
        ...
        # Precompute the neighbor lists of all HCP atoms and store them in a (N,12) array.
        ...
        for i, aindex in enumerate(hcp_indices):
            ...
            yield i / num_hcp
            # Visit each of the 12 neighbors of the current HCP atom in order.
            for j, neigh in enumerate(neigh_finder.find(aindex)):
                ...
                if structure_types[neigh.index] == PolyhedralTemplateMatchingModifier.Type.HCP:
                    ...
                    hcp_neighbors[i,j] = -neigh.index - 1 # Encode non-HCP neighbors as negative values
