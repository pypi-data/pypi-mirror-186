...
class AnalyticSimulation(tf.Module):
    ...
    def __call__(self, dataset: 'AnalyticDataset', voxel_batch_size: int = 1000,
                 execute_eager: Optional[bool] = False,
                 unstack_repetitions: bool = True) -> tf.Tensor:
        ...
        :param execute_eager: (bool) Default False: if True, Graph construction is skipped for
        ...
        """
        self._update()
        self._validate_dataset(dataset.map_names)
        ...
        if not execute_eager:
            @tf.function
            def simulation_execute(dataset, ks):
                return self._simulation_loop(dataset, ks)
        else:
            simulation_execute = self._simulation_loop

        # Run actual simulation
        k_space_shape = self.get_k_space_shape()
        batched_dataset = dataset(batchsize=voxel_batch_size)
        noise_less_k_space_stack = simulation_execute(batched_dataset, k_space_shape)
    ...
    def _validate_dataset(self, map_names):
