...
class CompressingTiler3D(Tiler):
    '''
    Processes layers into the compressed tile/block/cell stack/timeseries format
            ...
            with cleanup():
                ...
                for stack in stacks:
                    ...
                    for i, result in enumerate(results):
                        ...
                        rasters.append(processed_layer.path)
                        if i == 0:
                    ...
                    for tile in first_stack_layer.tiles(self._tile_extent, self._block_extent):
                        tile_out_path = os.path.join(
                            output_folder,
                            "{}_{}.zip".format(processed_layer_name, tile.name))

                        pool.apply_async(_write_tile, (os.path.abspath(tile_out_path), tile, rasters))
...
def _write_tile(out_path, tile, raster_paths):
    ...
    rasters = [gdal.Open(path, GA_ReadOnly) for path in raster_paths]
