...
class Tiler2D(Tiler):
    '''
    Processes layers into the uncompressed tile/block/cell format supported by the
    Flint platform.
    ...
    def __init__(self, bounding_box, tile_extent=1.0, block_extent=0.1, use_bounding_box_resolution=False):
                    ...
                    try:
                        with open(out_path, "wb", buffering=buffer_memory_limit) as blocked_file:
                            for block in tile.blocks:
                                data = band.ReadAsArray(block.x_offset, block.y_offset, block.x_size, block.y_size)
