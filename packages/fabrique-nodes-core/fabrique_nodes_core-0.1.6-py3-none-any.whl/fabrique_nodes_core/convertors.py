...
def convert_node_data_to_config(node_data: NodeData, id_: int = None, ConfigModelClass: BaseModel = None) -> NodeConfig:
    ...
    config_from_ui = node_data_dict.get('config')
    cfg = NodeConfig(**node_data_dict)
    if ConfigModelClass and config_from_ui:
        cfg.config = ConfigModelClass(**config_from_ui)
