...
def writeSongstoFile(trs, fileType):
    dataHeaders = ["Title/Composer","Performer","Duration","Stream on"]
    with open(artist+'.'+fileType, 'a') as f:
        x = 0
        for tr in trs:
            finalComma = ",\n"
            if((x+1)==len(trs)):
...
def finishingTouch(fileType):
    with open(artist+'.'+fileType, 'r') as f:
        allData = f.read()
    with open(artist+'.'+fileType, 'w') as w:
        w.write("[\n"+str(allData)+"\n]")
...
def getDiscography(link, fileType):
    ...
    for tr in trs:
        ...
        writeSongstoFile(newTrs, fileType)
        if((x+1)!=trsLength):
            with open(artist+'.'+fileType, 'a') as f:
                f.write(',\n')
        x+=1
