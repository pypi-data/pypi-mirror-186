...
class Config(configparser.ConfigParser):
    ...
    def make_config_dict(cls):
        """
            generate config file for user 
            and load global variable for use
        """
        config_path = Path.home() / f'{cls.CONFIG_NAME}'

        if not config_path.exists():
            for k,v in cls.AUTOJUMP_CONFIG.items():
                v = cls.dump_global_config(v)
                cls.seta(cls.AUTOJUMP_SECTION, k, v) 
            cls.write( str(config_path) )
        ...
        add_name = namedtuple(
            "GLOBAL_CONFIG",
            cls.AUTOJUMP_CONFIG.keys()  # add keys
        )
        # cls.AUTOJUMP_CONFIG = { k:eval(v) for k,v in cls.AUTOJUMP_CONFIG.items() }
        
        _global_config = cls[cls.AUTOJUMP_SECTION].values()
        _global_config = [*map(cls.load_global_config, _global_config)]

        cls.GLOBAL_CONFIG = add_name._make(
            _global_config
        )  # add value
