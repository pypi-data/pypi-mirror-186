...
from blazetest.core.config import BUILD_FOLDER_PATH, PWD, EXECUTABLE_FILES, CWD

logger = logging.getLogger(__name__)


def setup_logging(debug=False, stdout_enabled=True):
...
def create_build_folder() -> None:
    ...
    # Copy the `__main__.py` file
    shutil.copyfile(
        src=os.path.join(PWD, "deployment/aws/__main__.py"),
        dst=os.path.join(BUILD_FOLDER_PATH, "__main__.py"),
    )

    # Copy the `.envignore` file
    shutil.copyfile(
        src=os.path.join(PWD, "deployment/aws/.envignore"),
        dst=os.path.join(BUILD_FOLDER_PATH, ".envignore"),
    )
