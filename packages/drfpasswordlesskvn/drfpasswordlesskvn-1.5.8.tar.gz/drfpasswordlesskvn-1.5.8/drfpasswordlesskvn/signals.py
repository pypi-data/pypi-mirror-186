...
from drfpasswordlesskvn.models import generate_numeric_token
                ...
                try:
                    ...
                    if instance_email != old_email and instance_email != "" and instance_email is not None:
                        ...
                        if api_settings.PASSWORDLESS_AUTO_SEND_VERIFICATION_TOKEN is True:
                            ...
                            if success:
                                logger.info('drfpasswordlesskvn: Successfully sent email on updated address: %s'
                                            % instance_email)
                            else:
                                logger.info('drfpasswordlesskvn: Failed to send email to updated address: %s'
                                            % instance_email)
                ...
                try:
                    ...
                    if instance_mobile != old_mobile and instance_mobile != "" and instance_mobile is not None:
                        ...
                        if api_settings.PASSWORDLESS_AUTO_SEND_VERIFICATION_TOKEN is True:
                            ...
                            if success:
                                logger.info('drfpasswordlesskvn: Successfully sent SMS on updated mobile: %s'
                                            % instance_mobile)
                            else:
                                logger.info('drfpasswordlesskvn: Failed to send SMS to updated mobile: %s'
                                            % instance_mobile)
