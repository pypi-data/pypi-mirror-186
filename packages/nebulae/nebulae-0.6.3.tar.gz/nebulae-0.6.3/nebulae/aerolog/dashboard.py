...
class DashBoard(object):
    ...
    def log(self, gauge=True, tachograph=True, history=''):
        ...
        if gauge:
            ...
            for k in self.gauge_mile.keys():
                stage, entry = k.split(':')
                if stage not in boards.keys():
                    boards[stage] = []
                if self.format[entry][1] not in ('inviz', 'tailor'):
                    boards[stage].append(k)
            for stage in boards.keys():
                fig = plt.figure()
                ax = fig.add_subplot(111)
                for i, b in enumerate(boards[stage]):
                    ymin, ymax, xmin, xmax = self._argm(self.gauge_epoch[b])
                    xoff = len(self.trail_epoch[stage]) * 0.02
                    yoff = (ymax - ymin) * 0.02
                    ax.annotate(('%%%s' % (self.format[entry][0])) % ymin, (xmin - xoff, ymin + yoff))
                    ax.annotate(('%%%s' % (self.format[entry][0])) % ymax, (xmax - xoff, ymax + yoff))
                data = pd.DataFrame({b: np.array(self.gauge_epoch[b]) for b in boards[stage]}, index=self.trail_epoch[stage])
                sns.lineplot(data=data, markers=True, ax=ax)
                if len(boards[stage]) > 0 and self.max_epoch > 0:
                    plt.savefig(os.path.join(self.log_path, '%s_epoch_%d.jpg' % (stage.replace('/', '-'), self.max_epoch)))
                    plt.close()
        if tachograph:
            for k in self.gauge_mile.keys():
