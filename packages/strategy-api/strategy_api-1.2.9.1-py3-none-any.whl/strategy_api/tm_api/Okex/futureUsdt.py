...
class OkexRestApi(RestClient):
    ...
    def on_send_order(self, data: dict, request: Request) -> None:
        if data['code'] != "0":
            if request.extra:
                ...
                order.reference = data['data'][0]['sMsg']  # 拒绝原因
    ...
    def on_send_order_failed(self, status_code: int, request: Request) -> None:
        order: OrderData = copy(request.extra['order'])
        order.status = Status.REJECTED  # 被拒绝
        order.reference = f"code: {status_code}, msg: {request.response.json().get('msg')}"  # 拒绝原因
        self.gateway.on_order(order)
    ...
    def on_send_order_error(
        ...
        order: OrderData = copy(request.extra['order'])
        order.status = Status.REJECTED  # 被拒绝
        order.reference = repr(exception_value)  # 失败原因
        self.gateway.on_order(order)
