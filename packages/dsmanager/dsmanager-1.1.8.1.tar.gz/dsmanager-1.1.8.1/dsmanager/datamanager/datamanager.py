...
try:
    ...
    print(f"'SharepointSource' is not available because '{m_error.name}' is missing.")
try:
    from dsmanager.datamanager.datasources.sfsource import SFSource

    SOURCES["salesforce"] = SFSource
except ModuleNotFoundError as m_error:
    print(f"'SFSource' is not available because '{m_error.name}' is missing.")
try:
    from dsmanager.datamanager.datasources.kagglesource import KaggleSource

    SOURCES["kaggle"] = KaggleSource
except ModuleNotFoundError as m_error:
    print(f"'KaggleSource' is not available because '{m_error.name}' is missing.")
...
class DataManager:
        ...
        def __call__(
            ...
            **kwargs: Any,
            ...
            if not reload and name in self:
                ...
                data = source(self.__dm.logger).read(source_info, **kwargs)
            if save:
                self[name] = data
            ...
            **kwargs: Any,
            ...
            if not reload and name in self:
                ...
                if formatting:
                    format_dict(source_info, formatting)
                source = self.__dm._get_source_reader(source_info)
                data = source(self.__dm.logger).read_db(source_info, **kwargs)
            if save:
                self[name] = data
    ...
    def __init__(
        ...
        if metafile_path:
            ...
            raise TypeError("No metadata's path given")
    ...
    def preload(
        ...
        for key in metadata.keys():
            try:
                self.get_data(key)
                self.logger.info("Preloaded data %s.", key)
            except Exception as _:  # pylint: disable=broad-except
                try:
                    self.logger.info("Failed to get data so preload session %s.", key)
