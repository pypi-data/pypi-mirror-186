...
class Colors(object):
    ...
    def get_palette(self, palette='normal'):
        ...
        elif palette in self.colormaps.keys():
            cmap = self.colormaps[palette]
        else:
            self._palette_dict_check(palette)
        return cmap
    
    def create_colormap(self, *colors, n_colors=256):       
    ...
    def plot_palette(self, palette, label):
    ...
    def show_palette(self, palette='all'):
        if palette != 'all' and palette not in self.palettes.keys() and palette not in self.palettes_div.keys() and palette not in self.colors.keys():
            raise NameError('Palette does not exist!')
            
        elif palette in self.colors.keys():
            cmap = mcolors.ListedColormap(self.colors[palette])
            self.plot_palette(cmap, label=palette)    
            ...
            for palette in self.colormaps.keys():
                cmap = self.colormaps[palette]
                self.plot_palette(cmap, label=palette) 
        return
