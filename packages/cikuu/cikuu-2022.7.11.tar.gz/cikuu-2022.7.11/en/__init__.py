...
def es_skenp(doc):
	from dic.oneself import oneself 
	merge_np(doc) # transform doc , finally to be called 
	return "_^ " + ' '.join([ f"_{t.lemma_}_NP_{t.dep_}" if t.ent_type_ == 'NP' else f"_{t.pos_}_{t.tag_}_{t.dep_}" if t.pos_ in ('PROPN','NUM','X','SPACE') else f"{t.text if t.text in ('I') else t.text.lower()}_{t.lemma_}_{t.pos_}_{t.tag_}{oneself.get(t.text.lower(), '')}_{t.dep_}"  for t in doc]) # uniq by ana #,'PUNCT'
...
if __name__	== '__main__': 
	doc = spacy.nlp("Some of the Republican policies have left feminists dismayed and aghast.") 
