...
class IncentiveCalculation:
    ...
    def trainemodel(self):
        try:
            ...
            if 'model_name' not in json.loads(jsonData['modelParameter']).keys() or json.loads(jsonData['modelParameter'])["model_name"] == None:
                ...
                modelName = json.loads(jsonData['modelParameter'])['model_name'].lower().replace(" ","_")
            ...
            logger.log(f"modelType::{modelType}","0")
            ...
            if modelType == 'sentiment_analytics':
                ...
                rasa = Rasa()
                result = rasa.create_model(enterprise_key, mode, parsed_json, self.modelScope, modelName, modelType )
    ...
    def predict(self, textColumn, modelName,  modelType, modelScope):
        ...
        if modelType == "sentiment_analytics":
            ...
            rasa = Rasa()
            textColumn = {'text': ['Augmentin 300mg Intravenous','Augmentin 375mg Tablets', 'Calpol 250mg Suspension']}  #predict
            result = rasa.get_prediction(textColumn, enterprise, self.modelScope, modelType, modelName)
            logger.log(f"productidentification predict result :{result}","0")
