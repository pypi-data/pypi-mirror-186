...
def help():
    print('1. emelib.find_by_words("любое количество слов")')
    print('2. emelib.get_task_by_id(выбранный_id)')

    # print('для скачивания маджик файла -- emelib.practise()')
...
Таким образом, на первом шаге метода прямого поиска определяется направление движения к вектору “оптимальных” оценок a параметров модели по каждой координате. Значения приближений $a_i^{(2)}$ на втором шаге уже определяются по одной из двух формул $a_i^{(1)}=a_i^{(0)}±\Delta a_i$, и если $S_2^2(a_i^{(2)})< S_1^2$ , 1.	То движение выбранном направлении продолжается в противном случае, т.е когда $S_2^2 \geq S_1^2$ в качестве оценки i-го параметра модели выбирается значение $a_i^{(1)}$. Процесс определения оптимальных оценок параметров экономической модели составляет определенное количество шагов до тех пор, пока изменение значений отдельных параметров в любом направлении не будут приводить к росту полученного на предыдущем этапе значения суммы квадратов ошибки $S_j^2$. j индекс характеризует номер предпоследнего этапа расчетов
