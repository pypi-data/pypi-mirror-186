...
class database_table(database_file):
    ...
    def insert_rows(self, insert_data:list):
        ...
        # check if lists in insert_data have same length - todo
        for i in range(len(insert_data)):
            insert_data_i = insert_data[i]
            insert_data_i.append(secrets.token_hex(16))
            insert_data_i.append(str(datetime.now())[0:19])
            cur.execute(insert, tuple(insert_data_i))
