...
class Mixin:
    ...
    def generate_indices_based_on_transactions(self, transactions, client, version):
        ...
        for tx in transactions:
            ...
            if version == 1:
                ...
                index_all, index_transfer = self.determine_account_indices(classified_tx, tx, classificationResult, version)
                list_accounts_involved_all.append(ReplaceOne({"_id": index_all["_id"] }, index_all, upsert=True))
                if index_transfer:
                    list_accounts_involved_transfer.append(ReplaceOne({"_id": index_transfer["_id"] }, index_transfer, upsert=True))
            
                if classificationResult.contracts_involved:
                    for contract in classificationResult.list_of_contracts_involved:
                        index_contract = self.determine_contract_indices(classified_tx, tx, classificationResult, contract, version)
                        list_contracts_involved.append(ReplaceOne({"_id": index_contract["_id"] }, index_contract, upsert=True))

        try:
            if len(list_accounts_involved_all) > 0:
                result = self.collection_involved_accounts_all.bulk_write(list_accounts_involved_all)
                print (f"B: {tx['blockHeight']:,.0f} | Match {result.matched_count:,.0f} | Mod {result.modified_count:,.0f} | Upsert {result.upserted_count:2,.0f} in _all.")
        except Exception as e: 
            print (e)

        try:
            if len(list_accounts_involved_transfer) > 0:
                result = self.collection_involved_accounts_transfer.bulk_write(list_accounts_involved_transfer)
                print (f"B: {tx['blockHeight']:,.0f} | Match {result.matched_count:,.0f} | Mod {result.modified_count:,.0f} | Upsert {result.upserted_count:2,.0f} in _transfer.")
        except Exception as e: 
            print (e)

        try:
            if len(list_contracts_involved) > 0:
                result = self.collection_involved_contracts.bulk_write(list_contracts_involved)
                print (f"B: {tx['blockHeight']:,.0f} | Match {result.matched_count:,.0f} | Mod {result.modified_count:,.0f} | Upsert {result.upserted_count:2,.0f} in _contracts.")
        except Exception as e: 
            print (e)
    ...
    def determine_account_indices(self, 
    ...
    def determine_contract_indices(self, 
        classified_tx: Transaction, 
        tx, 
        classificationResult: ClassificationResult, 
        contract,
