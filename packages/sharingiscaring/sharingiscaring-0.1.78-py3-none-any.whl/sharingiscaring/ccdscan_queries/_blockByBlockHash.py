...
class Mixin:
    ...
    def ql_get_finalizers(self, block_hash, net=NET.MAINNET):
        ...
        if len(block_hash) == 64:
            while not done:
                this_batch, pageInfo = self.ql_get_finalizers_b_f(block_hash, before='null', after=after, net=net)
    ...
    def ql_get_finalizers_b_f(self, block_hash, before: str = None, after: str = None, net=NET.MAINNET):
        query = "query {"
        ...
        """
        try:
            url_to_use = self.graphql_url if net == NET.MAINNET \
                else self.graphql_url_testnet
