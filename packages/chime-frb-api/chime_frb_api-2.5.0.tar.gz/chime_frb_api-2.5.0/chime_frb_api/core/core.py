...
class API:
    ...
    def generate_token(
        ...
        log.info("Authorization Result: Passed")
        tokens = response.json()
        print("Please add your new tokens to your environment using the following:")
        log.debug(tokens)
        print(f"export CHIME_FRB_ACCESS_TOKEN=\"{tokens.get('access_token', None)}\"")
        self.access_token = tokens.get("access_token", None)
        print(f"export CHIME_FRB_REFRESH_TOKEN=\"{tokens.get('refresh_token', None)}\"")
