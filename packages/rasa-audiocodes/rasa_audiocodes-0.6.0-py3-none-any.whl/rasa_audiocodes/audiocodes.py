...
class AudiocodesInput(InputChannel):
    ...
    def _check_token(self, token: Optional[Text]) -> None:
        if not token or not token.replace("Bearer ", "") == self.token:
            raise sanic.SanicException(status_code=401)
    ...
    def _get_conversation(
        ...
        if not conversation:
            raise sanic.SanicException("Not found", status_code=404)
        conversation.update()
        return conversation
    ...
    def handle_start_conversation(self, body: Dict[Text, Any]) -> Dict[Text, Any]:
        ...
        if cid in self.conversations:
            raise sanic.SanicException("Conversation already exists", status_code=500)
