...
class TrackTable(QtWidgets.QTableView):
    ...
    def selectRow(self, rowIdx: int):
        ...
        """
        modelIndex = self.myModel.index(
            rowIdx, 0
        )  # rowIdx is in 'model' coordinates
        visualRow = self.proxy.mapFromSource(modelIndex).row()
        print(f"model rowIdx:{rowIdx} corresponds to visual row:{visualRow}")
    ...
    def mySelectRows(self, rows: Set[int]):
        ...
        if selectionModel:
            ...
            if rows:
                indexes = [
                    self.myModel.index(r, 0) for r in rows
                ]  # [QModelIndex]
                visualRows = [
                    self.proxy.mapFromSource(modelIndex)
    ...
    def on_selectionChanged(self, selected, deselected):
        ...
        selectedIndexes = [
            self.proxy.mapToSource(modelIndex).row()
        ...
        print(f"selectedIndexes:{selectedIndexes}")
