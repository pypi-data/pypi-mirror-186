...
class gDriveExplorer:
    ...
    def get_id_from_path(self, path_from_base):
        ...
        if path_from_base not in explore_dict.keys():
    ...
    def to_dill(self, lista_funciones, path_from_base, **kwargs):
        ...
        with open(name, 'wb') as dill_file:
            dill.dump(lista_funciones, dill_file)

        self.create_or_update_file(path_from_base)

        os.remove(name)

    def to_pickle(self, lista_funciones, path_from_base, **kwargs):
        '''
        write to csv using pydrive
        kwargs are the kwargs of pd.to_csv function
        '''
        ...
        with open(name, 'wb') as f:
            pickle.dump(lista_funciones, f)

        self.create_or_update_file(path_from_base)

        os.remove(name)

    def to_parquet(self, df, name, path_from_base, **kwargs):
    ...
    def to_excel_sheets(self, dic, path_from_base, **kwargs):
        ...
        with pd.ExcelWriter(name) as writer:
            for i in dic.keys():
                dic[i].to_excel(writer, sheet_name=i)
    ...
    def get_csv_from_zip(self, path_from_base, file_name, **kwargs):
        ...
        zipreader = zipfile.ZipFile(io.BytesIO(self.get_file_from_path(path_from_base)))

        df = pd.read_csv(zipreader.open(file_name), **kwargs)
