    ...
    for dataset_name in dataset_names:
        for classifier_name in classifier_names:
            ...
            if classifier_name == 'fcn':
                ...
                fit(fcn.Classifier_FCN, trainloader, valloader, input_shape, nb_classes, dataset_name, classifier_name, epochs, validation_data=True)

            elif classifier_name == 'cnn':
                from test_tscai_1.models import cnn
                fit(cnn.Classifier_CNN, trainloader, valloader, input_shape, nb_classes, dataset_name, classifier_name, epochs, validation_data=True)

            elif classifier_name == 'mlp':
                from test_tscai_1.models import mlp
                fit(mlp.Classifier_MLP, trainloader, valloader, input_shape, nb_classes, dataset_name, classifier_name, epochs, validation_data=True)

            elif classifier_name == 'resnet':
                from test_tscai_1.models import resnet
                fit(resnet.Classifier_RESNET, trainloader, valloader, input_shape, nb_classes, dataset_name, classifier_name, epochs, validation_data=True)

            elif classifier_name == 'inception':
                from test_tscai_1.models import inception
                fit(inception.Classifier_INCEPTION, trainloader, valloader, input_shape, nb_classes, dataset_name, classifier_name, epochs, validation_data=True)
