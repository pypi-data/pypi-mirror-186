...
class PGS3CH:
    ...
    def transform_params(self):
        ...
        self.pg_history_step = (
                self.config.get("pg_history_step", PG_HISTORY_STEP_DEFAULT)
                or PG_HISTORY_STEP_DEFAULT
        )
        self.need_optimize = bool(self.config.get("need_optimize", True) or True)
        self.need_clear_s3 = bool(self.config.get("need_clear_s3", False) or False)
        self.incremental = bool(self.config.get("incremental", False) or False)
        self.by_date = bool(self.config.get("by_date", False) or False)
        ...
        self.where_condition = bool(self.config.get("where_condition", False) or False)
