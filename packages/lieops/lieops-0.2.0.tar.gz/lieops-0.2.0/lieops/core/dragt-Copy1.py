...
def dragtfinn(*p, offset=[], tol=1e-14, tol_checks=0, order='auto', disable_tqdm=False, **kwargs):
    ...
    **kwargs
    ...
    from lieops.linalg.matrix import printmat
    ...
    if tol_checks > 0: # Perform some consistency checks
        ...
        for k in range(len(p)):
            print ((p[k] - op_result[k]).above(1e-14))
        print ('-1--')
        
        print (lexp(SA, power=10)(*lexp(SB, power=10)(*xieta0)))
        print ('-2--')
        print ([o(*xieta0) for o in op_result])
        
        print ('-3--')
        print (lexp(SB, power=10)(*lexp(SA, power=10)(*xieta0))) # a grees with -2-, but not with -1-

        
        p1_test = [e.extract(key_cond=lambda k: sum(k) >= 1) for e in p]
        p_new_test2 = [lexp(-SB)(*lexp(-SA)(qq, power=30), power=30) for qq in p1_test]
        for qq in p_new_test2:
            print (qq)
            print ()
        # xieta == p_new_test2,

        p_new_test3 = lexp(-SB)(*lexp(-SA)(*p1_test, power=30), power=30)
        for qq in p_new_test3:
            print (qq)
            print ()
        ...
        printmat(R)
        p1_test = [e.extract(key_cond=lambda k: sum(k) >= 1) for e in p]
        p_new_test = [sum([p1_test[k]*Ri[l, k] for k in range(dim2)]) for l in range(dim2)] # multiply Ri from right to         # Further idea: p[i]@p[k + dim] should be -1j*delta_{ik} etc. But this might often not be well satisfied in higher orders
        for qq in p_new_test:
            print (qq)
            print ()
