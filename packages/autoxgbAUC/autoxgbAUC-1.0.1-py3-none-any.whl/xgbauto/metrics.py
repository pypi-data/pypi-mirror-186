...
def calculate(self, y_true, y_pred):
    ...
    for metric_name, metric_func in self.valid_metrics.items():
        if self.problem_type == ProblemType.binary_classification:
            ...
            # dmcc_edit
            # adding calculation of auc_pr
            elif metric_name == "aucpr":
                precision, recall, thresholds = skmetrics.precision_recall_curve(y_true, y_pred[:, 1])
                metrics[metric_name] = metric_func(recall, precision)
            else:
