...
from typing import TypeVar, Any, Iterable, Sequence, get_origin, get_args
...
class BaseModel:
    ...
    @classmethod
    def get_query(cls, primary_keys: Sequence, query_class: pypika.Query = pypika.PostgreSQLQuery):
        """ Primary keys can be Params """
        q = query_class.from_(cls.t).select(*cls.fs).where(cls.primary_key_filter(primary_keys))
        return q
