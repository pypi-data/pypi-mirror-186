...
from rdkit.Chem import rdMolHash, MolToSmiles, rdmolops, MolToMolBlock, MolFromMolBlock, MolFromSmarts, GetSymmSSSR, SetAromaticity, AROMATICITY_SIMPLE
...
class ScaffoldGraph(nx.DiGraph, ABC):
    ...
    **Subclasses:**
    ...
    def toAromRingIndexStr(self, idxAtom, rings):
        #arom_rings = []
        strOut = ''
        for ring_idx in range(len(rings)):
            if idxAtom in rings[ring_idx]:
                if ring_idx+1 > 9:
                    strOut = strOut + str(ring_idx+1)
                else: 
                    strOut = strOut + '0' + str(ring_idx+1)

        missing0Count = 6 - len(strOut);
        if missing0Count < 0:
            print('GLOBAL ERROR - TOUT EST FOUTU! - missing0Count < 0')

        if missing0Count > 0:
          strOut = strOut + '0'*missing0Count;
    ...
    def _construct(self, molecules, init_args, ring_cutoff=10, progress=False):
        ...
        for molecule in tqdm(
            ...
            #print('Processing mol ', count_mol);
            #print(MolToMolBlock(molecule, kekulize=False));
           #if count == 15:
           #   count = count + 1;
           #   continue;
            ...
            blkOnMol = MolToMolBlock(molecule, kekulize=True)
            molQTest = MolFromSmarts("[#6,#7,#8;#16,a]")
            arom_atom_indices_ = molecule.GetSubstructMatches(molQTest)#.map((s) => s[0]);
            arom_atom_indices = sorted(list(map(lambda x: x[0], arom_atom_indices_)))
            #print('Processing mol 1', count_mol);
            
            #rings = [set(x) for x in GetSymmSSSR(molecule)]
            rings_aroma = [];
            rings = [list(x) for x in GetSymmSSSR(molecule)]
            
            #print('before ring cycle')
            for ring in rings:
                if len(list(set(ring) - set(arom_atom_indices))) == 0:
                    rings_aroma.append(ring);

            #print('after ring cycle')
            #print('SSSR')
            #print(rings_aroma)
            ...
            #print(editMolBlk)
            idxV2000 = blkOnMol.index("V2000\n")
            idxV2000ToEnd = blkOnMol.index("\n", idxV2000) +1
            count = 0;
            for idx_sorted in arom_atom_indices: 
                ...
                #print('before strOut')
                strOut = self.toAromRingIndexStr(idx_sorted, rings_aroma)
                #print('after strOut')

                #editArmMolBlk = replace_str_index(editArmMolBlk, idxV2000ToEnd + 56, "1")
                #blkOnMol = blkOnMol[:idxV2000ToEnd + 56] + "1" + blkOnMol[idxV2000ToEnd + 57:]
                blkOnMol = blkOnMol[:idxV2000ToEnd + 54] + strOut + blkOnMol[idxV2000ToEnd + 60:]

                #print('blkOnMol')
                #print(blkOnMol)

            molecule = MolFromMolBlock(blkOnMol)
            #print('Molecule')
            #print(molecule)

            init_molecule_name(molecule)
            #print('nofrings', CalcNumRings(molecule))
            #print('Processing mol 3', count_mol);
            ...
            if CalcNumRings(molecule) > ring_cutoff:
                ...
                continue
            #print('Processing mol 4', count_mol);
            scaffold = self._initialize_scaffold(molecule, init_args)
            #print(MolToMolBlock(scaffold.mol, kekulize=False));
            #print('Processing mol 5', count_mol);
    ...
    def _initialize_scaffold(self, molecule, init_args):
        ...
        #my changes severe
        editMolBlk = MolToMolBlock(molecule, kekulize=False);
        #print('INIT SCAFFOLD')
        #print(editMolBlk)
        ...
        #print('PostRW', molA)
        #my changes severe SetAromaticity(molA, AROMATICITY_SIMPLE);
        #molAMolBlk = MolToMolBlock(molA, kekulize=False);
        molecule = molA;
        
        #end of my changes severe
        ...
        #molAMolBlk = MolToMolBlock(scaffold_rdmol, kekulize=False);
        #print(molAMolBlk)
       
        scaffold = Scaffold(scaffold_rdmol)
        #molAMolBlk = MolToMolBlock(scaffold.mol, kekulize=False);
        #print(molAMolBlk)
        #print(scaffold.mol)
        #print('Processing mol 4a');
        annotation = None
        if init_args.get('annotate') is True:
            annotation = get_annotated_murcko_scaffold(molecule, scaffold_rdmol, False)
        #print('Processing mol 4b');
    ...
    def _preprocess_scaffold(self, scaffold_rdmol, init_args):
        ...
        """
        #print(init_args.get('discharge', False))
...
MolToSmiles(molecule))
    ...
    def add_scaffold_node(self, scaffold, **attr):
        ...
        **attr : keyword arguments, optional
        ...
        """
        default_attr = dict(type='scaffold', hierarchy=scaffold.rings.count, realmolecule=scaffold.mol)
        default_attr.update(attr)
    ...
    def from_supplier(cls, supplier, ring_cutoff=10, progress=False, annotate=True,
                      ...
                      discharge_and_deradicalize=False, remove_stereo=False, **kwargs):
        ...
        **kwargs : keyword arguments, optional
        ...
        init_args = dict(
            ...
            keep_largest=keep_largest_fragment,
            discharge=discharge_and_deradicalize,
            annotate=annotate, remove_stereo=remove_stereo
        )
        instance._construct(supplier, init_args, ring_cutoff=ring_cutoff, progress=progress)
        return instance
