...
from rdkit.Chem import (
    ...
    MolToMolBlock, MolFromMolBlock, RemoveStereochemistry
...
class MurckoRingFragmenter(Fragmenter):
    ...
    def extractRings1(self, str6):
        return [int(str6[0:2])-1, int(str6[2:4]) -1, int(str6[4:6]) -1]
    ...
    def fragment(self, scaffold):
        ...
        for rix, ring in enumerate(rings):  # Loop through all rings and remove
            ...
            molARW = RWMol(scaffold.mol)
            
            #editAromMolBlkMrk = MolToMolBlock(molARW)
            ...
            for parent in get_scaffold_frags(molARW): #(edit)
                ...
                for idx_atom in range(atomCount):
                    ...
                    idxV2000ToEnd = idxV2000ToEnd + 54 #56
                    #if molPBlok[idxV2000ToEnd] == "1":
                        #arom_atoms.append(idx_atom +1)
                    sixLetters = molPBlok[idxV2000ToEnd: idxV2000ToEnd+6]
                    sixLetters = sixLetters.replace(' ', '0');
                    #print('sixLetters')
                    #print(sixLetters)

                    if sixLetters != '000000':
                        atom_rings = self.extractRings1(sixLetters)
                        for idx_ring in atom_rings:
                            if idx_ring != -1:
                                if len(all_rings)-1 < idx_ring:
                                    for idx_at in range(0, idx_ring - len(all_rings)+1):
                                      all_rings.append([])
                                                                                                
                                all_rings[idx_ring].append(idx_atom)
                ...
                for idx_bond in range(bondCount):
                    ...
                    #print(atom_2)
                    b = False
                    for idx_ring in range(0, len(all_rings)):
                        if atom_1 -1 in all_rings[idx_ring] and atom_2 -1 in all_rings[idx_ring]:
                            b = True;
                            break

                    if b:#atom_1 in arom_atoms and atom_2 in arom_atoms:
...
MolToSmiles
    ...
    RemoveStereochemistry(annotated)
    if as_mol:
        return annotated
    ...
    try:
        blk = MolToMolBlock(annotated)
    except :
        blk = MolToMolBlock(mol) #my changes severe
        print('ERROR ANNOTATION', blk);
    return blk;
