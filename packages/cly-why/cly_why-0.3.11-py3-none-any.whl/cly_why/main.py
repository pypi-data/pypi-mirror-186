...
def get_funcs(all_funcs, ignore_char: str="_"):
    funcs = []
    for key, value in all_funcs:
        if callable(value) and not re.match(f"^{ignore_char}.*", key):
            #if value.__module__ in [__name__, "__main__"]:
            funcs.append(value)
    return funcs
...
def cly(args: list, all_funcs, def_cmd=cli_help, ignore_char: str="_", is_main: bool=False):
    if __name__ in ["__main__", args[0]] or is_main:
        try:
            ...
            sub_cmds = get_funcs(all_funcs, ignore_char=ignore_char)
            ...
            if len(cmd) == 1:
                ...
                print(f"Valid cmds are {sub_cmds}")
