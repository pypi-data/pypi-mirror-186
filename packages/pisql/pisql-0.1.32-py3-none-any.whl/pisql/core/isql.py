...
def renameDupColumns(columns: List[str]) -> List[str]:
    """Rename duplicate columns from a list of columns"""
    renamed_columns = []
    seen = {}
    for column in columns:
        if column in seen:
            seen[column] += 1
            renamed_columns.append(f"T{seen[column]}.{column}")
        else:
            seen[column] = 0
...
def expandRowAlongQuotes(row_aslist: List[str]) -> List[str]:
    """Expand a row that has quotes in it"""
    expanded = []
    for item in row_aslist:
        splitres = re.split(r"(NULL|\d+)[ ]*'[ ]*(NULL|\d+)", item)
        expanded += splitres if len(splitres) > 1 else [item]
    return [ v.strip() for v in expanded if v.strip() != "" ]

class iSqlDF(pl.DataFrame):
    def __init__(self, *args, **kwargs):
        if 'columns' in kwargs:
            kwargs['columns'] = renameDupColumns(kwargs['columns'])
...
class iSQL:
    ...
    @staticmethod
    def strRowColSplitter(raw: str, as_df: bool = True) -> Union[iSqlDF, iSqlRecords]:
        ...
        # just check 100 rows
        if any(len(row) != len(cols) for row in result_rows[:100]):
            result_rows = [ expandRowAlongQuotes(row) for row in result_rows ]

        if len(rawrows_aslist) == 1 and as_df:
    ...
    @staticmethod
    def createSqlTableFromDF(
        df : Union[pd.DataFrame, iSqlDF, pl.DataFrame],
        table_name: str,
        ):
        execInsertSql(df, table_name)
