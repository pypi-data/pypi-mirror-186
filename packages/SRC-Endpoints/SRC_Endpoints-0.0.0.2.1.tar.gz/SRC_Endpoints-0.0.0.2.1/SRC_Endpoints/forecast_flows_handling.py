...
def post_many_flows_to_database(dataset: pd.DataFrame, access_token: str, verbose: bool = True):
    ...
    header = {'Authorization': 'Bearer ' + access_token}
    response = requests.post(url, data=dict_list, headers=header)
...
def get_portfolio_weights(dataset: pd.DataFrame, schema: str, access_token: str, verbose: bool = True):
    """
    ...
    :param schema: the portfolio schema to be considered. (str)
    :param access_token: the access token of the user, must be an active access token. (str)
    :param verbose: determines if status is printed. Default=True. (bool)
    ...
    if not isinstance(dataset, pd.DataFrame):
        raise TypeError("The 'dataset' argument must be a Pandas DataFrame.")
    if not all(isinstance(v, str) for v in [schema, access_token]):
        raise TypeError("The 'schema' and 'access_token' arguments must be string types.")
    dict_list = dataset.to_json(orient='records')
    url = "https://www.samsonrockapis.com/forecast_flows&portfolio_weights&weight_schema={}".format(schema)
    header = {'Authorization': 'Bearer ' + access_token}
    response = requests.get(url, headers=header, data=dict_list)
    if response.status_code in [200, 201, 202, 203, 204]:
        if verbose:
            print("Status Code: {}, portfolio weights calculates.".format(response.status_code))
