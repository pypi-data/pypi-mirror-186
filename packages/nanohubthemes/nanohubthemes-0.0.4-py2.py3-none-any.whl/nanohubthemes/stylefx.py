...
# path to user nanohub-themes dir
user_dir = os.path.join(os.path.expanduser('~'), '.nanohub-themes')
...
try:
    ...
    jupyter_home = os.environ['HOME'] + "/.jupyter"; #jupyter_config_dir()
    jupyter_data = os.environ['HOME'] + "/.local/share/jupyter/" #jupyter_data_dir()
...
def getCurrentTheme():
    ...
    if os.path.exists(theme_name_file):
        with fileOpen(theme_name_file, 'r') as f:
            line = f.read()
            if line in themes:
                theme = line        
    return theme
...
def style_layout(style_less,
    ...
    promptPadding = '.25em'
    promptBorder = '1px solid @prompt-line'
...
def toggle_settings(
    ...
    if nbname:
        ...
        #           '@notebook-bg;}\n')
        toggle += ('span.save_widget span.filename {margin-left: 8px; height: initial; margin-top: 5px; height: initial;'
                   'font-size: 100%; color: @nb-name-fg;}\n')
        toggle += ('span.save_widget span.filename:hover {color:'
                   '@nb-name-hover;background-color:transparent}\n')
        toggle += ('#menubar {padding-top: 4px; }\n')
    else:
...
/** HEADER 9 **/
    ...
    // This happens on older Jupyter Notebook tools
            ...
            let tb = document.getElementById("maintoolbar-container");
            if (tb){
                let themelist = document.createElement("select");
                themelist.id = 'nHSelectTheme';
                themelist.role = 'combobox';
                themelist.className = 'form-control select-xs';
                let themes = ['chesterish', 'dark', 'darkbronco', 'dorkula', 'grade3', 'gruvboxd', 'gruvboxl', 'monokai', 'oceans16', 'onedork','solarizedd','solarizedl','nanohub', 'default'];
                let option = document.createElement("option");
                option.value = "";
                option.text = "Change Theme";
                ...
                tb.appendChild(themelist);
                tb.onchange = function(e){
                    try {
                        let kernel = IPython.notebook.kernel;
                        let theme = e.target.value;
                        function loadcustomCSS(out){
                            let head  = document.getElementsByTagName('head')[0];
                            let link  = document.createElement('link');
                            link.rel  = 'stylesheet';
                            link.type = 'text/css';
                            link.href = base + 'custom/custom.css?version=' + theme ;
                            link.media = 'all';
                            deletecustomCSS(out)
                            head.appendChild(link);
                            IPython.notebook.get_cells().map((e)=>e.refresh())
                        }
                        function deletecustomCSS(out){
                            let head  = document.getElementsByTagName('head')[0];
                            let links  = document.getElementsByTagName('link');
                            for (let i = 0; i < links.length; i++) {
                                let l = links[i];
                                if(l.href.includes( 'custom/custom.css')){
                                    head.removeChild(l);
                                    return;
                                }
                            }
                        }
                        let code_input = 'import nanohubthemes; '
                        let callbacks = {}
                        if (themes.includes(theme)){
                            if (theme == "default"){
                                code_input += 'nanohubthemes.reset_theme()';
                                callbacks = {
                                    iopub : { output : loadcustomCSS }
                                }
                            } else {
                                code_input += 'nanohubthemes.install_theme(theme="' + theme + '",';
                                code_input += 'monofont="opensans",';
                                code_input += 'monosize=12,';
                                code_input += 'nbfont="opensans",';
                                code_input += 'nbfontsize=12,';
                                code_input += 'tcfont="opensans",';
                                code_input += 'tcfontsize=12,';
                                code_input += 'dffontsize=12,';
                                code_input += 'outfontsize=12,';
                                code_input += 'mathfontsize=12,';
                                code_input += 'margins="auto",';
                                code_input += 'cellwidth="100%",';
                                code_input += 'lineheight=170,';
                                code_input += 'cursorwidth=2,';
                                code_input += 'cursorcolor="default",';
                                code_input += 'altprompt=False,';
                                code_input += 'altmd=False,';
                                code_input += 'altout=False,';
                                code_input += 'hideprompt=False,';
                                code_input += 'vimext=False,';
                                code_input += 'toolbar=True,';
                                code_input += 'nbname=True,';
                                code_input += 'kernellogo=False,';
                                code_input += 'dfonts=False,';
                                code_input += 'keepcustom=True)';
                                callbacks = {
                                    iopub : { output : loadcustomCSS }
                                }
                            }
...
# hub_session_container
    ...
    host = host.split('_')[0]
    uid = os.getuid()
    with open(jupyter_customjs, 'w') as f:
