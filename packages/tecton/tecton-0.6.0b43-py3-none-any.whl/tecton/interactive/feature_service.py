...
from tecton.unified import feature_service as unified_feature_service
...
class FeatureService(Fco):
    ...
    @classmethod
    def _from_spec(cls, feature_service_spec: specs.FeatureServiceSpec, fco_container: FcoContainer):
        """
        :param feature_service_spec: A FeatureServiceSpec.
        ...
        obj._spec = feature_service_spec
        obj.proto = feature_service_spec.data_proto
        obj._fco_container = fco_container
        obj.feature_set_config = FeatureSetConfig.from_spec(feature_service_spec, fco_container)
    ...
    def feature_views(self) -> List[Union[FeatureView, FeatureTable]]:
        ...
        """
        fvs = []
        for fd in self.feature_set_config.feature_definitions:
            if fd.is_feature_table:
                fvs.append(FeatureTable(fd.feature_view_proto, self._fco_container))
            else:
                fvs.append(get_feature_view_by_type(fd.fv_spec, self._fco_container))
        return fvs
...
def get_feature_service(name: str, workspace_name: Optional[str] = None) -> FeatureService:
    ...
    if conf.get_bool("UNIFIED_TECTON_OBJECTS_ENABLED"):
        return unified_feature_service.FeatureService._from_spec(feature_service_spec, fco_container)
    else:
        return FeatureService._from_spec(feature_service_spec, fco_container)
