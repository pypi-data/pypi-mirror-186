...
from .lookup import (
    ...
    is_siteType_allowed, is_primary_product_id_allowed, is_primary_product_termType_allowed
)
...
def is_run_required(model: str, term, node: dict):
    """
    ...
    term : str or dict
        ...
        If only the `@id` is provided, the full JSON-LD will be downloaded.
    node : dict
    ...
    """
    term_dict = download_hestia(term) if isinstance(term, str) else term
    return _run_model_required(model, term_dict, node) and _run_required(model, term_dict, node) if term_dict else True


def _run_required_default(model: str, term: dict, data: dict, module):
    return getattr(module, 'run')(data) if is_run_required(model, term, data) else []


def _run_required_emission(model: str, term: dict, data: dict, module):
    return getattr(module, 'run')(data) if is_run_required(model, term, data) else (
        emission._default_emission(model, term, data, getattr(module, 'RETURNS', {}))
    )
...
def run_if_required(model: str, term_id: str, data: dict, module):
    ...
    return RUN_REQUIRED_BY_TYPE.get(return_type, _run_required_default)(model, term, data, module) if all([
        term, return_type
    ]) else getattr(module, 'run')(data)
