...
class IOTObix(IOTDriver):
    ...
    def _set_reltime(self):
        ...
        if len(reltime) > 0:
            ...
            self.logging(content=f"set reltime({url})")
    ...
    def _add_watch(self):
        if 'url' not in self.watch.keys():
            self.logging(content=f"add watch({self.configs.get('url')}/obix/watchService/make/)")
    ...
    def _add_points(self, new_items: list):
        if 'url' in self.watch.keys():
            ...
            if len(add_items) > 0:
                ...
                self.logging(content=f"add point({self.watch.get('url')}add/)")
    ...
    def _del_watch(self):
        try:
            if 'url' in self.watch.keys():
                self.logging(content=f"delete watch({self.watch.get('url')}delete/)")
    ...
    def _pool_refresh(self):
        if 'url' in self.watch.keys():
            self.logging(content=f"pool refresh({self.watch.get('url')}pollRefresh/)")
