...
class KalmanFilter:
    """A kalman filter for 2D points"""

    def __init__(self, input_dim=42, cov_process=0.002, cov_measure=0.01):

        # Check input dimension
        assert input_dim == 42, "only 21 points allowed"
        ...
        self.state[: self.measure_num, :] = np.array(
            [320, 240] * (self.measure_num // 2)
        )[:, np.newaxis]
    ...
    def update(self, measurement):
        ...
        return self.state[: self.measure_num, :].reshape(-1, 2)
