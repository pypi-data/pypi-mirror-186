    ...
    async def running(self):
        ...
        try:
            ...
            async with self.session.ws_connect(self.uri, headers=headers, verify_ssl=False, heartbeat=30) as self.ws_client:
                try:
                    _LOGGER.error("AIOHTTP websocket connected")
                    ...
                    async for message in self.ws_client:
                        _LOGGER.error(f'AIOHTTP websocket message {message.type}')
            ...
            _LOGGER.error(f'AIOHTTP ClientResponseError')
            if error.code == 401:
            ...
            _LOGGER.error(f'AIOHTTP ClientConnectionError, asyncio.TimeoutError')
            ...
            _LOGGER.error(f'AIOHTTP Exception')
            if self.state != STATE_STOPPED:
