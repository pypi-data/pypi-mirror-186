...
class Evidence(BaseModel):
    ...
    def __init__(self, **data):
        ...
        if self.span_end <= self.span_start:
            raise ValueError(
                f"span_end ({self.span_end}) should be greater than span_start ({self.span_start})."
            )
...
class Icsr(BaseModel):
    ...
    def has_valid_evidence(self):
        return (
            (len(self.subject_evidence) > 0)
...
class Document(BaseModel):
    ...
    def get_valid_icsrs(self):
        return Document(
            identifier=self.identifier,
            icsrs=[icsr for icsr in self.icsrs if icsr.has_valid_evidence()],
        )
...
class Dataset(BaseModel):
    ...
    def get_valid_icsrs(self):
        return Dataset(
            documents={
                identifier: doc.get_valid_icsrs() for identifier, doc in self.items()
            }
        )
