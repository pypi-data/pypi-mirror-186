    ...
    def parse_project(self, init: bool = False) -> None:
        ...
        with self.parsing_mutex:
            ...
            if (__dbt_major_version__, __dbt_minor_version__) < (1, 4):
                # temporarily patched so we write partial parse to correct directory
                from dbt_core_interface.patch import write_manifest_for_partial_parse

                _project_parser.write_manifest_for_partial_parse = partial(
                    write_manifest_for_partial_parse, _project_parser
                )
