...
class Game:
    def __init__(self, nick: str, channel_ws_dict: Dict[str, Union[websocket.WebSocket, websocket.WebSocketApp]], prefix: str='|', owner: list=['AAfFKK','VEbeHK','coBad2','eYFDHl']):
        """"""
        self.nick = nick
        ...
        self.owner = owner
        self.games = list()
        self.game_dict = {'werewolves': self.werewolves}
        self.running = {'werewolves': False}
        self.init_dict = {'werewolves': self._init_werewolves}
        """
        self.LINE = {
            "真心话": self._truth,
            "结算": self._atLast,
...
"""
    ...
    def start(self, name):
        ...
        if name in self.game_dict:
            ...
            raise ValueError("name must be in ['werewolves']")
        ...
        try:
            if "cmd" in wss:
                ...
                if cmd=="chat":
                    ...
                    if text==f"{self.prefix}help":
                    ...
                    elif text==f"{self.prefix}uwu":
                        if werewolf[0]["uwu"]:
                            if werewolf[0].get('user'):
                                self.dsend(f"/me >\n[【狼人杀】](https://)生存已经开始了", channel)

                            else:
                                self.dsend(f"/me >\n[【{werewolf[2][0][None]}】](https://)正在清点村里人数", channel)

                        else:
                            werewolf[0]={"uwu":True,"owner":{'nick':nick, 'channel': channel},"num":werewolf[1],"time":time.time(),"round":0,"temp":{},"user":{},"sudden":[],"prophet":[],"werewolf":{"skip":False,"user":[],"wait":[],"kill":[],"dead":[]},"witch":{"med":{True:[True,True,"解药","救活"],False:[False,True,"毒药","杀死"]},"temp":[],"allow":[]},"day":{"wait":[],"kill":[],"dead":[]},"now":[]}
                            ...
                            time.sleep(0.5)
                            self.dsend(f"/me >\n[【{werewolf[2][0][None]}】](https://)开始清点村里人数 请用{self.prefix}1报数")
                        ...
                        if not werewolf[0]["user"]:
                            ...
                            if len(werewolf[0]["temp"])==len(werewolf[0]["num"]):
                                ...
                                for uwu in tmp:
                                    uvu=werewolf[0]["num"][tmp.index(uwu)]
                                    werewolf[0]["user"][uwu]=[uwu,werewolf[2][uvu][0],werewolf[2][0][werewolf[2][uvu][2]],uvu,werewolf[2][uvu][2],True,werewolf[0]["temp"][uwu]]

                                    time.sleep(1)
                                    self.dsend(f"/w {uwu} >\n·\n你的卡牌：\n[【{werewolf[2][uvu][0]}】](https://)/[【{werewolf[2][0][werewolf[2][uvu][2]]}】](https://)\n·\n你的任务：\n{werewolf[2][uvu][1]}\n·\n获胜条件：\n[【{werewolf[2][0][not werewolf[2][uvu][2]]}】](https://)全员阵亡", werewolf[0]["temp"][uwu])
                                                        ...
                                                        for times in range(120):
                                                            ...
                                                            for uvu in werewolf[0]["werewolf"]["user"]:
                                                                if uvu not in werewolf[0]["werewolf"]["wait"]:
                                                                    time.sleep(1)
                                                                    self.dsend(f"/w {uvu} >\n今晚你的同伴猝死了\n{random.choice(werewolf[5][6])}的狼群忙于为TA送葬 没有行动", werewolf[0]["user"][uvu][6])
                            ...
                            if not werewolf[0]["user"]:
                                if nick==werewolf[0]["owner"]:
                                    ...
                                    if allow==werewolf[0]["num"]:
                                        ...
                                        self.dsend(f"/w {nick} >\n设定的有效牌至少要有4张", channel)

                                    elif len(allow)<=len(werewolf[0]["temp"]):
                                        self.dsend(f"/w {nick} >\n花名册的人数已经达到/超过牌数了", channel)
                                ...
                                if nick in werewolf[0]["day"]["wait"]:
                                    if uwu in werewolf[0]["state"][True]:
                                        ...
                                        self.dsend(f"/w {nick} >\n你没必要{random.choice(werewolf[3][7])}死者", channel)

                                    else:
                                        del werewolf[0]["day"]["wait"][werewolf[0]["day"]["wait"].index(nick)]
                                        self.dsend(f"/w {nick} >\n你{random.choice(werewolf[3][5])} 放弃了投票", channel)
                                    ...
                                    if werewolf[0]["now"][0]==2:
                                        if uwu==nick:
                                            ...
                                            if uwu in werewolf[0]["state"][True]:
                                                if uwu in werewolf[0]["prophet"]:
                                                    self.dsend(f"/w {nick} >\n你已经预言过[{uwu}](https://)的身份了\nTA是[【{werewolf[0]['user'][uwu][1]}】](https://) 属于[【{werewolf[0]['user'][uwu][2]}】](https://)", channel)
                                                    ...
                                                    self.dsend(f"/w {nick} >\n[{uwu}](https://)是[【{werewolf[0]['user'][uwu][1]}】](https://) 属于[【{werewolf[0]['user'][uwu][2]}】](https://)", channel)
                                                ...
                                                self.dsend(f"/w {nick} >\n你{random.choice(werewolf[3][0])}今晚放弃预言 睡个好觉", channel)
                                        ...
                                        if uwu in werewolf[0]["state"][True]:
                                            ...
                                            self.dsend(f"/w {nick} >\n你{random.choice(werewolf[3][0])}杀{aua}", channel)
                                            ...
                                            for uvu in werewolf[0]["werewolf"]["wait"]:
                                                ...
                                                time.sleep(1)
                                                self.dsend(f"/w {uvu} >\n你的同伴[{nick}](https://){random.choice(werewolf[3][0])}杀{aua}", werewolf[0]["user"][uvu][6])
                                            ...
                                            del werewolf[0]["werewolf"]["wait"][werewolf[0]["werewolf"]["wait"].index(nick)]
                                            self.dsend(f"/w {nick} >\n你{random.choice(werewolf[3][0])}今晚做件好事 放过人类", channel)

                                            for uvu in werewolf[0]["werewolf"]["wait"]:
                                                time.sleep(1)
                                                self.dsend(f"/w {uvu} >\n你的同伴[{nick}](https://){random.choice(werewolf[3][3])}了", werewolf[0]["user"][uvu][6])
                                        ...
                                        if not werewolf[0]["werewolf"]["wait"]:
                                            ...
                                            for uvu in werewolf[0]["werewolf"]["user"]:
                                                ...
                                                if uvu==uwu:
                                                    self.dsend(f"/w {uvu} >\n今晚 你被杀死了", werewolf[0]["user"][uvu][6])

                                                else:
                                                    self.dsend(f"/w {uvu} >\n今晚 {aua}", werewolf[0]["user"][uvu][6])
                                        ...
                                        if uwu==nick:
                                            ...
                                            if uwu in werewolf[0]["witch"]["allow"]:
                                                ...
                                                self.dsend(f"/w {nick} >\n你{random.choice(werewolf[3][0])}今晚放弃投药 等待机会", channel)
