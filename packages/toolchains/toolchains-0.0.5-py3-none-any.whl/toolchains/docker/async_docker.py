...
class AsyncDocker:
    ...
    def run_async_loop(self, function):
        loop = None
        pawn.console.debug(f"function={function.__name__}, loop={loop}")
    ...
    async def _async_get_containers(self, filters={}, **kwargs):
        self.container_list = []
        pawn.console.log(f"[FILTER] {filters} ")
...
async def run_dyn_container(config, *args, **kwargs):
    ...
    pawn.console.log(f"[CONTAINER] Running container_name={container_name}")
