...
`ticket_session` can but must not be used for storing the session in
which the course was taken. Usually, level and session are inherited
from the parent study level container. Storing the same
information in course tickets is not necessary, unless the session
really differs or 'orphaned' course tickets (tickets without level
assignment) have to be stored in the Level Zero container.
`ticket_session` can't be edited through the UI, they only changed
by import.
