...
class logger:
    ...
    def log(self, *items, level='info'):
        ...
        if not message:
            ...
            coloredlogs.install(level=logging.INFO, logger=self.logger, fmt='%(message)s')
        ...
        if not message:
            ...
            self.log_file.setFormatter(log_format)
            coloredlogs.install(level=logging.INFO, logger=self.logger, fmt=self.indent + '%(message)s') # Reset to default log format
    ...
    def current_time(self, *items, level='info'):
        coloredlogs.install(level=logging.INFO, logger=self.logger, fmt=self.indent + '%(asctime)s.%(msecs)03d: %(message)s', datefmt='%H:%M:%S') # Add current time to printed log output format
    ...
    def set_indent(self, indent=' - '):
        ...
        self.log_file.setFormatter(log_format)
        coloredlogs.install(level=logging.INFO, logger=self.logger, fmt=indent + '%(message)s')
