...
class KubeModel(ABC):
    ...
    def _save_file_test(self):
        ...
        self.logger.debug('saving file to host')

        with open('/output/{job_id}:test:{func_id}.pkl', 'wb') as f:
            self.logger.debug('saving file, epoch: {self.epoch}')
            pickle.dump('test', f)
    ...
    def _load_file_test(self):
        ...
        self.logger.debug('loading file to host')

        with open('/output/{job_id}:test:{func_id}.pkl', 'wb') as f:
            self.logger.debug('loading file, epoch: {self.epoch}')
            state = pickle.load(f)
            self.logger.debug('file loaded, value: {state}')
            print('file loaded')    
