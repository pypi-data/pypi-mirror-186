...
#    Create and manage fluxwallet legacy/segwit single and multisignatures wallets from the commandline
#
...
def parse_args():
    parser = argparse.ArgumentParser(description="fluxwallet command line wallet")
    parser.add_argument(
        "wallet_name",
        nargs="?",
        default="",
    ...
    group_wallet.add_argument(
        ...
        "-l",
        action="store_true",
        help="List all known wallets in fluxwallet database",
    )
    group_wallet.add_argument(
        "--wallet-info", "-w", action="store_true", help="Show wallet information"
    )
    group_wallet.add_argument(
        "--update-utxos",
        "-x",
    ...
    group_wallet.add_argument(
        ...
        "-g",
        action="store_true",
        help="Generate a new masterkey, and show"
    ...
    group_wallet.add_argument(
        "--import-private", "-k", help="Import private key in this wallet"
    )

    group_wallet2 = parser.add_argument_group("Wallet Setup")
    group_wallet2.add_argument(
        "--passphrase",
        nargs="*",
        default=None,
...
def create_wallet(wallet_name, args, db_uri):
    ...
    if args.create_multisig:
        ...
        from fluxwallet.keys import get_key_format
...
def main():
    ...
    if (
        ...
        if (
            ...
            and input(
                "Wallet %s does not exist, create new wallet [yN]? " % args.wallet_name
            ).lower()
            != "y"
        ):
            clw_exit("Aborted")
