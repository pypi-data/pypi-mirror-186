...
from fluxwallet.transactions.helpers import get_unlocking_script_type
...
class WalletTransaction(TransactionBuilder):
    """
    ...
    def __init__(self, hdwallet, account_id=None, *args, **kwargs):
        ...
        if hdwallet.witness_type in ["segwit", "p2sh-segwit"]:
            witness_type = "segwit"
        # should base version on network

        TransactionBuilder.__init__(self, witness_type=witness_type, *args, **kwargs)
    ...
    def send(self, offline=False):
        ...
        # {'txid': {'code': -26, 'name': 'Error', 'message': '16: mandatory-script-verify-flag-failed (Script evaluated without error but finished with a false/empty top stack element)'}})
        if not res:
            self.error = "Cannot send transaction. %s" % srv.errors
            return None
        if "txid" in res and isinstance(res["txid"], str):
            ...
            for inp in self.inputs:
                ...
                utxos = (
                    ...
                    .filter(
                        ...
                        DbTransactionOutput.output_n == inp.outpoint.n,
        ...
        for ti in self.inputs:
            ...
            if not tx_input:
                ...
                new_tx_item = DbTransactionInput(
                    ...
                    output_n=ti.outpoint.n,
                    key_id=key_id,
                    value=ti.value,
                    prev_txid=ti.outpoint.txid,
                    index_n=ti.index_n,
                ...
                if ti.outpoint.txid:
                    tx_input.prev_txid = ti.outpoint.txid
...
class Wallet(object):
    ...
    def transactions_update_by_txids(self, txids):
        ...
        for t in txs:
            ...
            wt.store()
            utxos = [(ti.outpoint.txid.hex(), ti.output_n) for ti in wt.inputs]
    ...
    def transaction_create(
        ...
        random_output_order=False,  # I changed this
        message="",
    ):
        """
        ...
        # this gets the blockcount at init
        srv = Service(
            network=network, providers=self.providers, cache_uri=self.db_cache_uri
        )
        ...
        transaction.expiry_height = srv.blockcount() + 21
        # transaction.expiryheight = 1298740
        transaction.tx_new.nExpiryHeight = transaction.expiry_height
        # Add inputs
        sequence = 0xFFFFFFFF
        ...
        if input_arr is None:
            ...
            for inp in input_arr:
                ...
                if isinstance(inp, Input):
                    prev_txid = inp.outpoint.txid
                    output_n = inp.outpoint.n
                    key_id = None
                    value = inp.value
        ...
        # this is wrong - add method get_txid or something that double sha256 the complete tx.
    ...
    def transaction_import(self, t):
        ...
        """
        if isinstance(t, TransactionBuilder):
    ...
    def transaction_import_raw(self, rawtx, network=None):
        ...
        t_import = TransactionBuilder.parse_bytes(rawtx, network=network)
        rt = self.transaction_create(
            t_import.outputs,
            t_import.inputs,
            network=network,
