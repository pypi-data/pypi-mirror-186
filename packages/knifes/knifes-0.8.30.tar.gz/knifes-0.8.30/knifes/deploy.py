...
def celery(project_dir, log_dir, project_name, operate):
    cmd = f'cd {project_dir} && venv/bin/celery -A {project_name} multi {operate} worker --pidfile={log_dir}celery_%n.pid --logfile={log_dir}celery_%n%I.log --concurrency=1 --beat'
