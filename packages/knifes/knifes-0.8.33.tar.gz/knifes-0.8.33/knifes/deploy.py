...
def print_help(project_dir, app_name):
    print('reload      --reload gunicorn\n'
          ...
          'celery [start | restart | stopwait | stop]\n'
...
def main_script(chdir, logdir, app_name, project_name=None):
    ...
    if not args:
        ...
        celery(chdir, logdir, project_name, args[1])
    elif args[0] == 'celerybeat':
        celerybeat(chdir, logdir, project_name, args[1])
    else:
        print_help(chdir, app_name)


def celery(project_dir, log_dir, project_name, operate):
    pid_file = f'{log_dir}celery_worker_manager.pid'
    cmd = f'cd {project_dir} && venv/bin/celery -A {project_name} multi {operate} w1 --pidfile={pid_file} --logfile={log_dir}celery_%n%I.log --concurrency=1'
    succ, err = exec_shell(cmd)
    print_succ(succ)
    print_err(err)
    if operate in ('stop', 'stopwait'):
        exec_shell(f'kill -KILL {read_file(pid_file).strip()}')


def celerybeat(project_dir, log_dir, project_name, operate):
    pid_file = f'{log_dir}celery_beat.pid'

    # stop
    if operate in ('restart', 'stop'):
        pid = read_file(pid_file).strip()
        if not pid:
            print_err(f'restart/stop beat failed，pid not exists:{pid_file}')
        exec_shell(f'kill -KILL {pid}')
        print_succ(f'stopped old beat process: {pid}')

    # start new process
    if operate in ('restart', 'start'):
        cmd = f'cd {project_dir} && venv/bin/celery -A {project_name} beat --pidfile={pid_file} --logfile={log_dir}celery_beat.log --detach'
        # 判断是否已启动
        succ, err = exec_shell(f"pgrep -f '{cmd}'")
        if succ:
            print_succ('当前已启动，请通过restart命令重启')
            return
        succ, err = exec_shell(cmd)
        print_succ(succ)
        print_err(err)
        time.sleep(1)
        print_succ(f'new beat process: {read_file(pid_file).strip()}')
