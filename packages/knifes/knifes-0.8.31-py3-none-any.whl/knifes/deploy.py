...
def celery(project_dir, log_dir, project_name, operate):
    cmd = f'cd {project_dir} && venv/bin/celery -A {project_name} multi {operate} w1 --pidfile={log_dir}celery_%n.pid --logfile={log_dir}celery_%n%I.log --concurrency=1'
...
def celerybeat(project_dir, log_dir, project_name, operate):
    pid_file = f'{log_dir}celerybeat.pid'
    ...
    if operate in ('restart', 'stop'):
        ...
        exec_shell(f'kill -9 {pid}')
        print_succ(f'stopped old beat process: {pid}')

    # start new process
    if operate in ('restart', 'start'):
        cmd = f'cd {project_dir} && venv/bin/celery -A {project_name} beat --pidfile={pid_file} --logfile={log_dir}celerybeat.log --detach'
        # 判断是否已启动
