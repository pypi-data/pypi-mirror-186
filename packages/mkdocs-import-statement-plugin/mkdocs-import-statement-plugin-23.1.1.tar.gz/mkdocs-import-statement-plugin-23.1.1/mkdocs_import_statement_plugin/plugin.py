...
class ImportPlugin(BasePlugin):
    ...
    def process_import_line(self, path, option):
        """
    ...
    def import_comment_mermaid(self, path, option):
        return self.import_comment(path, "mermaid")

    def import_comment_dot(self, path, option):
        return self.import_comment(path, "dot")

    def import_comment_plantuml(self, path, option):
        return self.import_comment(path, "plantuml")

    def import_image(self, path, option):
    ...
    def import_script(self, path, texttype, option):
        if os.path.isabs(path):
            abspath = path
        else:
            abspath = os.path.abspath(path)
    ...
    def import_text(self, path, option):
        with open(path, 'r') as f:
            output = []
            for line in f:
    ...
    def import_table(self, path, option):
