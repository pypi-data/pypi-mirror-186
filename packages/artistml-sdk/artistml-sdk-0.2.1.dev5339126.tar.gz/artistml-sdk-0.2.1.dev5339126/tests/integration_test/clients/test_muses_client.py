...
def test_get_run_result():
    ...
    resp = client.create_run(run=Run(
        flow_id=resp.details.id,
        name='test_run',
        runtime_config=PipelineSpec.RuntimeConfig(parameters={
            "a": Value(number_value=3),
            "b": Value(number_value=70),
            "c": Value(number_value=100),
        }, ),
        experiment="artistml-2023",
        namespace="artistml-2022",
        service_account="artistml-2022",
    ...
    with open(filepath, "w") as f:
        json.dump(body, f)
    # TODO: fix run status
    assert resp.to_dict()['run']['status'] == "Error"
