...
async def present(
    ...
    if result["old_state"]:
        ...
        old_network_interfaces = result["old_state"].get("network_interfaces")
        changes = hub.tool.gcp.utils.compare_states(
            result["old_state"],
            {
                "resource_id": resource_id,
                **resource_body,
                "network_interfaces": old_network_interfaces,
            },
            "compute.instance",
        )
        ...
        network_interfaces_changed = hub.tool.gcp.utils.compare_states(
            {"network_interfaces": old_network_interfaces or []},
            {"network_interfaces": network_interfaces or []},
            "compute.instance",
        )

        if not changes and not network_interfaces_changed:
            result["result"] = True
            result["comment"].append(
        ...
        if network_interfaces_changed:
            # Process network interfaces including access configs
            update_network_interfaces_ret = (
                await hub.tool.gcp.compute.instance.update_network_interfaces(
                    ctx, result["old_state"], network_interfaces
                )
            )

            if not update_network_interfaces_ret["result"]:
                result["comment"] += update_network_interfaces_ret["comment"]
                result["result"] = False
                return result
        ...
        if changes:
            if network_interfaces_changed:
                ...
                resource_body["network_interfaces"] = get_ret["ret"].get(
                    "network_interfaces"
                )
            ...
            if not resource_body.get("network_interfaces"):
                resource_body["network_interfaces"] = old_network_interfaces
