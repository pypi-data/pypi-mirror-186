...
def delete_connections(connections: list, project: Union[str, Project] = None, hard=False, show_prompt=True):
    """
    ...
    if hard and show_prompt:
        confirm = input("are you sure you want to completely delete this connection?"
...
class Object(_APIObject, metaclass=ObjectResolver):
    ...
    def delete(self, hard=False, show_prompt=True):
        ...
        if hard and show_prompt:
            confirm = input("are you sure you want to completely delete this object?"
                            "object will be irrecoverably deleted (type 'YES' to confirm) ")

            if confirm != "YES":
                return

        return raise_on_error(self._client.delete("objects", **{
            'uids': ','.join([uid]), 'project': self.project.uid, "hard": hard})).json()
...
class Connection(_APIObject):
    ...
    def delete(self, hard=False, show_prompt=True):
        return delete_connections([self], project=self.subject.project, hard=hard, show_prompt=show_prompt)
