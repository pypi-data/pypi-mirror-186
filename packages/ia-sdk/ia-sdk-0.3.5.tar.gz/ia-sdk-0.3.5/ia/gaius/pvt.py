...
from ia.gaius.data_ops import Data, MongoData
from ia.gaius.tests.pvt_utils import *
...
class PVTMessage():
    ...
    def __init__(self, status: str, current_record: int, total_record_count: int, 
                 metrics: dict, cur_test_num: int, total_test_num: int, test_id: str=None, user_id: str=''):
        self.status = status
...
class PerformanceValidationTest():
    ...
    def __init__(self, agent: AgentClient,ingress_nodes: list,query_nodes: list,num_of_tests: int, pct_of_ds: float,
                 ...
                 shuffle:bool=False, sio=None, task=None, user_id:str='', mongo_db=None, dataset_info:dict=None):
        ...
        Args:
            ...
            dataset_info (dict, optional): information about how to retrieve dataset, used for MongoDB query. If dataset_location is mongodb, this must have the user_id, dataset_id, and data_files_collection_name keys
        """
    ...
    def run_classification_pvt(self):
        self.pvt_results = {}  
        self.test_num = 1
        for test_num in range(1,self.num_of_tests+1):
            ...
            if self.sio:
                self.sio.emit('pvt_status', 
                              PVTMessage(status='finished', current_record=0, total_record_count=0, 
                                       metrics={}, cur_test_num=0, total_test_num=0, 
                                       test_id=None, user_id='').toJSON())
        return
    ...
    def train_agent(self):
        ...
        for j, _ in enumerate(self.dataset.train_sequences):
            ...
            if self.sio:
                self.sio.emit('pvt_status', 
                              PVTMessage(status='training', current_record=j, total_record_count=train_seq_len, 
                                       metrics={}, cur_test_num=self.test_num, total_test_num=self.num_of_tests, 
                                       test_id=None, user_id='').toJSON())
            ...
            if self.dataset_location == 'filepath':
                ...
                sequence = self.dataset.convertBinaryStringtoSequence(self.dataset.retrieveDataRecord(self.dataset.train_sequences[j]))
            else:
    ...
    def test_agent(self):
        ...
        for k, _ in enumerate(self.dataset.test_sequences):
            if self.sio:
                self.sio.emit('pvt_status', 
                              PVTMessage(status='testing', current_record=k, total_record_count=test_seq_len, 
                                       metrics={}, cur_test_num=self.test_num, total_test_num=self.num_of_tests, 
                                       test_id=None, user_id='').toJSON())
            ...
            if self.dataset_location == 'filepath':
                ...
                sequence = self.dataset.convertBinaryStringtoSequence(self.dataset.retrieveDataRecord(self.dataset.test_sequences[k]))
            else:
                raise Exception(f"dataset location {self.dataset_location} is unknown")
