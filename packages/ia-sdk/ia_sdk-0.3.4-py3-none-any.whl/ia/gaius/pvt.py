...
class PerformanceValidationTest():
    ...
    - Emotives Polarity
    """
    def __init__(self, agent: AgentClient,ingress_nodes: list,query_nodes: list,num_of_tests: int,ds_filepath:str,pct_of_ds: float,results_filepath,
                 pct_res_4_train:float, test_type:str, test_prediction_strategy="continuous",
                 clear_all_memory_before_training:bool=True, turn_prediction_off_during_training:bool=False, 
                 shuffle:bool=False, sio=None, task=None, user_id:str='', mongo=None):
    ...
    def test_agent(self):
        ...
        for k, file_path in enumerate(self.dataset.test_sequences):
            if self.sio:
                self.sio.emit('pvt_status', 
                              PVTMessage(status='testing', current_record=j, total_record_count=test_seq_len, 
                                       metrics={}, cur_test_num=self.test_num, total_test_num=self.num_of_tests, 
                                       test_id=None, user_id='').toJSON())
