...
class LiftOver:
    def __init__(self, from_db, to_db=None, search_dir='.', cache_dir=os.path.expanduser("~/.pyliftover"),
        ...
        if to_db is None:
            ...
            f = open_liftover_chain_file(from_db=from_db, to_db=to_db, search_dir=search_dir,
                                         cache_dir=cache_dir, use_web=use_web, write_cache=write_cache)
    ...
    def convert_coordinate(self, chromosome, position, strand='+'):
        ...
        The list may be empty (no conversion), have a single element (unique conversion),
        or several elements (position mapped to several chains).

        The list contains tuples (target_chromosome, target_position, target_strand,
        conversion_chain_score), where conversion_chain_score is the "alignment score"
        field specified at the chain used to perform conversion. If there are several
        possible conversions, they are sorted by decreasing conversion_chain_score.
        
        If chromosome is completely unknown to the LiftOver, None is returned.
        
        Note that coordinates are 0-based, and even at negative strand are relative to
        the beginning of the genome.
        ...
        if query_results is None:
            ...
            results = []
            for (source_start, _, idx) in query_results:
                target_start, chain = self.chain_file.data_by_index[idx]
