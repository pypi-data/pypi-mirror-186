...
from .rtf_trainer import ResumableTrainer, FrozenSeq2SeqTrainer
...
class REaLTabFormer:
        ...
        **training_args_kwargs,
        ...
        if self.model_type == ModelType.tabular:
            ...
            if parent_realtabformer_path is not None:
                ...
                parent_config = json.loads((parent_realtabformer_path / ModelFileName.rtf_config_json).read_text())
                ...
                self.parent_gpt2_state_dict = _normalize_gpt2_state_dict(torch.load((parent_realtabformer_path / ModelFileName.rtf_model_pt)))
    ...
    def _init_relational(self, relational_config):
        ...
        if self.parent_gpt2_config is not None:
            warnings.warn(
                "A trained model for the parent table is available. The encoder will use the pretrained config and weights."
            )
    ...
    def fit(
        ...
        Args:
            ...
            orig_samples_rounds: This is the number of train/hold-out samples that will be used to compute the epoch
             sensitivity value.
        Returns:
            Trainer
        """
    ...
    def _train_with_sensitivity(
        ...
        if frac != _frac:
            warnings.warn(
                f'The frac ({frac}) set results to a sample larger than frac_max_data={frac_max_data}. Setting frac to {_frac}.'
            )
            frac = _frac
        ...
        mean_closest_bdm_path = self.checkpoints_dir / TabularArtefact.mean_best_disc_model
        ...
        for p_epoch in range(last_epoch, self.epochs, n_critic):
                ...
                for g_idx in range(gen_rounds):
                    for _ in range(orig_samples_rounds):
                        original_df = df.sample(
                            n=sensitivity_orig_frac_multiple * gen_total,
                            replace=False)
            ...
            _delta_mean_sensitivity_value = abs(mean_sensitivity_value - val_sensitivity)
            ...
            if _delta_mean_sensitivity_value < best_mean_sensitivity_value:
                ...
                trainer.state.save_to_json((mean_closest_bdm_path / "trainer_state.json").as_posix())
        ...
        self.trainer_state = json.loads((loaded_model_path / "trainer_state.json").read_text())
    ...
    def _set_up_relational_coder_configs(self) -> None:
        ...
        for coder_name in ["encoder", "decoder"]:
            ...
            # This will prevent runtime error. `RuntimeError: CUDA error: device-side assert triggered`

            if coder_name == "decoder" and coder.n_positions < self.relational_max_length:
                coder.n_positions = 128 + self.relational_max_length
            elif coder_name == "encoder" and coder.n_positions < len(self.vocab[coder_name]["column_token_ids"]):
                coder.n_positions = 128 + len(self.vocab[coder_name]["column_token_ids"])
    ...
    def _fit_relational(
        ...
        if self.parent_gpt2_state_dict is not None:
            pretrain_load = self.model.encoder.load_state_dict(self.parent_gpt2_state_dict, strict=False)
            assert not pretrain_load.missing_keys, "There should be no missing_keys after loading the pretrained GPT2 state!"
    ...
    def save(self, path: Union[str, Path], allow_overwrite: Optional[bool] = False):
        ...
        for ignore_key in ["parent_vocab", "parent_gpt2_config", "parent_gpt2_state_dict", "parent_col_transform_data"]:
