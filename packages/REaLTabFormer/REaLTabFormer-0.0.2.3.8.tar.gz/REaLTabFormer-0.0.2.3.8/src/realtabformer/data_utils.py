...
def process_numeric_data(
    ...
    if series.dtype == pd.Int64Dtype():
        ...
        series = series.map(
            lambda x: f"{x:.{numeric_precision}f}"
        ).str.rstrip("0")
...
def process_datetime_data(
    ...
    series = (series.view(int) / 1e9).astype(int)
...
def process_data(
    ...
    if target_col is not None:
        assert first_col_type is None, "Implicit ordering of columns when teacher-forcing of target is used is not supported yet!"
        tf_col_name = f"{TEACHER_FORCING_PRE}_{target_col}"
        assert tf_col_name not in df.columns, f"The column name ({tf_col_name}) must not be in the raw data. Found instead..."
