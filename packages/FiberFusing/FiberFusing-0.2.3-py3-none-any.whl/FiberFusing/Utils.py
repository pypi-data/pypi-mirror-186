...
def NearestPoints(Object0, Object1):
    ...
    return _buffer.Point(position=(P[0].x, P[0].y))


def Union(*Objects):
    if len(Objects) == 0:
        return _buffer.PolygonComposition(instance=geo.Polygon())
...
def Intersection(*Objects):
    ...
    return _buffer.PolygonComposition(instance=intersection)
...
def interpret_to_point(*args):
    args = tuple(arg if isinstance(arg, _buffer.Point) else _buffer.Point(position=arg) for arg in args)
