...
import ctypes, glob, os, shutil, subprocess, sys, ctypes.wintypes, json, warnings
...
def _get_config() -> dict:
    ...
    """
    return json.load(open(_CONFIG_PATH, "r"))
...
def _save_config(config: dict) -> None:
    ...
    json.dump(config, open(_CONFIG_PATH, "w"), indent=4)
...
def _update_config_file(verbose=True) -> bool:
    ...
    if not os.path.exists(_CONFIG_PATH):
        if verbose:
            warnings.warn(f"{_printColours.RED}The configuration file is missing... Starting to rebuild{_printColours.END}")
        _create_config_file(_CONFIG_PATH)
        return 
    ...
    if type(config) != dict:
        if verbose:
            warnings.warn(f"{_printColours.RED}The configuration file is corrupted... Starting to rebuild{_printColours.END}")
        _create_config_file(_CONFIG_PATH)
...
def _install_fonts(force: bool, verbose=True) -> None:
    ...
    if len(fonts) == 0:
        warnings.warn(f"{_printColours.YELLOW}No fonts detected in this version. If you were expecting some to install, please contact the maintainer of this package, Sam.{_printColours.END}")
        return
