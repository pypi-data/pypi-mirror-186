...
def writeStimuliFromInterpolation_general(current_level: list, n_total_magnets: int, n_apertures: int, magnets: list, tShift: list, Outputfile: str, path_resources: Union[str,list],
    ...
    for k in range(1, n_total_magnets + 1):
        for l in current_level:
            if n_apertures == 2:
                [time, data_R1, data_R2] = InterpolateResistance(l, path_resources[magnet_type[k-1]-1], n_apertures=n_apertures, Type = InterpolationType)
            else:
                [time, data_R1] = InterpolateResistance(l, path_resources[magnet_type[k-1]-1], n_apertures=n_apertures, Type=InterpolationType)
            if not R1.size > 0:
                R1 = data_R1[np.newaxis, ...]
            else:
                R1 = np.vstack((R1, data_R1))
