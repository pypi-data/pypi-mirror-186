...
class regression:
  ...
  def significance_of_parameters(self, a):
    ...
    print('t_tabl:', t.ppf(1-a/2, self.n-self.k-1))
    print('t_calc:\n', self.model.tvalues)
...
If F_calc > F_tabl, then Н0 rejected"""
  ...
  def predict(self, X, col, a):
    """pd.DataFrame([[1,1,2],[1,2,2]], columns = ['const','x1','x2']), 'x1' """
    P = self.model.predict(X)
    print('y = ', P)
  
    Se_2 = np.var(self.model.predict(self.df_x_b)-self.df_y, ddof = 1)**0.5     
    X = X[col]
    min_ = P-Se * t.ppf(1-a/2, self.n-self.k-1) * (1 + 1/self.n + (X-self.df_x.mean())**2/(sum((self.df_x-self.df_x.mean()**2))) )**0.5
    max_ = P[0]+Se * t.ppf(1-a/2, self.n-self.k-1) * (1 + 1/self.n + (X-self.df_x.mean())**2/(sum((self.df_x-self.df_x.mean()**2))) )**0.5
