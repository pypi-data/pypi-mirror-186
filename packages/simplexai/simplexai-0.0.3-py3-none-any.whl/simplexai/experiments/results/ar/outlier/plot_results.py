...
for cv in cv_list:
    ...
    simplex_residuals = torch.from_numpy(
        ((true_reps - simplex_reps) ** 2).mean(axis=-1)
    )
    knn_dist_residuals = torch.from_numpy(
        ((true_reps - knn_dist_reps) ** 2).mean(axis=-1)
    )
    knn_uniform_residuals = torch.from_numpy(
        ((true_reps - knn_uniform_reps) ** 2).mean(axis=-1)
    )
    counts_simplex = []
    counts_nn_dist = []
    counts_nn_uniform = []
    ...
    for k in range(simplex_residuals.shape[0]):
        ...
        count_nn_uniform = torch.count_nonzero(
            nn_uniform_top_id > n_outliers - 1
        ).item()
