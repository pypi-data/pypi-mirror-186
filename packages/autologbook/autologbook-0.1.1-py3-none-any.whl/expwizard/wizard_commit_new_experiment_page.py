...
class CommitNewExperimentPage(QWizardPage, Ui_NewExperimentCommitPage):
    ...
    def post_new_experiment_entry(self):
        ...
        # if the sample description is multi-lined, then recopy it in the message body.
        if '\n' in self.field('description'):
            msg = markdown.markdown(self.field('description').replace('\n', '<br>'))
        else:
            msg = ''
