...
class VariableOverview:
    ...
    def __init__(self, input_path: str, analysis_id: int):
        ...
        self.var_overview = self.__parse_json_file()
        
        return None
    
    def __validate_inputs(self) -> bool:
        full_file_path = f'{self.input_path}{os.path.sep}{self.analysis_id}.json'
        return False if not Path(full_file_path) else True
        
    def __parse_json_file(self) -> pd.DataFrame:
        
        # Parse the file into a pd.DataFrame object
        
        print(f'{self.input_path}')

# %%

test = VariableOverview('hep', 999)
