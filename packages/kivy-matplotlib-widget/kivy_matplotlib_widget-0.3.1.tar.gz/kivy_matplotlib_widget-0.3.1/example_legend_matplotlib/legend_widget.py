...
def MatplotlibInteractiveLegend(figure_wgt,legend_handles='auto'):
    ...
    # figure_wgt.figure.canvas.draw()   
    figure_wgt.figcanvas.draw()
            
    if leg._get_loc() == 0:
        #get legend bbox position after 3sec. Otherwise, bbox is not good
        Clock.schedule_once(partial(create_touch_legend,figure_wgt,leg,ncol,legend_handles),3)
    else:
        Clock.schedule_once(partial(create_touch_legend,figure_wgt,leg,ncol,legend_handles))

def create_touch_legend(figure_wgt,leg,ncol,legend_handles,_):    
    """ create touch legend """
    renderer = figure_wgt.figure.canvas.get_renderer()
...
class MatplotlibLegendBox(FloatLayout):
    ...
    def update_size(self):
        ...
        if self.figure_wgt:
            ...
            if leg:
                renderer = self.figure_wgt.figure.canvas.get_renderer()
                bbox = leg.get_window_extent(renderer)
