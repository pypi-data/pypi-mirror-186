        ...
        for expectation in expectations:
            if expectation.event_field_name not in event:
                expectation_failures.append(
                    'expectation "{}" failed due to the event not'
                    " containing the field name.".format(
                        expectation,
                    )
                )
                continue
            ...
            event_value = event[expectation.event_field_name]
            if not expectation.comparison_function(event_value, expectation.comparison_value):
                message = 'expectation "{}" failed due to the comparison failing.'.format(
                    expectation,
                )

                if event_value.isinstance(type(expectation.comparison_value)):
                    message = (
