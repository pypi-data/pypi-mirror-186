...
class ColdResult:
    def __init__(self):
        ...
        self.bp_label = []  # 背压参数在数据集中的列标签名，一般就是"背压"，但对于全场优化，可能有多个背压参数，以列表方式传入
    ...
    def pass_result(self, opt_result={}, true_state={}):
        """
        传入新的冷端优化结果，并根据冷端功率大小调节背压大小以符合客观规律。

        :param opt_result: 当前时刻的原始优化结果
        :param true_state: 当前时刻的运行值，至少包含电功率、供热、环境以及冷端运行方式等参数
        :return:
        """
        self.last_opt_result = self.opt_result
        self.last_true_state = self.now_state
