...
class Filesystem(NetBoxModel):
    ...
    def __str__(self):
        return f"Filesystem: {self.filesystem}"
...
class LinuxVolume(NetBoxModel):
    ...
    filesystem = models.OneToOneField(
        Filesystem,
        on_delete=models.CASCADE,
        related_name="filesystem",
    ...
    def get_absolute_url(self):
        return reverse("plugins:netbox_disk:linuxvolume", kwargs={"pk": self.pk})
...
class WindowsVolume(NetBoxModel):
    ...
    def __str__(self):
        return f"Name: {self.drive_name} filesystem: {self.filesystem}"

    class Meta:
        ordering = ("drive_name", "filesystem")


@register_search
class WindowsVolumeIndex(SearchIndex):
    model = LinuxVolume
    fields = (
        ("drive_name", 100),
        ("filesystem", 100),
