...
class WindowsVolume(NetBoxModel):
    ...
    def get_absolute_url(self):
        return reverse("plugins:netbox_storage:windowsvolume", kwargs={"pk": self.pk})
...
class WindowsVolumeDrive(NetBoxModel):
    windows_volume = models.ForeignKey(
        WindowsVolume,
        on_delete=models.CASCADE,
        related_name="windowsvolume",
        verbose_name="windowsvolume",
    )
    ...
    def get_absolute_url(self):
        return reverse("plugins:netbox_storage:windowsvolumedrive", kwargs={"pk": self.pk})

    def __str__(self):
        return f"Name: {self.windows_volume} filesystem: {self.drives}"

    class Meta:
        ordering = ("windows_volume", "drive")
