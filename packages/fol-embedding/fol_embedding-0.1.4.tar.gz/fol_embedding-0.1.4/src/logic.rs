...
impl Environment {
    ...
    fn read_equation(&mut self, eq: &str, mut variables: &mut HashMap<String, usize>) -> Result<Equation> {
    ...
    fn read_equations(&mut self, eqs: &str, variables: &mut HashMap<String, usize>) -> Result<Vec<Equation>> {
        let mut equations = Vec::new();
        for eq in eqs.split(',') {
            equations.push(self.read_equation(eq, variables)?);
        }
        Ok(equations)
    }
    ...
    fn read_sequent(&mut self, s: &str) -> Result<Sequent> {
        ...
        match tokens.next() {
            Some(t) => lhs = self.read_equations(t, &mut variables)?,
            None => return Err(Error::IllegalSequent(String::from(s))),
        }
        match tokens.next() {
            Some(t) => rhs = self.read_equations(t, &mut variables)?,
            None => return Err(Error::IllegalSequent(String::from(s))),
        }
        match tokens.next() {
