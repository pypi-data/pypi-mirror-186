...
def _find_devices(additional_ip_candidates=None):
    candidates = ['127.0.0.1', '10.41.1.1', '10.41.2.1'] + additional_ip_candidates \
        if additional_ip_candidates is not None else []
    devices = {}
    devices.update(_find_devices_address(candidates))
...
class DeviceCommand(CliCommand):
    ...
    def select(self, args):
        devices = _find_devices([args.ip] if args.ip else [])
        serial = args.serial
