...
class Response():
    def Make(self, body:str=None, statusCode:int=200, contentType:str=None, headers:dict=None) -> flask.Response:
        resp = flask.Response(body=body, status=statusCode, content_type=contentType)
        if headers != None:
            for k in headers:
    ...
    def Status(self, statusCode:int):
        return flask.Response(status=statusCode)
