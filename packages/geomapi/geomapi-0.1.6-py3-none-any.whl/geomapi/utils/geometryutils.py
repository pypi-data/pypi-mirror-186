...
def compute_nearest_neighbors(reference_points:np.array, query_points:np.array,n:int=1)->Tuple[np.array,np.array]:
    """Compute nearest neighbors (indices and distances) from querry to reference points.

    Args:
        1. reference_points (np.array): points used as a reference.\n
        2. query_points (np.arrayn): points to calculate the distance from.\n 
        3. n (int, optional): number of neighbors. Defaults to 1.\n

    Returns:
        Tuple[np.array,np.array]: distances, indices 
    """    
...
def get_points_in_hull(geometry : o3d.geometry.Geometry, hull: o3d.geometry.Geometry) -> Tuple[o3d.geometry.Geometry,o3d.geometry.Geometry]:
    ...
    **NOTE**: JELLE, be carefull -> both hull and geometry both have internal restrictions which are not clearly explained. geometry should be point cloud..
...
def get_lineset(geometry: o3d.geometry.TriangleMesh, color: np.array = ut.random_color()) -> o3d.geometry.LineSet:
...
def las_add_extra_dimensions(las:laspy.LasData, recordData:np.array, names:List[str]=['newField'], dtypes:List[str]=["uint8"] )-> laspy.LasData:
    ...
    **NOTE**: to be tested
    ...
    Args:
        ...
        2. recordData (np.array[len(pcd.points),n] or pandas.DataFrame): array or dataframe with len(columns) == len(pcd.points).\n
    ...
    assert 'las' in type(las)
    assert len(names)==len(dtypes)    
    if 'np.array' in type(recordData):
        assert recordData.shape[0]==las.header.point_count
        assert recordData.shape[1]==len(names)
    elif 'dataframe' in type(recordData):
        assert len(recordData)==las.header.point_count
        assert len(recordData.columns)==len(names)
...
def array_to_colors(array: np.array, colors:np.array) -> np.array:
    ...
    Raises:
        ValueError: colors.shape[1] != 3\n
        ValueError: colors.shape[1] != values.shape[1]\n
    ...
    assert colors.shape[0] == len(values)
...
def crop_geometry_by_distance(source: o3d.geometry.Geometry, reference:List[o3d.geometry.Geometry], threshold : float =0.1) -> o3d.geometry.PointCloud:
    ...
    if type(source) is o3d.cpu.pybind.geometry.PointCloud:
        sourcePcd=source    
        #compute distance
