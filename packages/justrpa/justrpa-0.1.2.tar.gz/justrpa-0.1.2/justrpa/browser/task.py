...
import traceback
import logging
from justrpa.browser.selenium import SeleniumExt
...
class TaskBrowser(SeleniumExt):
    ...
    def build_filename(self, template:str="", **kwargs)->str:
        ...
        """
        params = kwargs.copy()
        filepath_template = self.filepath_template if template=="" else template
        filepath_template = "{account}/{site}/{taskname}/{account}-{site} {filetag} {timetag}.csv" if filepath_template=="" else filepath_template
        if "site" in params:
        ...
        if "filetag" not in params:
            params['filetag'] = ""
        filepath = filepath_template.format(**params)
...
### ------------------------------------------------------------
    ...
    def get_filepath_template(self, site, module:str)->str:
        replaced = self.get_replace_template(site, module)
        if replaced != None:
            return replaced
        else:
            return self.get_module_filepath_template(module)
