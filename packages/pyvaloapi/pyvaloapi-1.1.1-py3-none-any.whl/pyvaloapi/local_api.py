...
class UnofficialAPI:
	...
	"""
	def get_party_from_puuid(self, puuid):
		header = self.base_pvp_header.copy()
		header["X-Riot-Entitlements-JWT"] = self.get_auth_info()[1]
		header["X-Riot-ClientVersion"] = self.client_version
		return self.handle_pvp_request(f"parties/v1/players/{puuid}", prefix="glz-eu-1", header=header).get_json()
	...
	def set_player_ready(self, puuid, state=False):
		header = self.base_pvp_header.copy()
		header["X-Riot-Entitlements-JWT"] = self.get_auth_info()[1]
		return self.handle_pvp_request(f"parties/v1/parties/{puuid}/members/{puuid}/setReady", prefix="glz-eu-1", header=header).post({"ready": state})	
	
	def set_party_accessibility(self, partyID, accessibility=True):
