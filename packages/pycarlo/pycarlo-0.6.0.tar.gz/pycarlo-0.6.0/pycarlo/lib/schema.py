...
class ConnectionModelType(sgqlc.types.Enum):
    ...
    * `POWER_BI`: Power BI
    '''
    __schema__ = schema
    __choices__ = ('ATHENA', 'BIGQUERY', 'DATABRICKS_DELTA', 'DATABRICKS_METASTORE', 'DATABRICKS_SQL_WAREHOUSE', 'DBT_CLOUD', 'DBT_CLOUD_V2', 'DBT_CORE', 'GLUE', 'HIVE', 'HIVE_MYSQL', 'HIVE_S3', 'LOOKER', 'LOOKER_GIT', 'LOOKER_GIT_CLONE', 'LOOKER_GIT_SSH', 'POWER_BI', 'PRESTO', 'PRESTO_S3', 'REDSHIFT', 'S3', 'S3_AIRFLOW_LOG_EVENTS', 'S3_METADATA_EVENTS', 'S3_QL_EVENTS', 'SNOWFLAKE', 'SPARK', 'TABLEAU')
...
class EventModelEventType(sgqlc.types.Enum):
    ...
    * `INACTIVE_MONITOR`: Inactive Monitor
    '''
    __schema__ = schema
    __choices__ = ('COMMENT', 'CUSTOM_RULE_ANOM', 'DBT_MODEL_ERROR', 'DBT_TEST_FAILURE', 'DELETE_TABLE', 'DIST_ANOM', 'FRESH_ANOM', 'INACTIVE_MONITOR', 'INCIDENT_OWNER_UPDATE', 'INCIDENT_SEVERITY_UPDATE', 'INCIDENT_SLACK_THREAD', 'INCIDENT_STATUS_UPDATE', 'JSON_SCHEMA_CHANGE', 'METRIC_ANOM', 'QUERY_RUNTIME_ANOM', 'SCHEMA_CHANGE', 'SIZE_ANOM', 'SIZE_DIFF', 'UNCHANGED_SIZE_ANOM')
...
class IncidentCategory(sgqlc.types.Enum):
    ...
    * `schema`None
    '''
    __schema__ = schema
    __choices__ = ('dimension', 'field_health', 'freshness', 'schema', 'sql_rule', 'volume')
...
class CustomRuleComparisonInput(sgqlc.types.Input):
    ...
    '''Time to look back for SLOs which compare current and past values.'''
...
class Account(sgqlc.types.Type):
    __schema__ = schema
    __field_names__ = ('id', 'uuid', 'name', 'created_on', 'config', 'allow_non_sso_login', 'data_share', 'notification_settings', 'data_collectors', 'users', 'user_invites', 'user_account_before_switch', 'warehouses', 'bi', 'connections', 'tableau_accounts', 'slack_credentials', 'slack_channels', 'slack_msg_details', 'resources', 'account_domains', 'slack_credentials_v2', 'identity_provider', 'active_collection_regions', 'internal_notifications')
...
class Connection(sgqlc.types.relay.Connection):
    __schema__ = schema
    __field_names__ = ('id', 'uuid', 'type', 'account', 'warehouse', 'bi_container', 'job_types', 'credentials_s3_key', 'integration_gateway_credentials_key', 'data', 'created_on', 'updated_on', 'dbt_projects', 'connection_identifiers', 'job_errors')
    id = sgqlc.types.Field(sgqlc.types.non_null(ID), graphql_name='id')
...
class DownstreamReport(sgqlc.types.Type):
    __schema__ = schema
    __field_names__ = ('mcon', 'display_name', 'report_type', 'object_id', 'owner_id', 'is_custom')
    mcon = sgqlc.types.Field(sgqlc.types.non_null(String), graphql_name='mcon')
    '''MCON of the report.'''
...
class Query(sgqlc.types.Type):
    ...
    * `namespace` (`String`)None
    ...
    export_monte_carlo_config_templates = sgqlc.types.Field(MonteCarloConfigTemplateExportResponse, graphql_name='exportMonteCarloConfigTemplates', args=sgqlc.types.ArgDict((
        ...
        ('export_name', sgqlc.types.Arg(Boolean, graphql_name='exportName', default=None)),
))
    )
    ...
    * `monitor_uuids` (`[UUID]!`): List of custom monitor uuids to
      export
    * `export_name` (`Boolean`): Indicates if must include the name in
    ...
    * `anchor_event` (`UUID!`)None (default: `"Anomaly event used for
    ...
    perform_correlation_sampling = sgqlc.types.Field(CorrelationSamplingResult, graphql_name='performCorrelationSampling', args=sgqlc.types.ArgDict((
        ...
        ('max_values', sgqlc.types.Arg(Int, graphql_name='maxValues', default=None)),
))
    )
...
class Incident(sgqlc.types.Type, Node):
    __schema__ = schema
    __field_names__ = ('uuid', 'warehouse', 'created_time', 'updated_time', 'owner', 'severity', 'system_set_severity', 'feedback', 'feedback_time', 'last_update_user', 'project', 'dataset', 'incident_type', 'incident_sub_types', 'incident_time', 'events', 'incident_reaction', 'slack_msg_details', 'reaction_type', 'summary', 'suggested_owner', 'topology')
    uuid = sgqlc.types.Field(sgqlc.types.non_null(UUID), graphql_name='uuid')
...
class User(sgqlc.types.Type, Node):
    __schema__ = schema
    __field_names__ = ('cognito_user_id', 'email', 'first_name', 'last_name', 'display_name', 'state', 'created_on', 'is_sso', 'sso_groups', 'sso_groups_updated_at', 'token_id', 'notification_settings_added', 'notification_settings_modified', 'invitees', 'eventmodel_set', 'incident_reactions_created', 'incident_reactions_modified', 'user_comments', 'creator', 'metricmonitoringmodel_set', 'combinedtablestatsmodel_set', 'object_properties', 'catalog_object_metadata', 'docs', 'doc_authors', 'lineage_nodes', 'lineage_edges', 'resources', 'lineage_block_patterns', 'lineage_repl_rules', 'monte_carlo_config_templates', 'domain_created_by', 'slack_credentials_v2', 'custom_users', 'unified_users', 'last_updated_unified_users', 'account', 'role', 'auth')
    cognito_user_id = sgqlc.types.Field(sgqlc.types.non_null(String), graphql_name='cognitoUserId')
