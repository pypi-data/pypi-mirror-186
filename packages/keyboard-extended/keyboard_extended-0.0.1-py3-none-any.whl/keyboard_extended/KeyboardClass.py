...
class Key:
    ...
    all_keys = []
    name_self_dict = {}
    standard_modifiers = frozenset(["alt", "linke windows", "rechte windows", "strg", "umschalt", "feststell", "tab", "strg", "anwendung"])
    aliase = {
    ...
    for k, v in aliase.items():
        for c in v: aliase_di[c] = k
    scancode_self_dict = {}
    # last_200 = []



    time_to_clicks_standard = {0.5: 9,
                            1.0: 24,
                            1.5: 39,
                            2.0: 54,
                            2.5: 69,
    ...
    def __init__(self, 
        ...
        self.key_chains = []
        upper_f = ('f13', 'f14', 'f15', 'f16', 'f17', 'f18', 'f19', 'f20', 'f21', 'f22', 'f23', 'f24')
        if name in upper_f: self.timer_to_pressed_counter = Key.time_to_clicks_upper_f
        else: self.timer_to_pressed_counter = Key.time_to_clicks_standard
        self.last_2000 = []
    ...
    def state(self, value):
        ...
        self.active_modifiers = [x for x in list(Key.standard_modifiers) if Key.name_self_dict[x].state=="down"]
        
        self._state = value
        if value == "up":
            for c, args, send_self in self._callbacks_up:
                if send_self:
                    if args != None: c(self, *args)
                    else: c(self)
                else:
                    if args != None: c(*args)
                    else: c()
    ...
    def bind(self, callback:Callable, args:Iterable=None, state:str="down", send_self:bool=False):
        """
    ...
    def unbind(self, callback:Callable, state:str="down"): 
        ...
        if   state == "up":
            for (callback_, args, send_self) in self._callbacks_up:
                if callback == callback_: to_remove.append((callback, args, send_self))
            for n in to_remove: self._callbacks_up.remove(n)
        elif state == "down":
        ...
        try:
            for keyname in Key.aliase[self._name]:
                ...
                for f, cs in self._alias_bound_timed_functions.items():
                    try: 
                        if callback == f:
                            try: key.unbind(cs[0], "down")
                            except: pass
                            try: key.unbind(cs[1], "down")
                            except: pass
                            try: key.unbind(cs[0], "up")
                            except: pass
                            try: key.unbind(cs[1], "up")
                            except: pass
                    except: pass
        except: pass
    ...
    def timed_hotkey(self, callback:Callable, timer:float=3, args:Iterable=None, send_self:bool=False, sleep_after_execution:float=0):
        class HotkeyTimer:
        ...
        def wait_for_call():
            while True:
                v = t.q.get()
                if send_self:
                    if args != None: callback(self, *args)
                    else: callback(self)
                else:
                    if args != None: callback(*args)
                    else: callback()
                t.q = Queue()
                t.fired = False
        ...
        def timed_hotkey_caller():
            if time()-self.down_time > t.timer and not t.fired and self.state == "down":
                t.fired = True
                self.down_time = time()
                t.q.put(0)
    ...
    def timed_hotkey_clickrate_based(self, callback:Callable, timer:float=3, args:Iterable=None, send_self:bool=False, sleep_after_execution:float=1): 
        try:
            self.timer_to_pressed_counter[round(float(timer), 1)]
        except:
            raise ValueError("timer of <Key object>.timed_hotkey_clickrate_based needs to be a multiple of 0.5 but may not exceed 30.")
        
        def check_pressed(callback:Callable, timer:float=3, args:Iterable=None, send_self:bool=False, sleep_after_execution:float=0): 
            ...
            try:
                ...
                if round(latest_event.time - event.time, 1) <= timer and round(time() - event.time, 1) <= timer and round(time() - latest_event.time, 1) <= 0.2: 
                    if args != None:
                        if send_self: callback(self, *args)
                        else: callback(*args)
                    else:
                        if send_self: callback(self)
                        else: callback()
                    sleep(sleep_after_execution)
                else: pass # print(round(time() - event.time, 1), timer)
            except Exception as e: pass # print(e)
        
        self.bind(check_pressed, args=[callback, timer, args, send_self, sleep_after_execution])


def get_Key(name:str, state:str="up", 
        last_up_time:float=None, 
        last_down_time:float=None, 
        is_keypad:bool=None, 
        scan_code:int=None, 
    ...
    Returns:
        Key: Instance of Key for keyboard bound functions.
    """
    try: return Key.name_self_dict[name]
    except: return Key(name, state, last_up_time, last_down_time, is_keypad, scan_code, active_modifiers)

def getKey(name:str, state:str="up", 
        last_up_time:float=None, 
        last_down_time:float=None, 
        is_keypad:bool=None, 
        scan_code:int=None, 
    ...
    Returns:
        Key: Instance of Key for keyboard bound functions.
    """
    return get_Key(name, state, last_up_time, last_down_time, is_keypad, scan_code, active_modifiers)
...
def keyboard_hook_callback(event:KeyboardEvent):
    ...
    if round(time()-event.time, 2) < 0.1:
        ...
        try: 
            try: name = Key.aliase_di[event.name]
            except: name = Key.aliase[event.name]
        ...
        for key in keys:
            key: Key
            if event.event_type == "up"     and key._last_state != event.event_type: key.up_time = event.time
            elif event.event_type == "down" and key._last_state != event.event_type: key.down_time = event.time
            ...
            if len(key.last_2000) > 2000: key.last_2000.pop(0)
...
def keyboard_hook_callback_with_callbacks_queued(event:KeyboardEvent):
    ...
    for key in keys:
        key: Key
        if event.event_type == "up"     and key.up_time < key.down_time: key.up_time = event.time
        elif event.event_type == "down" and key.up_time > key.down_time: key.down_time = event.time
        if event.event_type == "up":
            key.last_up_time = event.time
        elif event.event_type == "down":
            key.last_down_time = event.time
...
def init():
    try: unhook(keyboard_hook_callback_with_callbacks_queued)
    except: pass
...
if __name__ == "__main__":
    ...
    def test(key):
        print(Fore.BLACK+"call by", f"'{Fore.YELLOW}{key.name}'", Fore.BLACK+"with current state:", f"'{Fore.YELLOW}{key.state}'", Fore.BLACK+"and current time:", time())
        sleep(0.4)
