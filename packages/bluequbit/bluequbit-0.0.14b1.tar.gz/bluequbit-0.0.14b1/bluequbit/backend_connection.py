...
from .exceptions import BQBaseException, BQUnauthorizedException
...
class BackendConnection:
    def __init__(self, api_token=None, config_location=None):
        ...
        if not config_location.is_file():
            if from_none:
                raise BQBaseException(
                    0,
                    "Please specify an API key to init(). You can find your API key once you log in to https://dev.app.bluequbit.io",
                )
            api_config = {
                "token": api_token,
        ...
        try:
            ...
            if not response.ok:
                ...
                if response.status_code == 401:
                    raise BQUnauthorizedException()
                raise BQBaseException(0, "Couldn't reach BlueQubit " + response.text)
        except:
            self._authenticated = False
            self._session = None
            raise
    ...
    def send_request(self, req_type, path, params=None, json_req=None):
        if not self._authenticated:
            raise BQBaseException(
                0,
                "BQ was not initialized. Please provide correct API token to bluequbit.init(<token>).",
            )
