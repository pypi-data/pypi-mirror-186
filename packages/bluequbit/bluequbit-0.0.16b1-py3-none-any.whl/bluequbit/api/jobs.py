...
from ..exceptions import BQBaseException
...
def estimate_job_runtime(_connection, circuit):
    ...
    if not response.ok:
        raise BQBaseException(
            ...
            "Couldn't estimate job. " + response.json()["error_message"],
        )
...
def get(_connection, job_id):
    ...
    if not response.ok:
        raise BQBaseException(response.status_code, "Couldn't get job " + response.text)
    return response.json()["data"]
