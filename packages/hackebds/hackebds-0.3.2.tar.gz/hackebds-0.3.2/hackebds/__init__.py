...
def get_version():
    return Fore.GREEN+"Version: 0.3.2"+Fore.RESET
...
def num_get_power_bind_shell(num, shell_path, listen_port, passwd, filename):
	fun = power_bind_shell_dic.get(num)
	return fun(shell_path, listen_port, passwd, filename)
		...
		try:
			if (args.passwd!=None and args.bind_port != None and args.arch!=None and args.encode == False ):
				if (check_port(args.bind_port)==True):				
					if (args.power == True):
						num_get_power_bind_shell(arch_get_number(args.arch), args.shell ,args.bind_port, args.passwd, args.filename)
						return 
