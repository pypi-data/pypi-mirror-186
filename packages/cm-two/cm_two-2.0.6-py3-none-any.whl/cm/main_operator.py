...
class Operator:
    ...
    def __init__(self, root, settings, scale, can, deffaultScreenSize,
        ...
        if self.settings.mirrored:
            self.auto_exit_x, self.auto_exit_y = 1615, 862
            self.gross_x, self.gross_y = 417, 862
        else:
            self.auto_exit_x, self.auto_exit_y = 417, 862
            self.gross_x, self.gross_y = 1615, 862
    ...
    def cad_work(self, info):
        ...
        if info['cam_type'] == 'cad_gross':
            x = self.gross_x - self.auto_cams_width/2 - 5
            y = self.gross_y - self.auto_cams_height/2 + 10
            self.terminal.can.create_rectangle(x,y,x+self.auto_cams_width+10,
                                               y+self.auto_cams_height-31,
                                               #fill="#62ff24",
                                               fill="#E2E2E2",
                                               tag='cad_color')
        else:
            x = self.auto_exit_x - self.auto_cams_width/2 - 5
            y = self.auto_exit_y - self.auto_cams_height/2 + 10
            self.terminal.can.create_rectangle(x,y,x+self.auto_cams_width+10,
                                               y+self.auto_cams_height-31,
                                               fill="#E2E2E2",
                                               tag='cad_color')
    ...
    def draw_auto_entrance_pic(self, picture):
        ...
        im_r = img.crop((450, 400, 1000, 1150))
        img_byte_arr = io.BytesIO()
        im_r.save(img_byte_arr, format='PNG')
        img_byte_arr = img_byte_arr.getvalue()
        self.photoimg = ImageTk.PhotoImage(
