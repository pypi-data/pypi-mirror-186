...
You can also try functions ```ultra_align()``` and ```ultra_diff()``` for multi channel images and extreme precision, but it may require more resources
...
def ultra_align(ref_image: np.ndarray, science_image: np.ndarray) -> np.ndarray:
    ...
    ultra_align()
    ===
    A more advanced version of the alignment function that holds more information to perform the calculations, which results in a more precise alignment.
...
def ultra_diff(ref_image, science_image):
    ...
    ultra_diff()
    ===
    A more advanced version of the difference function that holds more information to perform the calculations, which results in a more precise alignment.
