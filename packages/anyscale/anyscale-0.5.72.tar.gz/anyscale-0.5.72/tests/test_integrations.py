...
@pytest.mark.parametrize("_try_get_ray_job_id_output", [None, "mock_ray_job_id"])
def test_set_wandb_project_group_env_vars_for_ray_job(
    ...
    _try_get_ray_job_id_output: Optional[str],
):
    ...
    with patch.dict(os.environ, os_env_var_dict), patch.multiple(
        ...
        get_auth_api_client=Mock(return_value=Mock(api_client=mock_api_client)),
        slugify=mock_slugify,
        _try_get_ray_job_id=Mock(return_value=_try_get_ray_job_id_output),
    ), patch.dict("sys.modules", ray=mock_ray):
        set_wandb_project_group_env_vars()
        if _try_get_ray_job_id_output:
...
def test_wandb_send_run_info_hook(mock_wandb, os_env_var_dict):
    ...
    mock_search_ray_job = Mock(return_value=Mock(id="mock_ray_job_id"))
    ...
    with patch.dict("sys.modules", wandb=mock_wandb), patch.dict(
        ...
        _search_ray_job=mock_search_ray_job,
    ):
        if not mock_wandb:
            with pytest.raises(Exception):
            ...
            if os.environ.get("RAY_JOB_ID"):
                mock_search_ray_job.assert_called_once_with(
                    os.environ.get("RAY_JOB_ID"), "mock_cluster_id", mock_api_client
                )
...
def test_search_ray_job():
    mock_api_client = Mock()
    mock_ray_job = Mock(ray_job_id="mock_ray_job_id")
    mock_ray_job.name = "mock_ray_job_name"

    mock_api_client.list_decorated_interactive_sessions_api_v2_decorated_interactive_sessions_get = Mock(
        side_effect=[
            Mock(results=[], metadata=Mock(next_paging_token="mock_paging_token_val")),
            Mock(results=[mock_ray_job], metadata=Mock(next_paging_token=None)),
        ]
    )

    assert (
        _search_ray_job("mock_ray_job_id", "mock_cluster_id", mock_api_client)
        == mock_ray_job
    )
    mock_api_client.list_decorated_interactive_sessions_api_v2_decorated_interactive_sessions_get.assert_any_call(
    ...
    mock_api_client.list_decorated_interactive_sessions_api_v2_decorated_interactive_sessions_get.assert_any_call(
        ...
        sort_field=JobsSortField.CREATED_AT,
        count=20,
        paging_token="mock_paging_token_val",
    )
