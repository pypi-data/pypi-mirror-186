    ...
    def transform_path_non_affine(self, path):
        ...
        for vertex, code in path.iter_segments(simplify=False):
            if last_vertex is None or code == Path.MOVETO:
                ...
                lons.append(np.array(vertex[0]))
                lats.append(np.array(vertex[1]))
                ...
                # THIS CUT VALUE NEEDS TO BE INVESTIGATED (CAREFULLY)
                split, = (np.hypot(dxdy[:, 0], dxdy[:, 1]) > 0.5).nonzero()
                ...
                if split.size > 0:
                    ...
                    lonlats_temp = np.array(self._geod.npts(lonlats_step[index, 0],
                    ...
                    split_temp, = (np.hypot(dxdy_temp[:, 0], dxdy_temp[:, 1]) > 0.5).nonzero()

                    lonlats_step = np.insert(lonlats_step,
                                             index,
                                             lonlats_temp,
                                             axis=0)

                    split[0] += split_temp[0]

                split = np.append(split, lonlats_step.shape[0])

                last_cut = 0
                for cut in split:
                    lons.append(lonlats_step[last_cut: cut, 0])
                    lats.append(lonlats_step[last_cut: cut, 1])
                    last_cut = cut
                    codes.extend([Path.LINETO]*len(lonlats_step))

                lonlats.extend(lonlats_step.tolist())
