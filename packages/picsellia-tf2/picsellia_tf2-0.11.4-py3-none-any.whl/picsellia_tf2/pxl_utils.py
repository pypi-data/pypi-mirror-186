...
def format_coco_file(imgdir, annotations, train_assets, test_assets):
    ...
    for image in annotations["images"]:
        image["path"] = os.path.join(imgdir, image["file_name"])
        tmp = image.copy()
        try:
            tmp["annotations"] = formatted_annotations[image["id"]]
            if image["file_name"] in train_assets_fname:
                formatted_file_train["images"].append(tmp)
            if image["file_name"] in test_assets_fname:
                formatted_file_test["images"].append(tmp)
        except KeyError:
            pass
    return formatted_file_train, formatted_file_test
...
def create_record_files(
    ...
    train_annotations, eval_annotations = format_coco_file(
        imgdir=imgdir,
        annotations=annotations,
        train_assets=train_list,
        test_assets=eval_list,
    )
    label_map = label_map_util.load_labelmap(label_path)
    label_map = label_map_util.get_label_map_dict(label_map)
    datasets = ["train", "eval"]

    annotations_ensemble = [train_annotations, eval_annotations]
...
def tf_events_to_dict(path, type=""):
    ...
    if not file_found:
        raise FileNotFoundError(
            "No tfEvent file found in this directory {}".format(path)
        )
