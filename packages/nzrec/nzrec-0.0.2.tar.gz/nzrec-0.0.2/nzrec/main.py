    ...
    def __init__(self, data_path, download_files=False, only_missing=True):
        ...
        if missing_files:
            if download_files:
                ...
                raise ValueError('There are missing data files. Files would have been downloaded if download_files=True (but it is not). Othersize if this was a mistake, please check/change the data_path: {}'.format(', '.join(missing_files)))
...
class Way(Feature):
    ...
    def upstream(self):
        ...
        """
        ways_up = utils.find_upstream(self._water._node_id, self._water._node_way_index, self._water._way, self._water._way_index)
    ...
    def downstream(self):
        ...
        """
        ways_down = utils.find_downstream(self._water._node_id, self._water._node_way_index, self._water._way, self._water._way_index)
