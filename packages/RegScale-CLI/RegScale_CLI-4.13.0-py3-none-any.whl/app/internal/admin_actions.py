...
def send_reminders(days: int):
    ...
    with job_progress:
        ...
        create_threads(
            ...
            args=(api, users, base_url, str(days), config, getting_items),
        ...
        if len(tenant_pipeline) > 0:
            ...
            dict_users = {users[i]["id"]: users[i + 1] for i in range(0, len(users), 2)}
            create_threads(
                process=analyze_pipeline,
                args=(config, analyze_items, dict_users),
...
def get_upcoming_or_expired_items(args: Tuple, thread: int) -> None:
    ...
    for i in range(len(threads)):
        ...
        workflows = api.get(
            url=f"{base_url}workflowInstances/userOpenItemsDays/{user['id']}/{days}"
        ).json()
            ...
            for pipe in pipelines:
                ...
                for item in pipelines[pipe]["Pipeline"]:
                    ...
                    for key in item.keys():
                        ...
                        elif "ById" in key or key.lower() == "ownerid":
                            # remove ById from the key
                            new_key = key.replace("Id", "")

                            # uncamel case the key
                        ...
                        # see if it's a by id field, replace the id with a user
