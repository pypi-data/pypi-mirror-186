...
def query_vuln(query_id: int, regscale_ssp_id: int, create_issue: bool):
...
@click.option(
    ...
    "--plugins",
    multiple=True,
    help="Enter one or more pluginID's seperated by a space to see a trendline. (by report date)",
...
def q_vuln(query_id: int, ssp_id: int, create_issue: bool) -> list:
    ...
    :param bool create_issue: Whether to create an issue in RegScale
...
def map_tenable_severity_to_regscale(code) -> str:
    """
    Map Severity Codes to RegScale
    :param code: Severity from Tenable to determine severity in RegScale
    :return: String to use in RegScale for severity
    :rtype: str
    """
    if code == "Low":
