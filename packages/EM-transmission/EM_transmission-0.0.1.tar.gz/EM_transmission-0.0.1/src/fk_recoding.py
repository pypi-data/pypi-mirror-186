...
class Fk_viewer(object):
    def __init__(self):
        ...
        self.path="recoding_file"
        #self.target_path="server_point"
        self.target_path="Documents/fk_img"
        #self.view = cv2.VideoCapture(0)
        self.view = cv2.VideoCapture('rtsp://admin:admin@192.168.1.101:554/media/1/1')
        ...
        while True:
            ...
            if self.out is None or not self.out.isOpened():
                if self.t_img is None or not self.t_img.is_alive():
                    ...
                    self.txt_idx_str=str(self.txt_idx)+".0" 
                    self.t_img = threading.Thread(target=self.sample_viewThread, args=(self.txt_idx_str,))
    ...
    def sample_viewThread(self, txt_idx):
        #print("txt_dix: ",txt_idx)
        fr =[]
        idx=0
        mbyte_cnt=0
        setting_frame_cnt=100
        ...
        while True:
            try:
                ...
                if self.file_exist:
                    try:
                        session = ftplib.FTP()
                        session.connect('172.17.6.166', 21) 
                        session.login("yhkim", "Kim`123")
                        ...
                        for file_tr_name in file_tmp:
                            ...
                            session.storbinary("STOR " + f"{self.target_path}/{file_tr_name}", uploadfile,20480,
