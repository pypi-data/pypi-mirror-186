...
print("Motor de Calidad Version Release 1.6.3")
...
def validateRules(object:DataFrame,rules:dict,registerAmount:int, entity: str, project:str,country: str,domain: str,subDomain: str,segment: str,area: str,error,dataDate:str,validData:str, Data):
    ...
    try:
        if validData == "TRUE":
            finalData = errorData.join(object,on = Data.get(JsonParts.Fields), how = LeftAntiType)
    ...
    return validationData.select(
        ...
        CountryId.value(when(lit(country).isNull(),upper(lit(dbutils.widgets.get('country')))).otherwise(country)),
        Country.value(when(lit(country).isNull(),upper(lit(dbutils.widgets.get('country')))).otherwise(country)),
