...
def lazy_import():
    ...
    globals()['BaseTransaction'] = BaseTransaction
    ...
    globals()['ExternalCardTransactionAllOf'] = ExternalCardTransactionAllOf
    globals()['ExternalCardTransactionSubtypes'] = ExternalCardTransactionSubtypes


class ExternalCardTransaction(ModelComposed):
    ...
    def openapi_types():
        ...
        return {
            ...
            'subtype': (ExternalCardTransactionSubtypes,),  # noqa: E501
    ...
    def _from_openapi_data(cls, *args, **kwargs):  # noqa: E501
        """ExternalCardTransaction - a model defined in OpenAPI

        Keyword Args:
            amount (int): The total amount of the transaction including both pending and already posted amounts. The value is represented as the smallest denomination of the applicable currency.
            creation_time (datetime): The exact time the transaction was recorded in the ledger
            currency (str): ISO 4217 alphabetic currency code of the transfer amount
            dc_sign (str): The `dc_sign` represents the direction money was moved. A value of `DEBIT` is money moving out of an account, a value of `CREDIT` is money moving into an account
            ...
            force_post (bool): Determines whether or not a transaction or auth was \"forced\" or not. A forced transaction skips any account balance checks
            group_id (str): The group id of the transaction. Every transaction in the ledger is one entry in a double-entry system and the primary and offset transactions share the same `group_id`
            id (str): The unique identifier of the transaction
            last_updated_time (datetime): The date and time the transaction was last modified
            partial_hold (bool): Determines whether or not the funds on hold were the result of a partial auth or not. If `true` the `pending_amount` of the transaction will be less than the requested amount. This is primarily used for certain types of card transactions.
            pending_amount (int): The amount amount of the transaction currently authorized or on hold
            posted_amount (int): The amount of the transaction that has been fully posted to the account
            status (str): The status of the transaction
            transaction_time (datetime): The time the transaction occurred. In most cases this will be roughly identical to creation_time, but it can differ in some situations if the payment doesn't appear in the ledger in real-time.
            ...
            external_card_transaction (CardTransactionData):
            subtype (ExternalCardTransactionSubtypes):
